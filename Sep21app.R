...
ui <- fluidPage(
  ...
  tabsetPanel(id = "main_tabs",
      ...
      fluidRow(
        column(3,
          ...
          div(class = "moab-card",
            ...
            selectInput("top_n_leagues", "League tier filter:",
                        choices = c("Top 1 per country"        = "1",
                                    "Top 2 per country"        = "2",
                                    "Top 3 per country"        = "3",
                                    "Top 4 per country"        = "4",
                                    "Top 5 per country"        = "5",
                                    "SB-verified in mapper only" = "sb",
                                    "All (no filter)"          = "all"),
                        selected = "all"),
            radioButtons("country_filter_mode", "Country filter:",
                         choices = c("All countries"    = "all",
                                     "Handpicked tier"  = "handpicked",
                                     "Custom selection" = "custom"),
                         selected = "all"),
            conditionalPanel(
              condition = "input.country_filter_mode == 'custom'",
              selectizeInput("country_custom_pick", "Pick countries:",
          ...
          div(class = "moab-card",
            ...
            tabsetPanel(
              ...
              tabPanel("Rule Inspector",
                ...
                fluidRow(
                  column(6,
                         numericInput("inspector_rule_pick", "Rule rank to inspect (from leaderboard):",
    ...
    tabPanel("Stage 2",
      ...
      fluidRow(
        column(3,
          ...
          uiOutput("s2_manual_picker"),
          actionButton("s2_run_selected_btn", "Run selected only",
        ...
        column(9,
          ...
          tabsetPanel(
            ...
            tabPanel("Kill breakdown (manual run only)",
                     br(),
                     p(em("For each Stage 1 pick, shows which intuition(s) rejected it. Populated only after 'Run selected only'.")),
                     DTOutput("s2_kill_breakdown")
            ),
    ...
    # ═══════════════════════════════════════════════════════════
    # TAB — FEATURE RANKER (solo + pair importance)
    # ═══════════════════════════════════════════════════════════
    tabPanel("Feature Ranker",
             br(),
             p(em("Ranks both individual features AND feature pairs by predictive power for the picked market. Pair scores flag genuine interactions (joint info > sum of solos) vs redundant duplicates (joint info ≈ solo). Send top N walks down the list: solos take 1 slot, interaction pairs take 2, redundant pairs get skipped so slots go to genuinely different signals.")),
             hr(),
             fluidRow(
               column(4,
                      div(class = "moab-card",
                          h4("Feature scope"),
                          p(em("Which categories to include. Stats/standings default off (thin coverage in early rounds).")),
                          checkboxGroupInput("ranker_categories", "Categories:",
                                             choices = c("goals", "h2h", "poisson", "goal_timing", "half_rate",
                                                         "stats", "standings", "meeting_point"),
                                             selected = c("goals", "h2h", "poisson", "goal_timing", "half_rate"))
                      ),
                      div(class = "moab-card",
                          h4("Forest settings"),
                          numericInput("ranker_ntree", "Number of trees:",
                                       value = 1000, min = 100, max = 3000, step = 100),
                          numericInput("ranker_top_n", "Top N features to send:",
                                       value = 9, min = 3, max = 30, step = 1),
                          checkboxInput("ranker_use_validate",
                                        "Also rank on Validate set (for consistency)",
                                        value = FALSE),
                          numericInput("ranker_interaction_threshold",
                                       "Interaction lift threshold (% above best solo):",
                                       value = 0.15, min = 0.05, max = 1.0, step = 0.05),
                          p(em("Pair scored X% above its best solo = interaction (green). Below best solo = redundant (red). Between = weak pair (grey)."))
                      ),
                      div(class = "moab-card",
                          actionButton("ranker_run_btn", "Rank features + pairs",
                                       class = "btn-primary", style = "width: 100%"),
                          br(), br(),
                          actionButton("ranker_send_top_btn",
                                       "Send top N to Ablation (dedup-aware)",
                                       class = "btn-success", style = "width: 100%")
                      ),
                      div(class = "moab-card",
                          verbatimTextOutput("ranker_status")
                      )
               ),
               column(8,
                      div(class = "moab-card",
                          h4("Ranked entries"),
                          verbatimTextOutput("ranker_summary"),
                          DTOutput("ranker_table")
                      )
               )
             )
    ),
    ...
    # ═══════════════════════════════════════════════════════════
    # TAB — ABLATION (scout tool for grid_spec discovery)
    # ═══════════════════════════════════════════════════════════
    tabPanel("Ablation",
             br(),
             p(em("Scout what feature combinations produce the strongest rules for the picked market. Tests every combination at each depth level with per-feature threshold sweeps. Output = ready-to-paste grid_spec code for the top winners; you then run normal Grid Search on that spec to inspect ranks.")),
             hr(),
             fluidRow(
               column(4,
                      div(class = "moab-card",
                          h4("Candidates"),
                          p(em("Pick 2-12 features to combine. All threshold sweeps happen automatically per feature type.")),
                          selectizeInput("ablation_features", "Features to test:",
                                         choices = NULL, multiple = TRUE,
                                         options = list(placeholder = "Type to search features..."))
                      ),
                      div(class = "moab-card",
                          h4("Combination depth"),
                          p(em("Depths available automatically match your feature count.")),
                          numericInput("ablation_max_depth", "Max depth to test:",
                                       value = 8, min = 1, max = 20, step = 1),
                          checkboxGroupInput("ablation_depths", NULL,
                                             choices = c("Pick features first" = "none"),
                                             selected = character(0))
                      ),
                      div(class = "moab-card",
                          h4("Data pools"),
                          p(em("Test each combo against Tune only, or Tune + Validate side-by-side. Validate requires files tagged Validate in the Data tab.")),
                          checkboxInput("ablation_use_validate",
                                        "Also evaluate on Validate set", value = FALSE)
                      ),
                      div(class = "moab-card",
                          h4("Filter"),
                          numericInput("ablation_min_picks",
                                       "Min picks per rule:",
                                       value = 30, min = 1, step = 5),
                          numericInput("ablation_top_per_combo",
                                       "Top N rules per combo:",
                                       value = 1, min = 1, max = 5, step = 1),
                          radioButtons("ablation_sort_by", "Sort winners by:",
                                       choices = c("Hit rate" = "hit_rate",
                                                   "Losses (asc)" = "losses",
                                                   "Volume" = "picks"),
                                       selected = "hit_rate")
                      ...
                      div(class = "moab-card",
                          verbatimTextOutput("ablation_size_estimate"),
                          br(),
                          actionButton("ablation_run_btn", "Run Ablation",
                                       class = "btn-primary", style = "width: 100%")
                      )
               ),
               column(8,
                      div(class = "moab-card",
                          h4("Winners"),
                          verbatimTextOutput("ablation_summary"),
                          DTOutput("ablation_results_table")
                      ),
                      div(class = "moab-card",
                          h4("Copy-ready grid_spec"),
                          p(em("Select a row in the table above; the corresponding grid_spec code appears here. Paste into your market file, save, then run Grid Search to inspect all ranks.")),
                          verbatimTextOutput("ablation_spec_out")
                      )
               )
             )
    ),
    ...
    # ═══════════════════════════════════════════════════════════
    tabPanel("Push cassette",
             br(),
             fluidRow(
               column(4,
                      h4("Push a tuned rule into production"),
                      p(em("Writes a self-contained cassette file to BASE_PATH/cassettes/<market>.R. Main MOAB picks it up on next Predictions run. Backups of any prior cassette for the same market are stamped with timestamp.")),
                      hr(),
                      h4("Source"),
                      radioButtons("push_source", NULL,
                                   choices = c("Current pipeline (Stage 1 + Stage 2 if run)" = "pipeline",
                                               "Rule rank from Rule Inspector"               = "rank"),
                                   selected = "pipeline"),
                      conditionalPanel(
                        condition = "input.push_source == 'rank'",
                        numericInput("push_rank", "Rank in current leaderboard:",
                                     value = 1, min = 1, step = 1),
                        p(em("Uses the CURRENT market picker + sort/floor/dedup settings from Grid Search tab. Stage 2 filter will NOT be included with this path — Stage 1 only."))
                      ),
                      hr(),
                      h4("Cassette filename"),
                      textInput("push_key", "Market key (used as filename & internal name):",
                                value = "",
                                placeholder = "e.g. over_15, over_25, btts, dc_1x"),
                      p(em("Cassettes overwrite by key. Lowercase, no spaces. Convention matches auto-bettor market strings (over_15 -> 'Over 1.5' etc).")),
                      hr(),
                      actionButton("push_preview_btn", "Preview cassette",
                                   class = "btn-primary", style = "width: 100%"),
                      br(), br(),
                      actionButton("push_apply_btn", "Write cassette to disk",
                                   class = "btn-warning", style = "width: 100%"),
                      hr(),
                      verbatimTextOutput("push_status")
               ),
               column(8,
                      h4("Cassette preview"),
                      p(em("What will be written to BASE_PATH/cassettes/<key>.R")),
                      verbatimTextOutput("push_preview_out")
               )
             )
    ),
...
server <- function(input, output, session) {
  ...
  rv <- reactiveValues(
    ...
    # Session 4 additions
    drift_action_status = "",
    # Push cassette additions (replace old deploy)
    push_preview = NULL,
    push_status = "",
    # Session 5 additions
    ...
    snapshot_status = "",
    launcher_snippet = "",
    # Top-N filter additions
    league_ranks = NULL,
    # Country filter additions
    country_list = character()
  )
  
  
  # Load league ranks + country list once on startup
  isolate({
    rv$league_ranks <- load_league_ranks()
    rv$country_list <- list_countries_for_selector()
  })
  
  # Populate country selector once ranks/list are loaded
  observe({
    updateSelectizeInput(session, "country_custom_pick",
                         choices = rv$country_list,
                         server = TRUE)
  })
  ...
  output$files_table <- renderDT({
    ...
    datatable(df, options = list(pageLength = 10, dom = "t"),
              editable = if (rv$load_mode == "multi") TRUE else FALSE,
              rownames = FALSE)
  })
  observeEvent(input$files_table_cell_edit, {
    ...
    rv$files_df <- rv$files_df  # force reactive trigger
    showNotification(paste0("Role set to ", nv), type = "message")
  })
  ...
  build_fixtures_pool <- reactive({
    if (is.null(rv$files_df) || nrow(rv$files_df) == 0) return(NULL)
    # Resolve top-N choice
    tn_raw <- input$top_n_leagues %||% "all"
    use_sb <- identical(tn_raw, "sb")
    top_n <- if (tn_raw %in% c("all", "sb")) NULL else as.integer(tn_raw)
    sb_pairs <- if (use_sb) load_sb_verified_pairs() else NULL
    # Resolve country choice
    cmode <- input$country_filter_mode %||% "all"
    keep_countries <- switch(cmode,
                             "all"        = NULL,
                             "handpicked" = HANDPICKED_COUNTRIES,
                             "custom"     = input$country_custom_pick,
                             NULL)
    if (identical(cmode, "custom") &&
        (is.null(keep_countries) || length(keep_countries) == 0)) {
      # Empty custom = keep everything (same as "all") so user sees data
      keep_countries <- NULL
    }
    
    apply_filters <- function(fx) {
      fx <- filter_final_only(fx)
      fx <- filter_fixtures_by_country(fx, keep_countries)
      fx <- filter_fixtures_top_n(fx, top_n, rv$league_ranks)
      fx <- filter_fixtures_by_sb_verified(fx, sb_pairs)
      fx
    }
    ...
    if (rv$load_mode == "single") {
      ...
      list(tune = apply_filters(res$fixtures), validate = list(), test = list())
    } else {
      out <- load_multi_files(rv$files_df)
      list(
        tune     = apply_filters(out$tune),
        validate = apply_filters(out$validate),
        test     = apply_filters(out$test)
      )
    }
  })
  ...
  render_leaderboard_with_rounds <- function(res, sort_by, min_picks, dedup,
                                             mode, pool, market,
                                             default_window, main_only) {
    ...
    disp <- cbind(Rank = seq_len(nrow(disp)), disp)
    # Add per-round columns if multiple files loaded
    if (!is.null(pool) && !is.null(market) && length(pool$tune) > 0) {
      feats_needed <- unique(unlist(lapply(res$grid_spec,
                                           function(cl) c(cl$feature, cl$feature2))))
      feats_needed <- feats_needed[!is.na(feats_needed) & feats_needed != ""]
      feat_df <- build_feature_matrix(pool$tune,
                                      feature_names = feats_needed,
                                      default_window = default_window,
                                      main_only = main_only)
      if ("source_file" %in% names(feat_df)) {
        rounds <- unique(feat_df$source_file)
        if (length(rounds) > 1) {
          grades <- vapply(seq_len(nrow(feat_df)),
                           function(i) {
                             g <- market$grade_fn(feat_df[i, ])
                             if (is.na(g)) NA_real_ else as.numeric(g)
                           }, numeric(1))
          for (rn in rounds) {
            mask_r <- feat_df$source_file == rn
            rn_clean <- gsub("[^A-Za-z0-9]+", "_", rn)
            rn_clean <- gsub("^_|_$", "", rn_clean)
            picks_col <- integer(nrow(disp))
            hit_col   <- rep(NA_real_, nrow(disp))
            grid <- expand_grid_spec(res$grid_spec)
            for (r in seq_len(nrow(disp))) {
              rid <- lb$rule_id[r]
              rule <- row_to_rule(grid[rid, , drop = FALSE], res$clause_meta)
              passes <- apply_rule(feat_df, rule)
              hits_r <- passes & mask_r & !is.na(grades)
              picks_col[r] <- sum(hits_r)
              if (sum(hits_r) > 0)
                hit_col[r] <- round(sum(grades[hits_r] == 1, na.rm = TRUE) /
                                      sum(hits_r), 4)
            }
            disp[[paste0("picks_", rn_clean)]] <- picks_col
            disp[[paste0("hit_",   rn_clean)]] <- hit_col
          }
        }
      }
    }
  ...
  output$picked_leaderboard <- renderDT({
    render_leaderboard_with_rounds(rv$last_search, input$sort_by,
                                   input$min_picks, input$dedup_lb, "picked",
                                   build_fixtures_pool(),
                                   rv$markets[[input$market_pick]],
                                   input$default_window, effective_main_only())
  })
  ...
  # Manual intuition picker — coexists with category filter
  output$s2_manual_picker <- renderUI({
    if (length(rv$intuitions) == 0) return(NULL)
    nms <- vapply(rv$intuitions, function(x) x$name, character(1))
    checkboxGroupInput("s2_manual_pick", "Manual pick (overrides category + depth):",
                       choices = sort(nms), selected = character(0))
  })
  
  # Run selected only — bypasses category filter and depth sweep,
  # runs the picked intuitions as ONE combo (first tunable value per intuition)
  observeEvent(input$s2_run_selected_btn, {
    ...
    picks <- input$s2_manual_pick %||% character(0)
    if (length(picks) == 0) {
      showNotification("Tick at least one intuition in the manual picker", type = "error"); return()
    }
    market <- rv$markets[[input$market_pick]]
    ...
    intu_active <- Filter(function(x) x$name %in% picks, intu_active)
    if (length(intu_active) == 0) {
      showNotification("Picked intuitions are inactive for this market", type = "error"); return()
    }
    ...
    withProgress(message = "Stage 2 (manual combo)", value = 0.1, {
      incProgress(0.3, detail = "Building feature matrix...")
      INTUITION_FEATURES <- list(
        no_stalemate_teams   = c("h_draws_n5", "a_draws_n5"),
        no_loser_teams       = c("h_losses_n5", "a_losses_n5"),
        no_consec_draws      = c("h_max_consec_draws_n5", "a_max_consec_draws_n5", "h2h_max_consec_draws"),
        last_not_goalless_draw = c("h_last_goalless_draw_n5", "a_last_goalless_draw_n5", "h2h_last_goalless_draw"),
        last_had_1h_goal     = c("h_last_ht_over05_n5", "a_last_ht_over05_n5", "h2h_last_ht_over05"),
        last_had_over15      = c("h_last_over15_n5", "a_last_over15_n5", "h2h_last_over15"),
        last_had_over25      = c("h_last_over25_n5", "a_last_over25_n5", "h2h_last_over25"),
        last_had_btts        = c("h_last_btts_n5", "a_last_btts_n5", "h2h_last_btts"),
        last_had_2h_over05   = c("h_last_2h_over05_n5", "a_last_2h_over05_n5", "h2h_last_2h_over05"),
        no_winless_teams = c("h_wins_n5", "a_wins_n5"),
        last_had_2h_over15   = c("h_last_2h_over15_n5", "a_last_2h_over15_n5", "h2h_last_2h_over15")
      )
      stage1_feats <- unique(unlist(lapply(rule, function(cl) c(cl$feature, cl$feature2))))
      stage1_feats <- stage1_feats[!is.na(stage1_feats) & stage1_feats != ""]
      intu_feats <- unique(unlist(INTUITION_FEATURES[vapply(intu_active, function(x) x$name, character(1))]))
      needed_feats <- unique(c(stage1_feats, intu_feats))
      feat_df <- build_feature_matrix(pool$tune, needed_feats,
                                      default_window = input$default_window,
                                      main_only = effective_main_only())
      
      # Build ONE combo: first param set per picked intuition
      combo <- lapply(intu_active, function(intu) {
        expand_intuition_params(intu)[[1]]
      })
      
      # Baseline = Stage 1 rule's hit rate on its own picks
      passes <- apply_rule(feat_df, rule)
      ...
      grades_all <- vapply(seq_len(nrow(feat_df)), function(i) {
        g <- market$grade_fn(feat_df[i, , drop = FALSE])
        if (is.null(g) || length(g) == 0) return(NA)
        if (is.na(g)) return(NA)
        isTRUE(g)
      }, logical(1))
      picked_idx <- which(passes & sample_mask & !is.na(grades_all))
      picks_baseline <- if (length(picked_idx) > 0) mean(grades_all[picked_idx]) else NA_real_
      picked_df <- feat_df[picked_idx, , drop = FALSE]
      
      incProgress(0.4, detail = "Grading combo...")
      res <- grade_combo(picked_df, combo, market$grade_fn,
                         track = "refine", baseline_hit_rate = picks_baseline)
      
      # Per-intuition kill breakdown
      kill_mat <- vapply(combo, function(cand) apply_candidate(cand, picked_df),
                         logical(nrow(picked_df)))
      if (!is.matrix(kill_mat)) kill_mat <- matrix(kill_mat, ncol = 1)
      colnames(kill_mat) <- vapply(combo, function(c) c$intuition$name, character(1))
      grades_picked <- vapply(seq_len(nrow(picked_df)), function(i) {
        g <- market$grade_fn(picked_df[i, , drop = FALSE])
        if (is.null(g) || length(g) == 0) return(NA)
        if (is.na(g)) return(NA)
        isTRUE(g)
      }, logical(1))
      kill_df <- data.frame(
        home = as.character(picked_df$home),
        away = as.character(picked_df$away),
        would_have_won = ifelse(is.na(grades_picked), "?",
                                ifelse(grades_picked, "WIN", "LOSS")),
        stringsAsFactors = FALSE
      )
      for (nm in colnames(kill_mat)) {
        kill_df[[paste0("killed_by_", nm)]] <- ifelse(is.na(kill_mat[, nm]), "?",
                                                      ifelse(kill_mat[, nm], "", "X"))
      }
      kill_df$kept <- apply(kill_mat, 1, function(r) all(r == TRUE, na.rm = FALSE) & !any(is.na(r)))
      rv$stage2_kill_breakdown <- kill_df
      
      refine_lb <- data.frame(
      ...
      rv$stage2_result <- list(
        refine = refine_lb,
        stage1_picks_baseline = picks_baseline,
        n_picked = length(picked_idx),
        n_candidates_expanded = length(combo),
        depths_swept = length(combo)
      )
      
      tune_row <- rv$last_search$leaderboard[
        rv$last_search$leaderboard$rule_id == as.integer(input$s2_rule_pick), ]
      tune_m <- list(picks = tune_row$picks, wins = tune_row$wins,
                     losses = tune_row$losses, hit_rate = tune_row$hit_rate)
      p <- new_pipeline(market_name = market$name, stage1_rule = rule,
                        stage2_combo = combo)
      p$tune_metrics <- tune_m
      rv$pipeline <- p
    })
    showNotification("Manual Stage 2 combo graded.", type = "message")
  })
  ...
  output$s2_kill_breakdown <- renderDT({
    df <- rv$stage2_kill_breakdown
    if (is.null(df) || nrow(df) == 0)
      return(datatable(data.frame(Message = "Run 'Run selected only' to populate."),
                       options = list(dom = "t"), rownames = FALSE))
    datatable(df, options = list(pageLength = 25, scrollX = TRUE),
              rownames = FALSE) |>
      formatStyle("kept",
                  backgroundColor = styleEqual(c(TRUE, FALSE), c("#dcfce7", "#fee2e2"))) |>
      formatStyle("would_have_won",
  ...
  #outputOptions(output, "val_pipeline_summary", suspendWhenHidden = FALSE)
  ...
  #outputOptions(output, "val_test_gate_ui", suspendWhenHidden = FALSE)
  ...
  #outputOptions(output, "val_metrics_table", suspendWhenHidden = FALSE, priority = 1)
  ...
  #outputOptions(output, "val_per_round_table", suspendWhenHidden = FALSE)
  ...
  #outputOptions(output, "val_stability_summary", suspendWhenHidden = FALSE)
  ...
  # ══════════════════════════════════════════════════════════
  # PUSH CASSETTE (replaces old Deploy)
  # ══════════════════════════════════════════════════════════
  
  # Cassettes folder — lives outside grid_searcher so main MOAB can source it
  # Auto-suggest cassette key when the market being tested changes.
  # Hits main MOAB's KPI slots (ht_draw, btts, over_15_ft) with exact keys;
  # everything else gets a sensible snake_case default. User can override.
  observe({
    mkt <- input$market_pick
    if (is.null(mkt) || mkt == "") return()
    m <- tolower(mkt)
    key_map <- list(
      # KPI-specific keys required by main MOAB
      "over 1.5"       = "over_15_ft",
      "over 1.5 ft"    = "over_15_ft",
      "over15"         = "over_15_ft",
      "btts"           = "btts",
      "both teams to score" = "btts",
      "ht draw"        = "ht_draw",
      "half time draw" = "ht_draw",
      # Everything else — snake_case default
      "over 2.5"       = "over_25",
      "1x2 home"       = "1x2_home",
      "1x2 away"       = "1x2_away",
      "1h draw"        = "1h_draw",
      ...
      "1up home"       = "1up_home",
      "1up away"       = "1up_away",
      "1h over 0.5"    = "1h_over_05",
      "2h over 0.5"    = "2h_over_05",
      "draw up to min 10" = "draw_up_to_10",
      "draw up to min 15" = "draw_up_to_15",
      "under 0.5 up to min 15" = "under_05_up_to_15"
    )
    suggested <- key_map[[m]]
    if (is.null(suggested)) {
      # Fallback: snake_case whatever the market name is
      suggested <- gsub("[^A-Za-z0-9]+", "_", m)
      suggested <- gsub("^_|_$", "", suggested)
    }
    updateTextInput(session, "push_key", value = suggested)
  })
  CASSETTES_DIR_PUSH <- file.path(dirname(APP_DIR), "cassettes")
  if (!dir.exists(CASSETTES_DIR_PUSH)) dir.create(CASSETTES_DIR_PUSH, recursive = TRUE)
  
  # Resolve the rule that will be pushed based on radio choice
  push_source_rule <- reactive({
    if (input$push_source == "pipeline") {
      p <- rv$pipeline
      if (is.null(p)) return(NULL)
      return(list(
        source = "pipeline",
        market_name = p$market_name,
        stage1_rule = p$stage1_rule,
        stage2_combo = p$stage2_combo,
        tune_metrics = p$tune_metrics,
        validate_metrics = p$validate_metrics,
    ...
    lb <- sort_leaderboard(r$leaderboard, sort_by = input$sort_by,
                           min_picks = input$min_picks, dedup = input$dedup_lb)
    rk <- as.integer(input$push_rank %||% 1)
    if (nrow(lb) < rk) return(NULL)
    rule_id <- lb$rule_id[rk]
    grid <- expand_grid_spec(r$grid_spec)
    rule <- row_to_rule(grid[rule_id, , drop = FALSE], r$clause_meta)
    metrics_row <- lb[rk, ]
    list(
      source = "rank",
      ...
      stage1_rule = rule,
      stage2_combo = NULL,
      tune_metrics = list(picks = metrics_row$picks,
                          wins = metrics_row$wins,
                          losses = metrics_row$losses,
  ...
  # Build the cassette file contents
  build_cassette_code <- function(src, key) {
    if (is.null(src) || is.null(src$stage1_rule)) return("# No rule available")
    if (is.null(key) || !nzchar(key)) return("# No market key given")
    # Header
    hdr <- c(
      paste0("# cassettes/", key, ".R"),
      paste0("# Auto-generated by Grid Searcher on ", format(Sys.time(), "%Y-%m-%d %H:%M:%S")),
      paste0("# Market: ", src$market_name),
      paste0("# Source: ", src$source,
             if (!is.null(src$rank)) paste0(" (rank ", src$rank, ")") else ""),
      if (!is.null(src$tune_metrics))
        paste0("# Tune metrics: picks=", src$tune_metrics$picks,
               " wins=", src$tune_metrics$wins,
               " losses=", src$tune_metrics$losses,
               " hit=", round((src$tune_metrics$hit_rate %||% NA) * 100, 2), "%")
      else "",
      if (!is.null(src$validate_metrics) && !is.null(src$validate_metrics$hit_rate))
        paste0("# Validate metrics: hit=", round(src$validate_metrics$hit_rate * 100, 2), "%")
      else "",
      if (!is.null(src$test_metrics) && !is.null(src$test_metrics$hit_rate))
        paste0("# Test metrics: hit=", round(src$test_metrics$hit_rate * 100, 2), "%")
      else "",
      "",
      "# Rule (Stage 1):",
      paste0("#   ", rule_label(src$stage1_rule)),
      if (!is.null(src$stage2_combo))
        paste0("# Stage 2 filter: ", paste(sapply(src$stage2_combo, function(x) {
          x$param_label %||% x$intuition$name %||% x$name %||% "?"
        }), collapse = " + "))
      else "# Stage 2 filter: (none)",
      ""
    )
    hdr <- hdr[nzchar(hdr) | hdr == ""]  # keep blank lines but drop character(0)
    
    # Body: emit stage1 dput + optional stage2 dput + cassette function
    body_lines <- c(
      paste0("cassette_", key, " <- function(enriched, thresholds = list(), standings = NULL) {"),
      "  # Loads the market's grader from the emit_full_block content.",
      "  # Delegates to the standard cassette runner below.",
      "  stage1_rule <- ",
      paste0("  ", deparse_one(src$stage1_rule)),
      if (!is.null(src$stage2_combo))
        c("  stage2_combo <- ",
          paste0("  ", deparse_one(src$stage2_combo)))
      else "  stage2_combo <- NULL",
      "",
      paste0("  run_cassette(enriched, stage1_rule, stage2_combo,"),
      paste0("               market_key = ", shQuote(key), ","),
      paste0("               market_display = ", shQuote(cassette_market_display(key)), ")"),
      "}",
      "",
      "# ── Runner (inlined so cassette is self-contained) ─────────",
      cassette_runner_code()
    )
    paste(c(hdr, body_lines), collapse = "\n")
  }
  
  # Helper: deparse an object onto one usable line (or multi-line block)
  deparse_one <- function(obj) {
    txt <- paste(deparse(obj, control = c("keepInteger", "keepNA", "niceNames")),
                 collapse = "\n  ")
    txt
  }
  
  # Human-readable market display string for the picks 'market' column
  cassette_market_display <- function(key) {
    m <- list(
      "over_15"           = "Over 1.5",
      "over_15_ft"        = "Over 1.5",
      "over_25"           = "Over 2.5",
      ...
      "1up_home"          = "1UP Home",
      "1up_away"          = "1UP Away",
      "1h_over_05"        = "1H Over 0.5",
      "2h_over_05"        = "2H Over 0.5",
      "draw_up_to_10"     = "Draw up to min 10",
      "draw_up_to_15"     = "Draw up to min 15",
      "under_05_up_to_15" = "Under 0.5 up to min 15",
      "ht_draw"           = "HT Draw"
    )
    m[[key]] %||% key
  }
  
  # The runner is inlined into each cassette so it's self-contained.
  cassette_runner_code <- function() {
    paste(c(
      "run_cassette <- function(enriched, stage1_rule, stage2_combo,",
      "                         market_key, market_display) {",
      "  if (length(enriched) == 0) return(NULL)",
      "  # Load feature builder + rule evaluator from grid_searcher engine",
      "  engine_dir <- file.path(BASE_PATH, 'grid_searcher', 'engine')",
      "  if (!dir.exists(engine_dir)) {",
      "    message('Cassette ', market_key, ': engine dir not found at ', engine_dir)",
      "    return(NULL)",
      "  }",
      "  # Source into GLOBAL env so functions are visible when apply_rule calls",
      "  # apply_clause etc. (source with local=TRUE breaks that chain).",
      "  sys.source(file.path(engine_dir, 'features.R'), envir = globalenv())",
      "  sys.source(file.path(engine_dir, 'grid_search.R'), envir = globalenv())",
      "  # Extract features referenced by the rule",
      "  feats <- unique(unlist(lapply(stage1_rule, function(cl) c(cl$feature, cl$feature2))))",
      "  intu_feat_map <- list(",
      "    no_stalemate_teams   = c('h_draws_n5', 'a_draws_n5'),",
      "    no_loser_teams       = c('h_losses_n5', 'a_losses_n5'),",
      "    no_consec_draws      = c('h_max_consec_draws_n5', 'a_max_consec_draws_n5', 'h2h_max_consec_draws'),",
      ...
      "    last_had_2h_over15   = c('h_last_2h_over15_n5', 'a_last_2h_over15_n5', 'h2h_last_2h_over15')",
      "  )",
      "  if (!is.null(stage2_combo) && length(stage2_combo) > 0) {",
      "    for (cand in stage2_combo) {",
      "      intu <- cand$intuition %||% cand",
      "      nm <- intu$name",
      "      if (!is.null(nm) && !is.null(intu_feat_map[[nm]])) {",
      "        feats <- c(feats, intu_feat_map[[nm]])",
      "      }",
      "    }",
      "  }",
      "  feats <- unique(feats[!is.na(feats) & feats != ''])",
      "  if (length(feats) == 0) return(NULL)",
      "  # Filter to upcoming fixtures (no FT result yet)",
      "  fx_upcoming <- enriched",
      "  if (length(fx_upcoming) == 0) return(NULL)",
      "  feat_df <- build_feature_matrix(fx_upcoming, feats,",
      "                                    default_window = 5, main_only = FALSE)",
      "  picked <- apply_rule(feat_df, stage1_rule)",
      "  # Stage 2 filter — combo is a list of intuition entries with filter_fn.",
      "  # Each intuition's filter_fn returns logical vector; all must pass.",
      "  if (!is.null(stage2_combo) && length(stage2_combo) > 0) {",
      "    for (cand in stage2_combo) {",
      "      intu <- cand$intuition %||% cand",
      "      params <- cand$params %||% list()",
      "      if (is.function(intu$filter_fn)) {",
      "        keep <- vapply(seq_len(nrow(feat_df)), function(i) {",
      "          v <- tryCatch(intu$filter_fn(feat_df[i, , drop = FALSE], params),",
      "                        error = function(e) NA)",
      ...
      "    fx <- fx_upcoming[[i]]$fixture",
      "    data.frame(",
      "      match_id     = as.character(fx$match_id %||% NA),",
      "      country      = fx$country %||% NA,",
      "      league       = fx$league %||% NA,",
      "      fixture_date = as.character(fx$fixture_date %||% NA),",
      "      match_time   = fx$match_time %||% NA,",
      "      home         = fx$home %||% NA,",
      "      away         = fx$away %||% NA,",
      "      pick         = market_display,",
  ...
  # Preview
  observeEvent(input$push_preview_btn, {
    src <- push_source_rule()
    if (is.null(src)) {
      showNotification("No source rule. Run Grid Search / seed pipeline first.",
                       type = "error"); return()
    }
    key <- trimws(input$push_key %||% "")
    if (!nzchar(key)) {
      showNotification("Enter a market key (e.g. over_15)", type = "error"); return()
    }
    if (grepl("[^a-z0-9_]", key)) {
      showNotification("Key must be lowercase letters, digits, underscores only",
                       type = "error"); return()
    }
    rv$push_preview <- build_cassette_code(src, key)
    rv$push_status <- paste0("Preview ready for cassettes/", key, ".R")
  })
  
  output$push_preview_out <- renderText({
    rv$push_preview %||% "Click 'Preview cassette' after selecting a source and entering a market key."
  })
  
  # Apply — write cassette file, with backup of any existing
  observeEvent(input$push_apply_btn, {
    if (is.null(rv$push_preview) || !nzchar(rv$push_preview)) {
      showNotification("Preview a cassette first", type = "error"); return()
    }
    key <- trimws(input$push_key %||% "")
    if (!nzchar(key)) {
      showNotification("Enter a market key", type = "error"); return()
    }
    target <- file.path(CASSETTES_DIR_PUSH, paste0(key, ".R"))
    if (file.exists(target)) {
      backup <- file.path(CASSETTES_DIR_PUSH,
                          paste0(key, ".backup_",
                                 format(Sys.time(), "%Y%m%d_%H%M%S"), ".R.bak"))
      file.copy(target, backup, overwrite = FALSE)
    }
    writeLines(rv$push_preview, target)
    rv$push_status <- paste0("Wrote ", target,
                             if (file.exists(target)) " (existing backed up)" else "")
    showNotification(rv$push_status, type = "message", duration = 8)
  })
  
  output$push_status <- renderText({ rv$push_status %||% "" })
  
  # ══════════════════════════════════════════════════════════
  # FEATURE RANKER TAB (solo + pair, fixed binning, Tune+Validate)
  # ══════════════════════════════════════════════════════════
  
  rv_ranker <- reactiveValues(results = NULL, status = "")
  
  # Smart binning: unique values for discrete (<=10 unique), quantiles for continuous
  smart_bins <- function(x) {
    x_clean <- x[!is.na(x)]
    if (length(x_clean) < 5) return(rep(NA_integer_, length(x)))
    uniq <- unique(x_clean)
    if (length(uniq) <= 10) {
      # Discrete: each unique value gets its own bin
      as.integer(factor(x, levels = sort(uniq)))
    } else {
      # Continuous: quantile bins
      brks <- unique(quantile(x_clean, probs = seq(0, 1, 0.2)))
      if (length(brks) < 3) return(rep(1L, length(x)))
      as.integer(cut(x, breaks = brks, include.lowest = TRUE, labels = FALSE))
    }
  }
  
  # MI between one feature and binary outcome
  mutual_info_1 <- function(x, y) {
    b <- smart_bins(x)
    valid <- !is.na(b) & !is.na(y)
    if (sum(valid) < 20) return(0)
    tab <- table(b[valid], y[valid])
  ...
  # Joint MI between two features and binary outcome
  mutual_info_2 <- function(x1, x2, y) {
    b1 <- smart_bins(x1); b2 <- smart_bins(x2)
    valid <- !is.na(b1) & !is.na(b2) & !is.na(y)
    if (sum(valid) < 20) return(0)
    joint_key <- paste0(b1[valid], "_", b2[valid])
    tab <- table(joint_key, y[valid])
    if (nrow(tab) < 2 || ncol(tab) < 2) return(0)
    p_xy <- tab / sum(tab)
    p_x  <- rowSums(p_xy); p_y <- colSums(p_xy)
    mi <- 0
  ...
  # Compute solo + pair scores for a given feature matrix
  compute_ranking <- function(feat_only, y) {
    active_feats <- names(feat_only)
    solo_mi <- vapply(active_feats,
                      function(f) mutual_info_1(feat_only[[f]], y),
                      numeric(1))
    names(solo_mi) <- active_feats
    pairs <- utils::combn(active_feats, 2, simplify = FALSE)
    pair_scores <- vapply(seq_along(pairs), function(i) {
      p <- pairs[[i]]
      mutual_info_2(feat_only[[p[1]]], feat_only[[p[2]]], y)
    }, numeric(1))
    list(solo = solo_mi, pairs = pairs, pair_scores = pair_scores)
  }
  
  observeEvent(input$ranker_run_btn, {
    market <- rv$markets[[input$market_pick]]
    if (is.null(market)) {
      showNotification("Pick a market on the Grid Search tab first.",
                       type = "error"); return()
    }
    pool <- build_fixtures_pool()
    if (is.null(pool) || length(pool$tune) == 0) {
      showNotification("Load fixtures first.", type = "error"); return()
    }
    cats <- input$ranker_categories %||% character(0)
    if (length(cats) == 0) {
      showNotification("Pick at least one category.", type = "error"); return()
    }
    all_feats <- feature_catalog_df()
    keep <- all_feats$feature[all_feats$category %in% cats]
    # Filter 1: drop n3 and n7 windowed features — only n5 gets ranked
    keep <- keep[!grepl("_n(3|7)$", keep)]
    if (length(keep) < 2) {
      showNotification("Need at least 2 features after n3/n7 filter.", type = "error"); return()
    }
    
    withProgress(message = "Building Tune feature matrix...", value = 0.05, {
      feat_df <- build_feature_matrix(pool$tune, keep,
                                      default_window = input$default_window,
                                      main_only = effective_main_only())
      grades <- vapply(seq_len(nrow(feat_df)),
                       function(i) {
      ...
      valid <- !is.na(grades)
      y <- as.integer(grades[valid])
      feat_only <- feat_df[valid, keep, drop = FALSE]
      na_cols <- vapply(feat_only, function(x) all(is.na(x)), logical(1))
      feat_only <- feat_only[, !na_cols, drop = FALSE]
      
      incProgress(0.1, detail = "Computing correlations for pair filter...")
      # Filter 2 + 3: build allowed-pair set (drop same root OR |r| >= 0.85)
      feat_root_fn <- function(name) sub("_n[0-9]+$", "", name)
      cor_mat <- suppressWarnings(cor(feat_only, use = "pairwise.complete.obs"))
      fnames <- names(feat_only)
      allowed_pairs <- list()
      for (i in seq_len(length(fnames)-1)) {
        for (j in (i+1):length(fnames)) {
          a <- fnames[i]; b <- fnames[j]
          r <- cor_mat[a, b]
          if (feat_root_fn(a) == feat_root_fn(b)) next
          if (!is.na(r) && abs(r) >= 0.85) next
          allowed_pairs[[length(allowed_pairs) + 1]] <- c(a, b)
        }
      }
      incProgress(0.2, detail = paste0("Tune: ", ncol(feat_only), " features, ",
                                       length(allowed_pairs), " allowed pairs..."))
      # Custom ranking that only scores allowed pairs
      compute_ranking_filtered <- function(feat_only, y, pairs) {
        active_feats <- names(feat_only)
        solo_mi <- vapply(active_feats,
                          function(f) mutual_info_1(feat_only[[f]], y),
                          numeric(1))
      ...
      tune_res <- compute_ranking_filtered(feat_only, y, allowed_pairs)
      
      # Optionally rank on Validate too
      val_res <- NULL
      if (isTRUE(input$ranker_use_validate) &&
          !is.null(pool$validate) && length(pool$validate) > 0) {
        incProgress(0.15, detail = "Building Validate matrix...")
        val_feat_df <- build_feature_matrix(pool$validate, keep,
                                            default_window = input$default_window,
                                            main_only = effective_main_only())
        val_grades <- vapply(seq_len(nrow(val_feat_df)),
                             function(i) {
                               g <- market$grade_fn(val_feat_df[i, ])
                               if (is.na(g)) NA_real_ else as.numeric(g)
                             }, numeric(1))
        v_valid <- !is.na(val_grades)
        val_y <- as.integer(val_grades[v_valid])
        val_feat_only <- val_feat_df[v_valid, names(feat_only), drop = FALSE]
        incProgress(0.15, detail = "Validate: ranking...")
        val_res <- compute_ranking_filtered(val_feat_only, val_y, allowed_pairs)
      }
      
      # Build unified table
      incProgress(0.15, detail = "Building unified table...")
      # Solos
      solo_rows <- data.frame(
        type = "solo",
        feature_a = names(tune_res$solo),
        feature_b = NA_character_,
        joint_tune = round(unname(tune_res$solo), 4),
        solo_a_tune = round(unname(tune_res$solo), 4),
        solo_b_tune = NA_real_,
        lift_tune = NA_real_,
        classification = "solo",
        stringsAsFactors = FALSE
      )
      if (!is.null(val_res)) {
        solo_rows$joint_validate  <- round(unname(val_res$solo[names(tune_res$solo)]), 4)
        solo_rows$solo_a_validate <- solo_rows$joint_validate
        solo_rows$solo_b_validate <- NA_real_
      }
      # Pairs
      thresh_pct <- input$ranker_interaction_threshold %||% 0.15
      pair_rows <- do.call(rbind, lapply(seq_along(tune_res$pairs), function(i) {
        p <- tune_res$pairs[[i]]
        js <- tune_res$pair_scores[i]
        sa <- tune_res$solo[p[1]]; sb <- tune_res$solo[p[2]]
        max_solo <- max(sa, sb, na.rm = TRUE)
        lift_pct <- if (max_solo > 0) (js - max_solo) / max_solo else NA_real_
        cls <- if (!is.na(lift_pct) && lift_pct >= thresh_pct) "interaction"
        else if (js <= max_solo) "redundant"
        else "weak_pair"
        row <- data.frame(
          type = "pair",
          feature_a = p[1],
          feature_b = p[2],
          joint_tune = round(js, 4),
          solo_a_tune = round(sa, 4),
          solo_b_tune = round(sb, 4),
          lift_tune = round(lift_pct, 3),
          classification = cls,
          stringsAsFactors = FALSE)
        if (!is.null(val_res)) {
          js_v <- val_res$pair_scores[i]
          sa_v <- val_res$solo[p[1]]; sb_v <- val_res$solo[p[2]]
          row$joint_validate <- round(js_v, 4)
          row$solo_a_validate <- round(sa_v, 4)
          row$solo_b_validate <- round(sb_v, 4)
        }
        row
      }))
      
      all_entries <- rbind(solo_rows, pair_rows)
      all_entries <- all_entries[order(-all_entries$joint_tune), ]
      all_entries$rank <- seq_len(nrow(all_entries))
      
      rv_ranker$results <- all_entries
      rv_ranker$status <- paste0("Ranked ", length(tune_res$solo),
                                 " solos + ", length(tune_res$pairs),
                                 " pairs on ", sum(valid),
                                 " gradable fixtures (Tune)",
                                 if (!is.null(val_res))
                                   paste0(" + ", sum(v_valid), " (Validate)")
                                 else "")
      showNotification(rv_ranker$status, type = "message")
    })
  })
  
  output$ranker_summary <- renderText({
    if (is.null(rv_ranker$results)) return("Click 'Rank features + pairs' to run.")
    df <- rv_ranker$results
    n_interact <- sum(df$classification == "interaction", na.rm = TRUE)
    n_redun    <- sum(df$classification == "redundant", na.rm = TRUE)
    n_solo     <- sum(df$type == "solo")
    paste0("Entries: ", nrow(df), " (", n_solo, " solos, ",
           sum(df$type == "pair"), " pairs)\n",
           "Interactions: ", n_interact, " | Redundants: ", n_redun, "\n",
           "Top N send: solos = 1 slot, interactions = 2 slots (both features kept),\n",
           "redundants skipped in favor of genuinely different signals.")
  })
  
  output$ranker_table <- renderDT({
    df <- rv_ranker$results
    if (is.null(df)) return(NULL)
    show_cols <- c("rank", "type", "classification",
                   "feature_a", "feature_b", "joint_tune",
                   "solo_a_tune", "solo_b_tune", "lift_tune")
    val_cols <- intersect(c("joint_validate", "solo_a_validate", "solo_b_validate"),
                          names(df))
    disp <- df[, c(show_cols, val_cols), drop = FALSE]
    datatable(disp, rownames = FALSE,
              options = list(pageLength = 25, scrollX = TRUE)) |>
      formatStyle("classification",
                  backgroundColor = styleEqual(
                    c("solo", "interaction", "redundant", "weak_pair"),
                    c("#e0f2fe", "#dcfce7", "#fee2e2", "#f5f5f5")))
  })
  
  # Dedup-aware send (unchanged logic from previous version)
  observeEvent(input$ranker_send_top_btn, {
    df <- rv_ranker$results
    if (is.null(df) || nrow(df) == 0) {
      showNotification("Run ranker first.", type = "error"); return()
    }
    n_cap <- as.integer(input$ranker_top_n %||% 9)
    sent <- character(0)
    for (i in seq_len(nrow(df))) {
      if (length(sent) >= n_cap) break
      row <- df[i, ]
      if (row$classification == "redundant") next
      if (row$type == "solo") {
        if (!(row$feature_a %in% sent) && (length(sent) + 1) <= n_cap) {
          sent <- c(sent, row$feature_a)
        }
        next
      }
      to_add <- setdiff(c(row$feature_a, row$feature_b), sent)
      if (length(to_add) == 0) next
      if (row$classification == "weak_pair" && length(to_add) == 1) {
        sm <- df$solo_a_tune[df$type == "solo"]
        med_solo <- median(sm, na.rm = TRUE)
        new_feat <- to_add
        new_solo <- if (new_feat == row$feature_a) row$solo_a_tune else row$solo_b_tune
        if (is.na(new_solo) || new_solo < med_solo) next
      }
      if ((length(sent) + length(to_add)) <= n_cap) {
        sent <- c(sent, to_add)
      }
    }
    if (length(sent) == 0) {
      showNotification("No features to send after dedup.", type = "warning"); return()
    }
    updateSelectizeInput(session, "ablation_features",
                         choices = names(FEATURE_CATALOG),
                         selected = sent, server = TRUE)
    showNotification(paste0("Sent ", length(sent), " features to Ablation."),
                     type = "message")
    updateTabsetPanel(session, "main_tabs", selected = "Ablation")
  })
  
  output$ranker_status <- renderText({ rv_ranker$status })
  
  # ══════════════════════════════════════════════════════════
  # ABLATION TAB
  # ══════════════════════════════════════════════════════════
  
  # Auto-suggest thresholds based on feature name pattern
  auto_threshold_grid <- function(feature_name) {
    fn <- tolower(feature_name)
    # H2H mean-shaped (checked first so it doesn't fall into avg_ branch)
    if (grepl("^h2h_avg", fn)) return(c(1.5, 2.0, 2.5, 3.0))
    # H2H rate-shaped
    if (grepl("^h2h_.*_rate$", fn) || grepl("^h2h_blank_rate$", fn)
        || grepl("^h2h_home_clean_rate$", fn)) return(c(0.3, 0.4, 0.5, 0.6))
    # Probability features (Poisson p_*)
    if (grepl("^p_", fn)) return(c(0.4, 0.5, 0.6, 0.7))
    # Lambda features
    if (grepl("lambda", fn)) return(c(2.0, 2.2, 2.5, 2.8, 3.0))
    # Rate features (all *_rate_*)
    if (grepl("_rate", fn)) return(c(0.3, 0.4, 0.5, 0.6, 0.7))
    # Mean features (avg_*)
    if (grepl("avg_", fn)) return(c(1.0, 1.5, 2.0, 2.5))
    # Variance features
    if (grepl("variance", fn)) return(c(0.5, 1.0, 1.5))
    # Count features (h_*, a_*, h2h_* — everything else, integers)
    c(2, 3, 4, 5)
  }
  
  # Populate feature dropdown on startup — every catalog feature
  observe({
    updateSelectizeInput(session, "ablation_features",
                         choices = names(FEATURE_CATALOG),
                         server = TRUE)
  ...
  # Auto-cap depth choices at number of selected features
  observe({
    n_feats <- length(input$ablation_features %||% character(0))
    if (n_feats == 0) {
      updateCheckboxGroupInput(session, "ablation_depths",
                               choices = c("Pick features first" = "none"),
                               selected = character(0))
      return()
    }
    max_d <- min(n_feats, as.integer(input$ablation_max_depth %||% 8))
    labels <- paste0("Depth ", 1:max_d,
                     ifelse(1:max_d == 1, " (solo)",
                            ifelse(1:max_d == 2, " (pairs)",
                                   ifelse(1:max_d == 3, " (triples)",
                                          ifelse(1:max_d == 4, " (quads)",
                                                 paste0(" (", 1:max_d, "-way)"))))))
    updateCheckboxGroupInput(session, "ablation_depths",
                             choices = setNames(as.character(1:max_d), labels),
                             selected = as.character(1:max_d))
  })
  
  # Compute upfront size estimate so user sees the cost before pressing Run
  ablation_estimate <- reactive({
    feats <- input$ablation_features %||% character(0)
    depths <- as.integer(input$ablation_depths %||% character(0))
    n <- length(feats)
    if (n == 0 || length(depths) == 0) {
      return(list(total_rules = 0, text = "Pick features + depths."))
    }
    thresh_per_feat <- vapply(feats, function(f) length(auto_threshold_grid(f)),
                              integer(1))
    total_rules <- 0
    lines <- character(0)
    for (d in depths) {
      if (d > n) next
      combos <- choose(n, d)
      # Threshold multiplier — use median per-feature count as proxy
      per_combo_rules <- median(thresh_per_feat)^d
      dr <- combos * per_combo_rules
      total_rules <- total_rules + dr
      lines <- c(lines,
                 paste0("  Depth ", d, ": ", combos, " combos \u00d7 ",
                        round(per_combo_rules), " threshold sets \u2248 ",
                        formatC(dr, big.mark = ",", format = "d"),
                        " rules"))
    }
    txt <- paste(c(paste0("Total rules to test: ",
                          formatC(total_rules, big.mark = ",", format = "d")),
                   lines), collapse = "\n")
    list(total_rules = total_rules, text = txt)
  })
  
  output$ablation_size_estimate <- renderText({ ablation_estimate()$text })
  
  # Run the ablation sweep
  rv_ablation <- reactiveValues(results = NULL, run_meta = NULL)
  
  observeEvent(input$ablation_run_btn, {
    feats <- input$ablation_features %||% character(0)
    if (length(feats) < 1) {
      showNotification("Pick at least one feature.", type = "error"); return()
    }
    depths <- as.integer(input$ablation_depths %||% character(0))
    if (length(depths) == 0) {
      showNotification("Pick at least one depth.", type = "error"); return()
    }
    market <- rv$markets[[input$market_pick]]
    if (is.null(market)) {
      showNotification("Pick a market on the Grid Search tab first.",
    ...
    # Build feature matrix ONCE with every candidate feature
    withProgress(message = "Building feature matrix...", value = 0.05, {
      feat_df <- build_feature_matrix(pool$tune, feats,
                                      default_window = input$default_window,
                                      main_only = effective_main_only())
      incProgress(0.1, detail = "Ready. Sweeping combos...")
      
      # For each combo at each depth, build a grid_spec, run grid_search, take
      # the top rule (by user's sort choice), collect into results table
      all_rows <- list()
      total_combos <- sum(vapply(depths,
                                 function(d) if (d > length(feats)) 0L else as.integer(choose(length(feats), d)),
                                 integer(1)))
      done_combos <- 0
      
      for (d in depths) {
        if (d > length(feats)) next
        combos <- utils::combn(feats, d, simplify = FALSE)
        for (combo in combos) {
          done_combos <- done_combos + 1
          # Build a per-combo grid_spec
          spec <- lapply(combo, function(f) {
            list(family = "ge", feature = f,
                 grid = auto_threshold_grid(f))
          })
          res <- tryCatch(
            run_grid_search(feat_df, grade_fn = market$grade_fn,
                            grid_spec = spec,
                            min_picks = input$ablation_min_picks),
            error = function(e) NULL)
          if (is.null(res) || nrow(res$leaderboard) == 0) {
            incProgress(0.85 / total_combos,
                        detail = paste0(done_combos, "/", total_combos))
            next
          }
          lb <- sort_leaderboard(res$leaderboard,
                                 sort_by = input$ablation_sort_by,
                                 min_picks = input$ablation_min_picks,
                                 dedup = TRUE)
          if (nrow(lb) == 0) {
            incProgress(0.85 / total_combos,
          ...
          top_n <- min(input$ablation_top_per_combo, nrow(lb))
          for (i in seq_len(top_n)) {
            rid <- lb$rule_id[i]
            rule <- row_to_rule(expand_grid_spec(spec)[rid, , drop = FALSE],
                                res$clause_meta)
            all_rows[[length(all_rows) + 1]] <- data.frame(
              depth = d,
              combo = paste(combo, collapse = " + "),
              rule_label = lb$rule_label[i],
              picks = lb$picks[i],
              wins = lb$wins[i],
              losses = lb$losses[i],
              hit_rate = round(lb$hit_rate[i], 4),
              spec_code = I(list(spec)),
              stringsAsFactors = FALSE
            )
          }
          incProgress(0.85 / total_combos,
      ...
      # Per-round breakdown: if feat_df carries source_file (multi-file mode),
      # compute per-round picks/hit-rate for each winner.
      if (length(all_rows) > 0 && "source_file" %in% names(feat_df)) {
        rounds <- unique(feat_df$source_file)
        if (length(rounds) > 1) {
          incProgress(0.05, detail = "Per-round breakdown...")
          grades_all <- vapply(seq_len(nrow(feat_df)),
                               function(i) {
                                 g <- market$grade_fn(feat_df[i, ])
                                 if (is.na(g)) NA_real_ else as.numeric(g)
                               }, numeric(1))
          for (rn in rounds) {
            ...
            for (i in seq_along(all_rows)) {
              spec <- all_rows[[i]]$spec_code[[1]]
              rule <- lapply(spec, function(cl) {
                th <- as.numeric(
                  gsub(paste0(cl$feature, " >= "), "",
                       regmatches(all_rows[[i]]$rule_label,
                                  regexpr(paste0(cl$feature, " >= [0-9.]+"),
                                          all_rows[[i]]$rule_label))))
                list(family = cl$family, feature = cl$feature,
                     feature2 = cl$feature2 %||% NA_character_, threshold = th)
              })
              passes <- apply_rule(feat_df, rule)
              hits_r <- passes & mask_r & !is.na(grades_all)
              n_picks <- sum(hits_r)
              n_wins  <- sum(grades_all[hits_r] == 1, na.rm = TRUE)
              hit_r <- if (n_picks > 0) round(n_wins / n_picks, 4) else NA_real_
              rn_clean <- gsub("[^A-Za-z0-9]+", "_", rn)
              rn_clean <- gsub("^_|_$", "", rn_clean)
              all_rows[[i]][[paste0("picks_", rn_clean)]] <- n_picks
              all_rows[[i]][[paste0("hit_",   rn_clean)]] <- hit_r
            }
          }
        }
      }
      ...
      if (length(all_rows) > 0 && isTRUE(input$ablation_use_validate) &&
          !is.null(pool$validate) && length(pool$validate) > 0) {
        incProgress(0.15, detail = "Validate-set evaluation...")
        val_feat_df <- build_feature_matrix(pool$validate, feats,
                                            default_window = input$default_window,
                                            main_only = effective_main_only())
        val_grades <- vapply(seq_len(nrow(val_feat_df)),
                             function(i) {
        ...
        for (i in seq_along(all_rows)) {
          ...
          passes <- apply_rule(val_feat_df, rule)
          hits <- passes & !is.na(val_grades)
          n_p <- sum(hits)
          n_w <- sum(val_grades[hits] == 1, na.rm = TRUE)
          all_rows[[i]]$val_picks    <- n_p
          all_rows[[i]]$val_wins     <- n_w
          all_rows[[i]]$val_losses   <- n_p - n_w
          all_rows[[i]]$val_hit_rate <- if (n_p > 0) round(n_w / n_p, 4) else NA_real_
        }
      }
      
      if (length(all_rows) == 0) {
        rv_ablation$results <- data.frame()
        showNotification("No combos produced rules above min picks.",
                         type = "warning")
      } else {
        df <- do.call(rbind, all_rows)
        # Sort winners by chosen metric
        # Multi-key sort: primary = user choice, secondary = the other key
        if (input$ablation_sort_by == "hit_rate") {
          df <- df[order(-df$hit_rate, -df$picks, df$depth), , drop = FALSE]
        } else if (input$ablation_sort_by == "losses") {
          df <- df[order(df$losses, -df$picks, df$depth), , drop = FALSE]
        } else {
          df <- df[order(-df$picks, -df$hit_rate, df$depth), , drop = FALSE]
        }
        rv_ablation$results <- df
        rv_ablation$run_meta <- list(
          market = market$name,
          feats = feats,
          depths = depths,
          when = format(Sys.time(), "%Y-%m-%d %H:%M:%S")
        )
        showNotification(paste0("Ablation done. ", nrow(df), " rows."),
                         type = "message")
      }
    })
  })
  
  output$ablation_summary <- renderText({
    m <- rv_ablation$run_meta
    if (is.null(m)) return("Run ablation to see results.")
    df <- rv_ablation$results
    if (is.null(df) || nrow(df) == 0) return("No results.")
    paste0("Market: ", m$market, "\n",
           "Features tested: ", length(m$feats), "\n",
           "Depths: ", paste(m$depths, collapse = ", "), "\n",
           "Winners: ", nrow(df), "\n",
           "Run at: ", m$when)
  })
  
  output$ablation_results_table <- renderDT({
    df <- rv_ablation$results
    if (is.null(df) || nrow(df) == 0) return(NULL)
    # Show per-round columns if they exist
    base_cols <- c("depth", "combo", "rule_label",
                   "picks", "wins", "losses", "hit_rate")
    round_cols <- grep("^(picks|hit)_", names(df), value = TRUE)
    val_cols <- grep("^val_", names(df), value = TRUE)
    show_cols <- c(base_cols, round_cols, val_cols)
    disp <- df[, intersect(show_cols, names(df)), drop = FALSE]
    datatable(disp, rownames = FALSE,
              options = list(pageLength = 20, scrollX = TRUE),
              selection = "single")
  })
  
  # When user clicks a row, emit its grid_spec code
  output$ablation_spec_out <- renderText({
    sel <- input$ablation_results_table_rows_selected
    df <- rv_ablation$results
    if (is.null(df) || nrow(df) == 0)
      return("Run ablation, then select a row above.")
    if (length(sel) == 0)
      return("Select a row in the table to see its grid_spec code.")
    spec <- df$spec_code[[sel]]
    # Emit copy-ready code
    lines <- c("  grid_spec = list(")
    for (i in seq_along(spec)) {
      cl <- spec[[i]]
      grid_str <- paste0("c(", paste(cl$grid, collapse = ", "), ")")
      lines <- c(lines,
                 paste0('    list(family = "ge", feature = "', cl$feature,
                        '", grid = ', grid_str, ')',
                        if (i < length(spec)) "," else ""))
    }
    lines <- c(lines, "  )")
    paste(lines, collapse = "\n")
  })
