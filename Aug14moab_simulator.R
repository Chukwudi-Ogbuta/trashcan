                           ...
                           div(style = "padding-top:8px;",
                               ...
                               div(class = "cd",
                                   ...
                                   div(style = "margin-bottom:12px;",
                                       checkboxInput("sb_filter", 
                                                     label = "Filter to SportyBet leagues only (live API)",
                                                     value = TRUE)),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$run_btn, {
    ...
    # Filter to SportyBet leagues if checkbox enabled
    enriched_for_run <- rv$enriched
    if (isTRUE(input$sb_filter) && exists("filter_enriched_to_sportybet")) {
      log_msg("Fetching live SportyBet directory...")
      filt <- tryCatch(filter_enriched_to_sportybet(rv$enriched),
                       error = function(e) {
                         log_msg(paste0("SB filter failed: ", e$message))
                         list(enriched = rv$enriched, kept = length(rv$enriched), dropped = 0L)
                       })
      enriched_for_run <- filt$enriched
      log_msg(paste0("SB filter: kept ", filt$kept, ", dropped ", filt$dropped,
                     " (", length(enriched_for_run), " fixtures will be analyzed)"))
      if (length(enriched_for_run) == 0) {
        showNotification("No fixtures left after SportyBet filter", type = "warning")
        return()
      }
    }
    ...
    withProgress(message = "Simulating", value = 0.3, {
      tryCatch({
        ...
        rv$flat_data <- build_flat_data(rv$graded %||% preds, enriched_for_run)
