...
server <- function(input, output, session) {
  ...
  # ═══ WINAMAX MAPPING TAB ═══════════════════════════════════
  WMX_CACHE_PATH <- file.path(BASE_PATH, "winamax_directory_cache.rds")
  isolate({
    rv$wmx_dir <- if (file.exists(WMX_CACHE_PATH))
      tryCatch(readRDS(WMX_CACHE_PATH),
               error = function(e) NULL) else NULL
    rv$wmx_last_action <- if (!is.null(rv$wmx_dir))
      paste("Loaded cached directory (",
            nrow(rv$wmx_dir), "leagues)")
    else "Not refreshed yet"
  })
  
  output$wmx_last_action <- renderText(rv$wmx_last_action)
  
  observeEvent(input$wmx_refresh_btn, {
    req(rv$mapper)
    if (!exists("fetch_winamax_directory")) {
      showNotification("winamax_helpers.R not loaded.", type = "error"); return()
    }
    withProgress(message = "Fetching Winamax directory via Selenium...", value = 0.3, {
      wdir <- fetch_winamax_directory()
      if (is.null(wdir) || nrow(wdir) == 0) {
        showNotification("Winamax scrape returned nothing.", type = "error"); return()
      }
      rv$wmx_dir <- wdir
      saveRDS(wdir, WMX_CACHE_PATH)
      incProgress(0.5, detail = "Matching Verified SB rows")
      diag <- match_verified_to_winamax(rv$mapper, wdir, fuzzy_threshold = 0.75)
      rv$wmx_diag <- diag
      
      m <- rv$mapper
      for (col in c("winamax_country_id", "winamax_league_id",
                    "winamax_country_name", "winamax_league_name",
                    "winamax_url", "winamax_match_reason", "winamax_reviewed_on")) {
        if (!col %in% names(m)) {
          m[[col]] <- if (col %in% c("winamax_country_id", "winamax_league_id"))
            NA_integer_ else NA_character_
        }
      }
      for (i in seq_len(nrow(diag))) {
        ...
        idx <- which(m$sb_country == d$sb_country & m$sb_league == d$sb_league)
        if (length(idx) == 0) next
        if (!is.na(m$winamax_reviewed_on[idx])) next
        m$winamax_country_id[idx]   <- d$winamax_country_id
        m$winamax_league_id[idx]    <- d$winamax_league_id
        m$winamax_country_name[idx] <- d$winamax_country_name
        m$winamax_league_name[idx]  <- d$winamax_league_name
        m$winamax_url[idx]          <- d$winamax_url
        m$winamax_match_reason[idx] <- d$reason
      }
      rv$mapper <- m
      saveRDS(m, MAP_PATH)
      rv$wmx_last_action <- paste("Refreshed at",
                                  format(Sys.time(), "%H:%M:%S"))
      showNotification(paste0(
        "Winamax directory: ", nrow(wdir), " leagues \u00b7 ",
        "SB \u2192 Winamax matches: ", sum(!is.na(diag$winamax_league_id)),
        " / ", nrow(diag)),
        type = "message", duration = 5)
    })
  })
  
  output$wmx_review_ui <- renderUI({
    req(rv$mapper)
    m <- rv$mapper
    verified <- m[!is.na(m$status) & m$status == "Verified" &
                    !is.na(m$fs_country) & !is.na(m$fs_league), , drop = FALSE]
    if (!"winamax_reviewed_on" %in% names(verified))
      verified$winamax_reviewed_on <- NA_character_
    filt <- input$wmx_filter %||% "needs"
    verified <- switch(filt,
                       "needs"    = verified[is.na(verified$winamax_reviewed_on), , drop = FALSE],
                       "reviewed" = verified[!is.na(verified$winamax_reviewed_on), , drop = FALSE],
                       verified)
    if (nrow(verified) == 0) {
      return(div(style = "text-align:center; padding:32px; color:#888;",
                 if (filt == "needs") "All Verified SB rows reviewed."
    ...
    wdir <- rv$wmx_dir
    wmx_countries <- if (!is.null(wdir)) sort(unique(wdir$country_name))
    else character()
    ...
    action_html <- vapply(seq_len(nrow(verified)), function(i) {
      rid <- row_ids[i]
      default <- if (!is.na(verified$winamax_league_id[i])) "Approve"
      else "Not Available"
      paste0(
        '<select data-row="', rid, '" ',
        'onchange="Shiny.setInputValue(\'wmx_action_', rid, '\', this.value, {priority:\'event\'});',
        'var ct=document.getElementById(\'wmx_ct_', rid, '\');',
        'var lg=document.getElementById(\'wmx_lg_', rid, '\');',
        'if(ct){ct.disabled=(this.value!==\'Specify Winamax\');}',
        'if(lg){lg.disabled=(this.value!==\'Specify Winamax\');}">',
        '<option value="Approve"', if (default == "Approve") ' selected' else '', '>Approve</option>',
        '<option value="Not Available"', if (default == "Not Available") ' selected' else '', '>Not Available on Winamax</option>',
        '<option value="Specify Winamax">Specify Winamax</option>',
        '</select>')
    }, character(1))
    
    country_html <- vapply(seq_len(nrow(verified)), function(i) {
      rid <- row_ids[i]
      default_ct <- verified$winamax_country_name[i] %||%
        (if (verified$fs_country[i] %in% wmx_countries)
          verified$fs_country[i] else "")
      opts <- paste0('<option value="">-- country --</option>',
                     paste0('<option value="', htmltools::htmlEscape(wmx_countries),
                            '"', ifelse(wmx_countries == default_ct,
                                        ' selected', ''),
                            '>', htmltools::htmlEscape(wmx_countries),
                            '</option>', collapse = ""))
      paste0('<select id="wmx_ct_', rid, '" disabled ',
             'onchange="Shiny.setInputValue(\'wmx_specify_ct_', rid,
             '\', this.value, {priority:\'event\'});">',
             opts, '</select>')
    }, character(1))
    ...
    league_html <- vapply(seq_len(nrow(verified)), function(i) {
      rid <- row_ids[i]
      init_ct <- verified$winamax_country_name[i] %||%
        (if (verified$fs_country[i] %in% wmx_countries)
          verified$fs_country[i] else NA_character_)
      choices <- if (!is.na(init_ct) && !is.null(wdir))
        sort(unique(wdir$league_name[
          wdir$country_name == init_ct]))
      else character()
      opts <- paste0('<option value="">-- league --</option>',
                     paste0('<option value="', htmltools::htmlEscape(choices),
                            '">', htmltools::htmlEscape(choices),
                            '</option>', collapse = ""))
      paste0('<select id="wmx_lg_', rid, '" disabled ',
             'onchange="Shiny.setInputValue(\'wmx_specify_lg_', rid,
             '\', this.value, {priority:\'event\'});">',
             opts, '</select>')
    }, character(1))
    ...
    display <- data.frame(
      ...
      SB_League       = verified$sb_league,
      FS_League       = verified$fs_league,
      Proposed_Winamax = ifelse(is.na(verified$winamax_league_id), "\u2014",
                                paste0(verified$winamax_country_name, " \u00b7 ",
                                       verified$winamax_league_name)),
      Reason          = ifelse(is.na(verified$winamax_match_reason),
                               "not attempted", verified$winamax_match_reason),
      Action          = action_html,
      Specify_Country = country_html,
      Specify_League  = league_html,
      Reviewed_on     = verified$winamax_reviewed_on %||% NA,
      stringsAsFactors = FALSE
    )
  ...
  observe({
    ...
    for (i in seq_len(nrow(verified))) {
      local({
        rid <- row_ids[i]
        ct_input <- paste0("wmx_specify_ct_", rid)
        observeEvent(input[[ct_input]], {
          ct <- input[[ct_input]]
          if (is.null(ct) || !nzchar(ct)) return()
          choices <- sort(unique(rv$wmx_dir$league_name[
            rv$wmx_dir$country_name == ct]))
          opts <- paste0('<option value="">-- league --</option>',
                         paste0('<option value="',
                                htmltools::htmlEscape(choices), '">',
                                htmltools::htmlEscape(choices), '</option>',
                                collapse = ""))
          session$sendCustomMessage("updateLeagueOpts",
                                    list(id = paste0("wmx_lg_", rid), html = opts))
        }, ignoreInit = TRUE)
      })
    }
  })
  
  observeEvent(input$wmx_submit_btn, {
    req(rv$mapper, rv$wmx_dir)
    m <- rv$mapper
    ...
    for (i in seq_len(nrow(verified))) {
      row <- verified[i, ]
      rid <- row_ids[i]
      action <- input[[paste0("wmx_action_", rid)]]
      if (is.null(action) || !nzchar(action)) {
        action <- if (!is.na(row$winamax_league_id)) "Approve" else "Not Available"
      }
      full_idx <- which(m$sb_country == row$sb_country &
                          m$sb_league  == row$sb_league)
      if (length(full_idx) == 0) { n_skip <- n_skip + 1; next }
      ...
      if (action == "Approve") {
        if (is.na(row$winamax_league_id)) { n_skip <- n_skip + 1; next }
        m$winamax_reviewed_on[full_idx] <- today
        n_ok <- n_ok + 1
      } else if (action == "Not Available") {
        m$winamax_country_id[full_idx]   <- NA_integer_
        m$winamax_league_id[full_idx]    <- NA_integer_
        m$winamax_country_name[full_idx] <- NA_character_
        m$winamax_league_name[full_idx]  <- NA_character_
        m$winamax_url[full_idx]          <- NA_character_
        m$winamax_match_reason[full_idx] <- "not_available"
        m$winamax_reviewed_on[full_idx]  <- today
        n_ok <- n_ok + 1
      } else if (action == "Specify Winamax") {
        ct <- input[[paste0("wmx_specify_ct_", rid)]]
        lg <- input[[paste0("wmx_specify_lg_", rid)]]
        if (is.null(ct) || is.na(ct) || !nzchar(ct)) ct <- row$winamax_country_name
        if (is.null(ct) || is.na(ct) || !nzchar(ct)) {
          if (!is.null(rv$wmx_dir) &&
              row$fs_country %in% rv$wmx_dir$country_name) {
            ct <- row$fs_country
          }
        }
        if (is.null(lg) || is.na(lg) || !nzchar(lg)) lg <- row$winamax_league_name
        if (is.null(ct) || is.na(ct) || !nzchar(ct) ||
            is.null(lg) || is.na(lg) || !nzchar(lg)) {
          n_skip <- n_skip + 1; next
        }
        pick <- rv$wmx_dir[rv$wmx_dir$country_name == ct &
                             rv$wmx_dir$league_name == lg, , drop = FALSE]
        if (nrow(pick) == 0) { n_skip <- n_skip + 1; next }
        m$winamax_country_id[full_idx]   <- pick$country_id[1]
        m$winamax_league_id[full_idx]    <- pick$league_id[1]
        m$winamax_country_name[full_idx] <- pick$country_name[1]
        m$winamax_league_name[full_idx]  <- pick$league_name[1]
        m$winamax_url[full_idx]          <- pick$url[1]
        m$winamax_match_reason[full_idx] <- "manual"
        m$winamax_reviewed_on[full_idx]  <- today
        n_ok <- n_ok + 1
      }
    }
    ...
    rv$mapper <- m
    saveRDS(m, MAP_PATH)
    rv$wmx_last_action <- paste("Saved at", format(Sys.time(), "%H:%M:%S"))
    showNotification(paste0("Submitted ", n_ok, " Winamax decisions",
                            if (n_skip > 0) paste0(" (", n_skip, " skipped)") else ""),
                     type = "message", duration = 5)
  })
}
