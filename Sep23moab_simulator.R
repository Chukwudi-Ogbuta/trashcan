...
if (file.exists(SB_HELPERS_PATH)) source(SB_HELPERS_PATH, local = FALSE)
...
ui <- fluidPage(
  ...
  div(class = "pg",
      tabsetPanel(id = "tabs",
                           ...
                           div(style = "padding-top:8px;",
                               ...
                               div(class = "cd",
                                   ...
                                   div(style = "display:flex; gap:10px; align-items:center; flex-wrap:wrap;",
                                       ...
                                       downloadButton("dl_results_rds", "Save sim-ready file (RDS)", class = "bo"),
                  ...
                  tabPanel("Flat Data", value = "flat",
                           div(style = "padding-top:8px;",
                               div(class = "lab", "Flat data view"),
                               div(class = "lab-sub", "One row per pick. All features exposed. Filter and search to spot what differentiates winners from losers."),
                               div(class = "cd",
                                   div(class = "ct", "Filters"),
                                   div(style = "display:flex; gap:10px; flex-wrap:wrap; align-items:center;",
                                       selectInput("flat_market", "Market", choices = NULL, selected = NULL, width = "200px"),
                                       selectInput("flat_outcome", "Outcome", choices = c("All", "WIN", "LOSS", "PENDING"), selected = "All", width = "130px"),
                                       selectInput("flat_round", "Round", choices = c("All"), selected = "All", width = "120px"),
                                       textInput("flat_country", "Country", placeholder = "filter...", width = "150px"),
                                       textInput("flat_league", "League", placeholder = "filter...", width = "180px"),
                                       div(style = "margin-left:auto; display:flex; gap:10px;",
                                           downloadButton("dl_flat_csv", "Download CSV", class = "bo"),
                                           downloadButton("dl_flat_xlsx", "Download Excel", class = "bgold")))),
                               div(class = "cd",
                                   div(class = "ct", "Picks"),
                                   div(style = "font-family:var(--fb); font-size:11px; color:var(--text-2); margin-bottom:8px;",
...
server <- function(input, output, session) {
  ...
  observeEvent(input$enriched_upload, {
    ...
    # If this is a merged sim-ready file (each fixture has result_*
    # fields baked in), auto-extract results into the cache so we
    # can grade without re-scraping.
    extracted_cache <- list()
    for (it in df) {
      fx <- it$fixture
      if (is.null(fx) || nrow(fx) == 0) next
      if (!"result_status" %in% names(fx)) next
      mid <- as.character(fx$match_id[1])
      if (is.null(mid) || is.na(mid)) next
      extracted_cache[[mid]] <- list(
        status        = fx$result_status[1] %||% NA,
        ft_home       = fx$result_ft_home[1] %||% NA,
        ft_away       = fx$result_ft_away[1] %||% NA,
        ht_home       = fx$result_ht_home[1] %||% NA,
        ht_away       = fx$result_ht_away[1] %||% NA,
        total_corners = fx$result_total_corners[1] %||% NA,
        home_corners  = fx$result_home_corners[1] %||% NA,
        away_corners  = fx$result_away_corners[1] %||% NA,
        total_cards   = fx$result_total_cards[1] %||% NA,
        home_cards    = fx$result_home_cards[1] %||% NA,
        away_cards    = fx$result_away_cards[1] %||% NA,
        total_shots   = fx$result_total_shots[1] %||% NA,
        total_sot     = fx$result_total_sot[1] %||% NA,
        goal_times_home = fx$result_goal_times_home[[1]] %||% list(),
        goal_times_away = fx$result_goal_times_away[[1]] %||% list(),
        stats_home    = fx$result_stats_home[[1]] %||% list(),
        stats_away    = fx$result_stats_away[[1]] %||% list(),
        stats_home_1h = fx$result_stats_home_1h[[1]] %||% list(),
        stats_away_1h = fx$result_stats_away_1h[[1]] %||% list(),
        stats_home_2h = fx$result_stats_home_2h[[1]] %||% list(),
        stats_away_2h = fx$result_stats_away_2h[[1]] %||% list()
      )
    }
    if (length(extracted_cache) > 0) {
      rv$results_cache <- extracted_cache
      n_final <- sum(sapply(extracted_cache, function(r) {
        st <- r$status; !is.null(st) && !is.na(st) && st == "FINAL"
      }))
      log_msg(paste0("File is sim-ready: extracted ", length(extracted_cache),
                     " results (", n_final, " FINAL) — grading enabled."))
    }
  })
  ...
  # (Removed standalone results-upload — merged sim-ready files now carry
  # results inside fixture$result_* and are extracted by enriched upload.)
  ...
  observeEvent(input$scrape_btn, {
    ...
    tryCatch({
      ...
      # containing both features + results — ready for grid search.
      merged <- rv$enriched
      scalar_fields <- c("status", "ft_home", "ft_away", "ht_home", "ht_away",
                         "total_corners", "home_corners", "away_corners",
                         "total_cards", "home_cards", "away_cards",
                         "total_shots", "total_sot")
      list_fields   <- c("goal_times_home", "goal_times_away",
                         "stats_home", "stats_away",
                         "stats_home_1h", "stats_away_1h",
                         "stats_home_2h", "stats_away_2h")
      for (i in seq_along(merged)) {
        mid <- as.character(merged[[i]]$fixture$match_id %||% NA)
        r <- cache[[mid]]
        for (f in scalar_fields) {
          col <- paste0("result_", f)
          v <- if (!is.null(r)) r[[f]] else NULL
          merged[[i]]$fixture[[col]] <- if (is.null(v) || length(v) == 0) NA else v[1]
        }
        for (f in list_fields) {
          col <- paste0("result_", f)
          v <- if (!is.null(r)) r[[f]] else NULL
          merged[[i]]$fixture[[col]] <- I(list(if (is.null(v)) list() else v))
        }
      }
  ...
  # ─────────────────────────────────────────────────────────────
  # Flat data builder — one wide row per pick, all features visible.
  # Joins each pick back to its enriched fixture and explodes form/h2h.
  # ─────────────────────────────────────────────────────────────
  build_flat_data <- function(preds, enriched) {
    if (length(preds) == 0 || length(enriched) == 0) return(NULL)
    # Build lookup: match_id -> enriched fixture object
    by_id <- list()
    for (ef in enriched) {
      fx <- ef$fixture
      if (is.null(fx) || nrow(fx) == 0) next
    ...
    rws <- list()
    for (mkt in names(preds)) {
      df <- preds[[mkt]]
      if (is.null(df) || nrow(df) == 0) next
      for (i in seq_len(nrow(df))) {
        mid <- as.character(df$match_id[i])
        ef <- by_id[[mid]]
        if (is.null(ef)) next
        fx <- ef$fixture
        
        # Standings
        h_rank <- if ("home_rank" %in% names(fx)) fx$home_rank[1] else NA
        a_rank <- if ("away_rank" %in% names(fx)) fx$away_rank[1] else NA
        h_ppg <- if ("home_ppg" %in% names(fx)) fx$home_ppg[1] else NA
        a_ppg <- if ("away_ppg" %in% names(fx)) fx$away_ppg[1] else NA
        h_gd <- if ("home_gd" %in% names(fx)) fx$home_gd[1] else NA
        a_gd <- if ("away_gd" %in% names(fx)) fx$away_gd[1] else NA
        lg_size <- if ("league_size" %in% names(fx)) fx$league_size[1] else NA
        
        # Result (if graded)
        outcome <- if ("outcome" %in% names(df)) df$outcome[i] else "PENDING"
        ft_score <- ""
        ht_score <- ""
        if ("result_status" %in% names(fx) && !is.na(fx$result_status[1]) &&
            fx$result_status[1] == "FINAL") {
          fh <- fx$result_ft_home[1]; fa <- fx$result_ft_away[1]
          hh <- fx$result_ht_home[1]; ha <- fx$result_ht_away[1]
          if (!is.na(fh) && !is.na(fa)) ft_score <- paste0(fh, "-", fa)
          if (!is.na(hh) && !is.na(ha)) ht_score <- paste0(hh, "-", ha)
        }
        
        # Round (derived from sim history or NA)
        rnd <- if ("round" %in% names(df)) df$round[i] else NA
        ...
        row <- list(
          ...
          league       = df$league[i],
          fixture_date = df$fixture_date[i],
          match_time   = if ("match_time" %in% names(df)) df$match_time[i] else NA,
          home         = df$home[i],
          away         = df$away[i],
          pick         = df$pick[i],
          ...
          ft_score     = ft_score,
          ht_score     = ht_score,
          h_rank       = h_rank, a_rank = a_rank, league_size = lg_size,
          h_ppg = h_ppg, a_ppg = a_ppg, h_gd = h_gd, a_gd = a_gd,
          match_id = mid,
          round = rnd
        )
        
        # Home team last 7 form (H1-H7)
        hf <- ef$home_form %||% list()
        for (k in 1:7) {
          if (length(hf) >= k) {
            m <- hf[[k]]
            ft_f <- m$ft_for %||% NA
            ft_a <- m$ft_against %||% NA
            ht_f <- m$ht_for %||% NA
            ht_a <- m$ht_against %||% NA
            score <- if (!is.na(ft_f) && !is.na(ft_a)) paste0(ft_f, "-", ft_a) else ""
            ht <- if (!is.na(ht_f) && !is.na(ht_a)) paste0(ht_f, "-", ht_a) else ""
            res <- if (!is.na(ft_f) && !is.na(ft_a)) {
              if (ft_f > ft_a) "W" else if (ft_f < ft_a) "L" else "D"
            } else ""
            venue <- if (isTRUE(m$concerned_was_home)) "h" else "a"
            comp <- m$competition_full %||% ""
          } else {
            score <- ""; ht <- ""; res <- ""; venue <- ""; comp <- ""
          }
        ...
        # Away team last 7 (A1-A7)
        af <- ef$away_form %||% list()
        for (k in 1:7) {
          if (length(af) >= k) {
            m <- af[[k]]
            ft_f <- m$ft_for %||% NA
        ...
        for (k in 1:5) {
          if (length(h2h) >= k) {
            ...
            score <- if (!is.na(ft_f) && !is.na(ft_a)) paste0(ft_f, "-", ft_a) else ""
            ht <- if (!is.na(ht_f) && !is.na(ht_a)) paste0(ht_f, "-", ht_a) else ""
            # h2h "concerned" team is home team of CURRENT fixture
            res <- if (!is.na(ft_f) && !is.na(ft_a)) {
              if (ft_f > ft_a) "W" else if (ft_f < ft_a) "L" else "D"
            } else ""
            venue <- if (isTRUE(m$concerned_was_home)) "h" else "a"
  ...
  observeEvent(input$run_btn, {
    ...
    # SB filtering already happened in main MOAB pipeline (via mapper).
    # Enriched fixtures are trusted as-is.
    enriched_for_run <- rv$enriched
    
    withProgress(message = "Simulating", value = 0.3, {
      tryCatch({
        preds <- env$run_moab_analysis(enriched_for_run, thresholds)
        rv$sim_results <- preds
        rv$graded <- grade_with_cache(preds)
        rv$flat_data <- tryCatch(build_flat_data(rv$graded %||% preds, enriched_for_run), error = function(e) NULL)
        ...
        log_msg(paste0("RUN ERROR: ", e$message))
        log_msg(paste0("TRACE: ", paste(deparse(sys.calls()), collapse = " | ")))
  ...
  observeEvent(input$commit_btn, {
    ...
    showModal(modalDialog(
      title = "Commit run",
      textInput("commit_label", "Label for this run",
                value = paste0("Sim_", format(Sys.time(), "%Y%m%d_%H%M")),
                width = "100%"),
      footer = tagList(modalButton("Cancel"),
                       actionButton("commit_confirm", "Commit", class = "bd")),
      easyClose = TRUE))
  })
  
  observeEvent(input$commit_confirm, {
    req(rv$sim_results)
    label <- trimws(input$commit_label %||% "")
    if (nchar(label) == 0) label <- paste0("Sim_", format(Sys.time(), "%Y%m%d_%H%M"))
    ...
    log_msg(paste0("Committed run: ", label))
    showNotification(paste0("Committed: ", label), type = "message", duration = 3)
    removeModal()
  })
  ...
  # Flat data tab
  # ─────────────────────────────────────────────────────────────
  # Update market choices when flat_data changes
  observe({
    fd <- rv$flat_data
    if (is.null(fd) || nrow(fd) == 0) {
      updateSelectInput(session, "flat_market", choices = character(), selected = NULL)
      updateSelectInput(session, "flat_round", choices = c("All"), selected = "All")
      return()
    }
    markets <- unique(fd$market)
    updateSelectInput(session, "flat_market",
                      choices = c("All", markets),
                      selected = if (length(markets) > 0) markets[1] else "All")
    rnds <- unique(as.character(fd$round))
    rnds <- rnds[!is.na(rnds) & rnds != ""]
    updateSelectInput(session, "flat_round",
                      choices = c("All", rnds),
                      selected = "All")
  })

  # Filtered flat data
  flat_filtered <- reactive({
    fd <- rv$flat_data
    if (is.null(fd) || nrow(fd) == 0) return(NULL)
    d <- fd
    mkt <- input$flat_market %||% "All"
    if (mkt != "All" && !is.null(mkt)) d <- d[d$market == mkt, , drop = FALSE]
    oc <- input$flat_outcome %||% "All"
    if (oc != "All") d <- d[d$outcome == oc, , drop = FALSE]
    rnd <- input$flat_round %||% "All"
    if (rnd != "All") d <- d[as.character(d$round) == rnd, , drop = FALSE]
    cf <- trimws(input$flat_country %||% "")
    if (nzchar(cf)) d <- d[grepl(cf, d$country, ignore.case = TRUE), , drop = FALSE]
    lf <- trimws(input$flat_league %||% "")
    if (nzchar(lf)) d <- d[grepl(lf, d$league, ignore.case = TRUE), , drop = FALSE]
    d
  })
  
  observe({
    if (!is.null(rv$flat_data)) {
      log_msg(paste0("Flat data built: ", nrow(rv$flat_data), " rows, ", ncol(rv$flat_data), " cols"))
    }
  })

  output$flat_count_status <- renderText({
    fd <- flat_filtered()
    if (is.null(fd) || nrow(fd) == 0) return("No picks yet. Run a simulation first.")
    w <- sum(fd$outcome == "WIN", na.rm = TRUE)
    l <- sum(fd$outcome == "LOSS", na.rm = TRUE)
    p <- sum(fd$outcome == "PENDING", na.rm = TRUE)
    hr <- if ((w + l) > 0) sprintf(" (hit %.1f%%)", 100 * w / (w + l)) else ""
    paste0(nrow(fd), " picks shown  \u00b7  ", w, " W / ", l, " L / ", p, " pending", hr)
  })

  output$flat_data_table <- DT::renderDataTable({
    fd <- flat_filtered()
    if (is.null(fd) || nrow(fd) == 0) {
      return(DT::datatable(data.frame(message = "Run a simulation to see flat data."),
  ...
  output$dl_flat_xlsx <- downloadHandler(
    ...
    content = function(file) {
      fd <- flat_filtered()
      if (is.null(fd)) fd <- data.frame(message = "No data")
      wb <- openxlsx::createWorkbook()
      openxlsx::addWorksheet(wb, "picks")
      openxlsx::writeData(wb, "picks", fd)
      openxlsx::freezePane(wb, "picks", firstActiveRow = 2, firstActiveCol = 5)
      # Color-code WIN / LOSS rows
      if (nrow(fd) > 0 && "outcome" %in% names(fd)) {
        win_style <- openxlsx::createStyle(fgFill = "#e8f7ee")
        loss_style <- openxlsx::createStyle(fgFill = "#fde4e1")
        win_rows <- which(fd$outcome == "WIN") + 1L
        loss_rows <- which(fd$outcome == "LOSS") + 1L
        if (length(win_rows) > 0)
          openxlsx::addStyle(wb, "picks", win_style, rows = win_rows,
                              cols = seq_len(ncol(fd)), gridExpand = TRUE)
        if (length(loss_rows) > 0)
          openxlsx::addStyle(wb, "picks", loss_style, rows = loss_rows,
                              cols = seq_len(ncol(fd)), gridExpand = TRUE)
      }
  ...
  output$comparison_grid <- renderUI({
    ...
    body_rows <- lapply(ord, function(id) {
      ...
      actions <- sprintf('<td class="grid-cell" style="white-space:nowrap;"><button class="bo" onclick="Shiny.setInputValue(\'rename_run\',\'%s\',{priority:\'event\'});" style="padding:4px 8px; font-size:9px; margin-right:4px;">RENAME</button><button class="bo bdanger" onclick="Shiny.setInputValue(\'delete_run\',\'%s\',{priority:\'event\'});" style="padding:4px 8px; font-size:9px; color:#fff;">DELETE</button></td>', id, id)
      paste0('<tr><td style="padding:10px 12px; font-weight:600;">', e$label, '</td>', cells, ov, actions, '</tr>')
    })
    header <- paste0('<th style="padding:10px 12px; background:var(--ivory-2);">Run</th>',
                     ...
                     '<th style="padding:10px 12px; background:var(--ivory-2); font-size:9px;">ACTIONS</th>')
  ...
  observeEvent(input$rename_run, {
    id <- input$rename_run
    if (is.null(id) || !id %in% names(rv$sim_history)) return()
    current <- rv$sim_history[[id]]$label
    showModal(modalDialog(
      title = "Rename run",
      textInput("rename_label", "New label", value = current, width = "100%"),
      footer = tagList(modalButton("Cancel"),
                       actionButton("rename_confirm", "Save", class = "bd")),
      easyClose = TRUE))
    session$userData$rename_target <- id
  })
  
  observeEvent(input$rename_confirm, {
    id <- session$userData$rename_target
    if (is.null(id) || !id %in% names(rv$sim_history)) return()
    new_label <- trimws(input$rename_label %||% "")
    if (nchar(new_label) == 0) { removeModal(); return() }
    rv$sim_history[[id]]$label <- new_label
    save_sim_history(rv$sim_history)
    log_msg(paste0("Renamed to: ", new_label))
    removeModal()
  })
  
  observeEvent(input$delete_run, {
    id <- input$delete_run
    if (is.null(id) || !id %in% names(rv$sim_history)) return()
    label <- rv$sim_history[[id]]$label
    showModal(modalDialog(
      title = "Delete run?",
      HTML(paste0("<p>Delete <strong>", htmltools::htmlEscape(label), "</strong>?</p>")),
      footer = tagList(modalButton("Cancel"),
                       actionButton("delete_confirm", "Delete", class = "bd bdanger")),
      easyClose = TRUE))
    session$userData$delete_target <- id
  })
  
  observeEvent(input$delete_confirm, {
    id <- session$userData$delete_target
    if (is.null(id) || !id %in% names(rv$sim_history)) return()
    label <- rv$sim_history[[id]]$label
    rv$sim_history[[id]] <- NULL
    save_sim_history(rv$sim_history)
    log_msg(paste0("Deleted: ", label))
    removeModal()
  })
}
