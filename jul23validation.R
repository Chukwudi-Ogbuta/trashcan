...
validate_report_local <- function(report_id, snap) {
  ...
  ## --- check #3: pump continuity ------------------------------------------
  ## Priority: same-day earlier block for this pump. If none, walk back to
  ## the most recent prior day's last block that has this pump.
  if (nrow(psales_here) > 0) {
    ## Candidate 1: same-day earlier blocks (same station)
    same_day_earlier <- snap$reports[snap$reports$station_id == station_id &
                                     snap$reports$report_date == rep_date &
                                     snap$reports$block_number < rep_block, ,
                                     drop = FALSE]
    ## Candidate 2: any prior day (same station)
    prior_day <- snap$reports[snap$reports$station_id == station_id &
                              snap$reports$report_date < rep_date, ,
                              drop = FALSE]

    candidate_reports <- rbind(same_day_earlier, prior_day)
    if (nrow(candidate_reports) > 0) {
      prior_ps <- snap$pump_sales[snap$pump_sales$report_id %in% candidate_reports$id &
                                  snap$pump_sales$pump_id %in% psales_here$pump_id, ,
                                  drop = FALSE]
      if (nrow(prior_ps) > 0) {
        prior_ps <- merge(prior_ps, candidate_reports,
                          by.x = "report_id", by.y = "id")
        ## Rank: same-day earlier blocks first (nearest block descending),
        ## then prior days (most recent day descending, then latest block).
        prior_ps$is_same_day <- prior_ps$report_date == rep_date
        prior_ps <- prior_ps %>%
          dplyr::arrange(dplyr::desc(is_same_day),
                         dplyr::desc(report_date),
        ...
        for (j in seq_len(nrow(joined))) {
          ...
          if (length(o) == 1 && length(pc) == 1 &&
            ...
            if (abs(diff) > 0.5) {
              ...
              ref_label <- if (isTRUE(joined$is_same_day[j]))
                             sprintf("block %d closing",
                                     as.integer(joined$block_number[j]))
                           else
                             sprintf("prior closing (%s)",
                                     format(as.Date(joined$report_date[j]), "%d %b"))
              direction <- if (diff < 0)
                             paste0("today opening %s < ", ref_label,
                                    " %s (rollback)")
                           else
                             paste0("today opening %s > ", ref_label,
                                    " %s (gap suggests unrecorded sales)")
              add_issue(
                type = "pump_continuity_gap",
                severity = "high",
                variance = diff,
                note = sprintf(
                  paste0("P.%s: ", direction),
                  pnum, o, pc
                )
              )
            }
