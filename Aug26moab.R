...
server <- function(input, output, session) {
  ...
  observeEvent(input$fetch_btn, {
    ...
    # SportyBet filter — drop leagues not available on SportyBet today
    if (isTRUE(input$sb_scrape_filter)) {
      log_msg("Fetching live SportyBet directory...")
      sb_dir <- tryCatch(fetch_sportybet_directory(), error = function(e) NULL)
      rv$sb_directory <- sb_dir
      if (is.null(sb_dir) || nrow(sb_dir) == 0) {
        log_msg("WARN: Could not load SportyBet directory; scraping ALL verified leagues.")
      } else {
        log_msg(paste0("SportyBet API: ", nrow(sb_dir), " leagues across ",
                       length(unique(sb_dir$country)), " countries"))
        # Build a df with country/league columns matching what filter expects
        before_n <- nrow(verified)
        v_for_filt <- data.frame(country = as.character(verified$country),
                                 league  = as.character(verified$league_name),
                                 stringsAsFactors = FALSE)
        filt <- filter_to_sportybet_df(v_for_filt, sb_dir, fuzzy_threshold = 0.75)
        rv$sb_diagnostic <- filt$diagnostic
        verified <- verified[filt$matched, , drop = FALSE]
        n_exact <- if (!is.null(filt$diagnostic)) sum(filt$diagnostic$reason == "exact", na.rm = TRUE) else NA
        n_fuzzy <- if (!is.null(filt$diagnostic)) sum(grepl("^fuzzy", filt$diagnostic$reason), na.rm = TRUE) else NA
        log_msg(paste0("SB filter: kept ", nrow(verified), " of ", before_n,
                       " leagues  |  exact: ", n_exact, "  fuzzy: ", n_fuzzy,
                       "  dropped: ", before_n - nrow(verified)))
      }
    }
