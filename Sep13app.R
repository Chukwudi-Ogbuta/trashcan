...
server <- function(input, output, session) {
  ...
  build_cassette_code <- function(src, key) {
    ...
    hdr <- c(
      ...
      if (!is.null(src$stage2_combo))
        paste0("# Stage 2 filter: ", paste(sapply(src$stage2_combo, function(x) x$id %||% x$name %||% "?"), collapse = " + "))
  ...
  cassette_runner_code <- function() {
    paste(c(
      ...
      "    for (intu in stage2_combo) {",
      "      if (is.function(intu$filter_fn)) {",
      "        picked <- picked & intu$filter_fn(feat_df)",
      "      }",
      "    }",
      "  }",
      "  hits <- which(picked)",
  ...
  observeEvent(input$ranker_run_btn, {
    ...
    if (length(keep) < 2) {
      showNotification("Need at least 2 features.", type = "error"); return()
    }
    ...
    withProgress(message = "Building Tune feature matrix...", value = 0.05, {
      ...
      tune_res <- compute_ranking(feat_only, y)
      ...
      if (isTRUE(input$ranker_use_validate) &&
        ...
        val_res <- compute_ranking(val_feat_only, val_y)
      }
