      ...
      fluidRow(
        ...
        column(9,
          ...
          tabsetPanel(
            ...
            tabPanel("Rescue (mine Stage 1 rejected)",
              br(),
              p(em("Applied to fixtures the Stage 1 rule REJECTED. Improvement = recovered wins at high hit rate.")),
              DTOutput("s2_rescue_lb")
            ),
...
server <- function(input, output, session) {
  ...
  output$s2_summary <- renderText({
    ...
    paste(
      ...
      paste0("Stage 1 rejected: ", r$n_rejected,
             "  baseline hit rate on rejected: ",
             round((r$stage1_rejected_baseline %||% NA) * 100, 2), "%"),
  ...
  output$s2_rescue_lb <- renderDT({ render_stage2_lb(rv$stage2_result$rescue) })
