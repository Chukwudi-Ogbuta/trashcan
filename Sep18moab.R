...
parse_stats_page <- function(page) {
  ...
  # Prefer stable data-testid selectors; fall back to hashed classes if needed.
...
run_parallel_worker <- function(worker_id, work_chunk, chrome_path, chromedriver_path,
  ...
  scrape_stats <- function(match_url, skip_stats = FALSE) {
    ...
    parse_tab <- function(page) {
      ...
      if (length(rows) == 0) rows <- page %>% html_nodes("div.wcl-row_2oCpS")
      for (r in rows) {
        cn <- r %>% html_node("div[data-testid='wcl-statistics-category'] span")
        ...
        if (is.null(hv) || is.null(av)) next
        key <- slug(cat_name)
        home[[key]] <- html_text(hv, trim = TRUE)
        away[[key]] <- html_text(av, trim = TRUE)
      }
      list(home = home, away = away)
    }
