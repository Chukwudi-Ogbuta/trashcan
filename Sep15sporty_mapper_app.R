...
WMX_HLP    <- file.path(BASE_PATH, "winamax_helpers.R")

source(HELPERS)
if (file.exists(XBET_HLP)) source(XBET_HLP)
if (file.exists(WMX_HLP)) source(WMX_HLP)
  ...
  div(class = "pg",
      ...
      div(class = "card",
          tabsetPanel(id = "tabs",
                      ...
                      tabPanel("Winamax mapping",
                               br(),
                               div(style = "margin-bottom:12px; display:flex; gap:12px; align-items:center;",
                                   actionButton("wmx_refresh_btn", "Refresh from Winamax (Selenium)",
                                                class = "btn-primary"),
                                   actionButton("wmx_submit_btn", "Submit Winamax reviews",
                                                class = "btn-primary"),
                                   div(style = "flex:1;"),
                                   textOutput("wmx_last_action", inline = TRUE)),
                               div(style = "margin-bottom:12px;",
                                   radioButtons("wmx_filter", NULL,
                                                choices = c("Needs review" = "needs",
                                                            "Reviewed" = "reviewed",
                                                            "All" = "all"),
                                                selected = "needs", inline = TRUE)),
                               div(style = "margin-bottom:12px; font-size:12px; color:#666;",
                                   "Only Verified SB rows shown. Approve auto-match, mark as Not Available on Winamax, or Specify manually."),
                               uiOutput("wmx_review_ui")),
