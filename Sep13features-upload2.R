...
# ─── Stat count helpers: count matches in window where a stat meets a threshold ─
# Direction: "ge" (>=) or "le" (<=). Returns integer count, NA if window empty.
count_stat_ge <- function(form_window, stat_name, threshold, half = c("full", "1h", "2h")) {
  half <- match.arg(half)
  container <- switch(half, full = "stats", `1h` = "stats_1h", `2h` = "stats_2h")
  if (is.null(form_window) || length(form_window) == 0) return(NA_integer_)
  sum(vapply(form_window, function(m) {
    s <- m[[container]]
    if (is.null(s) || length(s) == 0) return(FALSE)
    v <- parse_stat_value(s[[stat_name]])
    !is.na(v) && v >= threshold
  }, logical(1)))
}
count_stat_le <- function(form_window, stat_name, threshold, half = c("full", "1h", "2h")) {
  half <- match.arg(half)
  container <- switch(half, full = "stats", `1h` = "stats_1h", `2h` = "stats_2h")
  if (is.null(form_window) || length(form_window) == 0) return(NA_integer_)
  sum(vapply(form_window, function(m) {
...
# Ratio-based: SOT / total_shots per match, count matches meeting threshold
count_shot_accuracy_ge <- function(form_window, threshold_pct, half = c("full", "1h", "2h")) {
  half <- match.arg(half)
  container <- switch(half, full = "stats", `1h` = "stats_1h", `2h` = "stats_2h")
  if (is.null(form_window) || length(form_window) == 0) return(NA_integer_)
  sum(vapply(form_window, function(m) {
    s <- m[[container]]
    if (is.null(s) || length(s) == 0) return(FALSE)
    sot <- parse_stat_value(s$shots_on_target)
    tot <- parse_stat_value(s$total_shots)
    if (is.na(sot) || is.na(tot) || tot == 0) return(FALSE)
    (sot / tot * 100) >= threshold_pct
  }, logical(1)))
}
count_shot_accuracy_le <- function(form_window, threshold_pct, half = c("full", "1h", "2h")) {
  half <- match.arg(half)
  container <- switch(half, full = "stats", `1h` = "stats_1h", `2h` = "stats_2h")
  if (is.null(form_window) || length(form_window) == 0) return(NA_integer_)
  sum(vapply(form_window, function(m) {
...
# Over/under-performing: goals scored vs xG in each match
count_over_perform_ge <- function(form_window, gap) {
  if (is.null(form_window) || length(form_window) == 0) return(NA_integer_)
  sum(vapply(form_window, function(m) {
    if (is.null(m$stats) || length(m$stats) == 0) return(FALSE)
    xg <- parse_stat_value(m$stats$expected_goals_xg)
    if (is.na(xg) || is_missing_scalar(m$ft_for)) return(FALSE)
    goals <- as.numeric(m$ft_for[1])
    !is.na(goals) && (goals - xg) >= gap
  }, logical(1)))
}
count_under_perform_ge <- function(form_window, gap) {
  if (is.null(form_window) || length(form_window) == 0) return(NA_integer_)
  sum(vapply(form_window, function(m) {
    if (is.null(m$stats) || length(m$stats) == 0) return(FALSE)
    xg <- parse_stat_value(m$stats$expected_goals_xg)
...
pred_win   <- function(m) {
  if (is_missing_scalar(m$ft_for) || is_missing_scalar(m$ft_against)) return(FALSE)
  as.numeric(m$ft_for[1]) > as.numeric(m$ft_against[1])
}
pred_draw  <- function(m) {
  if (is_missing_scalar(m$ft_for) || is_missing_scalar(m$ft_against)) return(FALSE)
  as.numeric(m$ft_for[1]) == as.numeric(m$ft_against[1])
}
pred_loss  <- function(m) {
  if (is_missing_scalar(m$ft_for) || is_missing_scalar(m$ft_against)) return(FALSE)
  as.numeric(m$ft_for[1]) < as.numeric(m$ft_against[1])
}

# Was the match a goalless draw (0-0)?
pred_goalless_draw <- function(m) {
  pred_draw(m) && !is_missing_scalar(m$ft_for) && as.numeric(m$ft_for[1]) == 0
}

# Max consecutive draws at start of window (streak from most recent)
max_consec_draws <- function(form_window) {
  if (is.null(form_window) || length(form_window) == 0) return(NA_integer_)
  streak <- 0L
  for (m in form_window) {
    if (isTRUE(pred_draw(m))) streak <- streak + 1L
    else break
  }
  streak
}

# Max consecutive draws in H2H (streak from most recent)
h2h_max_consec_draws <- function(h2h_list) {
  if (is.null(h2h_list) || length(h2h_list) == 0) return(NA_integer_)
  streak <- 0L
  for (m in h2h_list) {
    if (isTRUE(pred_draw(m))) streak <- streak + 1L
    else break
...
# Was 1-goal up at any point during the match
pred_was_1up <- function(m) {
  own_times <- parse_goal_times(m$goal_times_concerned)
  opp_times <- parse_goal_times(m$goal_times_opponent)
  if (length(own_times) == 0) return(FALSE)
  mins  <- c(own_times, opp_times)
  sides <- c(rep("own", length(own_times)), rep("opp", length(opp_times)))
  ord <- order(mins)
  sides <- sides[ord]
  own_score <- 0; opp_score <- 0
  for (s in sides) {
    if (s == "own") own_score <- own_score + 1
    else opp_score <- opp_score + 1
    if (own_score > opp_score) return(TRUE)
  }
  FALSE
}

# Goalless start: TRUE if no goal from EITHER side in first `minute` minutes
pred_goalless_start <- function(m, minute = 20) {
  own_times <- parse_goal_times(m$goal_times_concerned)
  opp_times <- parse_goal_times(m$goal_times_opponent)
  all_times <- c(own_times, opp_times)
  if (length(all_times) == 0) return(TRUE)
  min(all_times) > minute
}
...
pred_2h_over25    <- function(m) {
  if (is_missing_scalar(m$ft_for) || is_missing_scalar(m$ft_against) ||
      is_missing_scalar(m$ht_for) || is_missing_scalar(m$ht_against)) return(FALSE)
  ((as.numeric(m$ft_for[1]) - as.numeric(m$ht_for[1])) +
      (as.numeric(m$ft_against[1]) - as.numeric(m$ht_against[1]))) >= 3
}
pred_ht_over15    <- function(m) {
  if (is_missing_scalar(m$ht_for) || is_missing_scalar(m$ht_against)) return(FALSE)
  (as.numeric(m$ht_for[1]) + as.numeric(m$ht_against[1])) >= 2
}
...
# ─── Scored first / Scored back after conceding first ───────
# scored_first_count: games where team's earliest goal minute < opponent's
# scored_back_count : games where opponent scored first AND team scored anyway
scored_first_count <- function(form_window) {
  if (is.null(form_window) || length(form_window) == 0) return(NA_integer_)
  sum(vapply(form_window, function(m) {
    c_times <- parse_goal_times(m$goal_times_concerned)
    o_times <- parse_goal_times(m$goal_times_opponent)
    if (length(c_times) == 0) return(FALSE)
    if (length(o_times) == 0) return(TRUE)
    min(c_times) < min(o_times)
  }, logical(1)))
}

scored_back_count <- function(form_window) {
  if (is.null(form_window) || length(form_window) == 0) return(NA_integer_)
  sum(vapply(form_window, function(m) {
    c_times <- parse_goal_times(m$goal_times_concerned)
    o_times <- parse_goal_times(m$goal_times_opponent)
    if (length(o_times) == 0) return(FALSE)
    if (length(c_times) == 0) return(FALSE)
    min(o_times) < min(c_times)
  }, logical(1)))
}
  ...
  for (fn in feature_names) {
    ...
    if (side %in% c("h", "a") && !is.null(fw)) {
      v <- switch(root,
        ...
        second_h_over_05 = count_where(fw, pred_2h_over_05),
        second_h_over25 = count_where(fw, pred_2h_over25),
        ht_over15       = count_where(fw, pred_ht_over15),
        wins            = count_where(fw, pred_win),
        draws           = count_where(fw, pred_draw),
        losses          = count_where(fw, pred_loss),
        scored_first    = scored_first_count(fw),
        scored_back     = scored_back_count(fw),
        goalless_15     = count_where(fw, function(m) pred_goalless_start(m, 15)),
        goalless_20     = count_where(fw, function(m) pred_goalless_start(m, 20)),
        goalless_45     = count_where(fw, function(m) pred_goalless_start(m, 45)),
        was_1up         = count_where(fw, pred_was_1up),
        # ── "Last match" features (first entry in window) ──
        last_ht_over05      = if (length(fw) > 0) as.integer(pred_ht_scored(fw[[1]]) | pred_ht_conceded(fw[[1]])) else NA_integer_,
        last_over15         = if (length(fw) > 0) as.integer(pred_over_15(fw[[1]])) else NA_integer_,
        last_over25         = if (length(fw) > 0) as.integer(pred_over_25(fw[[1]])) else NA_integer_,
        last_btts           = if (length(fw) > 0) as.integer(pred_btts(fw[[1]])) else NA_integer_,
        last_2h_over05      = if (length(fw) > 0) as.integer(pred_2h_over_05(fw[[1]])) else NA_integer_,
        last_2h_over15      = if (length(fw) > 0) as.integer(pred_2h_over15(fw[[1]])) else NA_integer_,
        last_goalless_draw  = if (length(fw) > 0) as.integer(pred_goalless_draw(fw[[1]])) else NA_integer_,
        max_consec_draws    = max_consec_draws(fw),
        # ── MEAN features ──
        ...
        # ── STAT COUNT features: full match ──
        possession_ge_55        = count_stat_ge(fw, "ball_possession", 55, "full"),
        possession_le_45        = count_stat_le(fw, "ball_possession", 45, "full"),
        shot_accuracy_ge_40     = count_shot_accuracy_ge(fw, 40, "full"),
        shot_accuracy_le_25     = count_shot_accuracy_le(fw, 25, "full"),
        corners_ge_7            = count_stat_ge(fw, "corner_kicks", 7, "full"),
        corners_le_5            = count_stat_le(fw, "corner_kicks", 5, "full"),
        big_chances_ge_4        = count_stat_ge(fw, "big_chances", 4, "full"),
        big_chances_le_2        = count_stat_le(fw, "big_chances", 2, "full"),
        blocked_shots_ge_4      = count_stat_ge(fw, "blocked_shots", 4, "full"),
        xg_ge_1_5               = count_stat_ge(fw, "expected_goals_xg", 1.5, "full"),
        xg_le_0_8               = count_stat_le(fw, "expected_goals_xg", 0.8, "full"),
        gk_saves_ge_4           = count_stat_ge(fw, "goalkeeper_saves", 4, "full"),
        shots_inside_box_ge_8   = count_stat_ge(fw, "shots_inside_the_box", 8, "full"),
        shots_inside_box_le_5   = count_stat_le(fw, "shots_inside_the_box", 5, "full"),
        shots_outside_box_ge_5  = count_stat_ge(fw, "shots_outside_the_box", 5, "full"),
        touches_opp_box_ge_24   = count_stat_ge(fw, "touches_in_opposition_box", 24, "full"),
        touches_opp_box_le_12   = count_stat_le(fw, "touches_in_opposition_box", 12, "full"),
        yellow_cards_ge_3       = count_stat_ge(fw, "yellow_cards", 3, "full"),
        yellow_cards_le_1       = count_stat_le(fw, "yellow_cards", 1, "full"),
        red_cards_ge_1          = count_stat_ge(fw, "red_cards", 1, "full"),
        over_perform_ge_2       = count_over_perform_ge(fw, 2),
        under_perform_ge_2      = count_under_perform_ge(fw, 2),
        # ── STAT COUNT features: 1H ──
        possession_ge_55_1h     = count_stat_ge(fw, "ball_possession", 55, "1h"),
        possession_le_45_1h     = count_stat_le(fw, "ball_possession", 45, "1h"),
        shot_accuracy_ge_40_1h  = count_shot_accuracy_ge(fw, 40, "1h"),
        shot_accuracy_le_25_1h  = count_shot_accuracy_le(fw, 25, "1h"),
        corners_ge_7_1h         = count_stat_ge(fw, "corner_kicks", 7, "1h"),
        corners_le_5_1h         = count_stat_le(fw, "corner_kicks", 5, "1h"),
        big_chances_ge_4_1h     = count_stat_ge(fw, "big_chances", 4, "1h"),
        big_chances_le_2_1h     = count_stat_le(fw, "big_chances", 2, "1h"),
        blocked_shots_ge_4_1h   = count_stat_ge(fw, "blocked_shots", 4, "1h"),
        xg_ge_1_5_1h            = count_stat_ge(fw, "expected_goals_xg", 1.5, "1h"),
        xg_le_0_8_1h            = count_stat_le(fw, "expected_goals_xg", 0.8, "1h"),
        gk_saves_ge_4_1h        = count_stat_ge(fw, "goalkeeper_saves", 4, "1h"),
        shots_inside_box_ge_8_1h = count_stat_ge(fw, "shots_inside_the_box", 8, "1h"),
        shots_inside_box_le_5_1h = count_stat_le(fw, "shots_inside_the_box", 5, "1h"),
        shots_outside_box_ge_5_1h = count_stat_ge(fw, "shots_outside_the_box", 5, "1h"),
        touches_opp_box_ge_24_1h = count_stat_ge(fw, "touches_in_opposition_box", 24, "1h"),
        touches_opp_box_le_12_1h = count_stat_le(fw, "touches_in_opposition_box", 12, "1h"),
        # ── STAT COUNT features: 2H ──
        possession_ge_55_2h     = count_stat_ge(fw, "ball_possession", 55, "2h"),
        possession_le_45_2h     = count_stat_le(fw, "ball_possession", 45, "2h"),
        shot_accuracy_ge_40_2h  = count_shot_accuracy_ge(fw, 40, "2h"),
        shot_accuracy_le_25_2h  = count_shot_accuracy_le(fw, 25, "2h"),
        corners_ge_7_2h         = count_stat_ge(fw, "corner_kicks", 7, "2h"),
        corners_le_5_2h         = count_stat_le(fw, "corner_kicks", 5, "2h"),
        big_chances_ge_4_2h     = count_stat_ge(fw, "big_chances", 4, "2h"),
        big_chances_le_2_2h     = count_stat_le(fw, "big_chances", 2, "2h"),
        blocked_shots_ge_4_2h   = count_stat_ge(fw, "blocked_shots", 4, "2h"),
        xg_ge_1_5_2h            = count_stat_ge(fw, "expected_goals_xg", 1.5, "2h"),
        xg_le_0_8_2h            = count_stat_le(fw, "expected_goals_xg", 0.8, "2h"),
        gk_saves_ge_4_2h        = count_stat_ge(fw, "goalkeeper_saves", 4, "2h"),
        shots_inside_box_ge_8_2h = count_stat_ge(fw, "shots_inside_the_box", 8, "2h"),
        shots_inside_box_le_5_2h = count_stat_le(fw, "shots_inside_the_box", 5, "2h"),
        shots_outside_box_ge_5_2h = count_stat_ge(fw, "shots_outside_the_box", 5, "2h"),
        touches_opp_box_ge_24_2h = count_stat_ge(fw, "touches_in_opposition_box", 24, "2h"),
        touches_opp_box_le_12_2h = count_stat_le(fw, "touches_in_opposition_box", 12, "2h"),
    ...
    if (side == "h2h" && length(h2h_list) > 0) {
      v <- switch(root,
        ...
        avg_ht_goals    = {
          vals <- vapply(h2h_list, function(m) {
            if (is_missing_scalar(m$ht_for) || is_missing_scalar(m$ht_against)) NA_real_
            else as.numeric(m$ht_for[1]) + as.numeric(m$ht_against[1])
          }, numeric(1))
          vals <- vals[!is.na(vals)]
          if (length(vals) == 0) NA_real_ else mean(vals)
        },
        ht_over_05      = {
          if (length(h2h_list) == 0) NA_integer_ else
            sum(vapply(h2h_list, function(m) {
              if (is_missing_scalar(m$ht_for) || is_missing_scalar(m$ht_against)) return(FALSE)
              (as.numeric(m$ht_for[1]) + as.numeric(m$ht_against[1])) >= 1
            }, logical(1)))
        },
        avg_second_h_goals = {
          vals <- vapply(h2h_list, function(m) {
            if (is_missing_scalar(m$ft_for) || is_missing_scalar(m$ft_against) ||
                is_missing_scalar(m$ht_for) || is_missing_scalar(m$ht_against)) NA_real_
            else (as.numeric(m$ft_for[1]) - as.numeric(m$ht_for[1])) +
              (as.numeric(m$ft_against[1]) - as.numeric(m$ht_against[1]))
          }, numeric(1))
          vals <- vals[!is.na(vals)]
          if (length(vals) == 0) NA_real_ else mean(vals)
        },
        second_h_over_05 = {
          if (length(h2h_list) == 0) NA_integer_ else
            sum(vapply(h2h_list, pred_2h_over_05, logical(1)))
        },
        second_h_over_15 = {
          if (length(h2h_list) == 0) NA_integer_ else
            sum(vapply(h2h_list, pred_2h_over15, logical(1)))
        },
        second_h_over_25 = {
          if (length(h2h_list) == 0) NA_integer_ else
            sum(vapply(h2h_list, pred_2h_over25, logical(1)))
        },
        ht_over_15 = {
          if (length(h2h_list) == 0) NA_integer_ else
            sum(vapply(h2h_list, pred_ht_over15, logical(1)))
        },
        home_wins       = {
          if (length(h2h_list) == 0) NA_integer_ else
            sum(vapply(h2h_list, function(m) {
        ...
        draws           = {
          if (length(h2h_list) == 0) NA_integer_ else
            sum(vapply(h2h_list, pred_draw, logical(1)))
        },
        away_wins       = {
          if (length(h2h_list) == 0) NA_integer_ else
            sum(vapply(h2h_list, function(m) {
        ...
        goalless_15     = {
          if (length(h2h_list) == 0) NA_integer_ else
            sum(vapply(h2h_list, function(m) pred_goalless_start(m, 15), logical(1)))
        },
        goalless_20     = {
          if (length(h2h_list) == 0) NA_integer_ else
            sum(vapply(h2h_list, function(m) pred_goalless_start(m, 20), logical(1)))
        },
        goalless_45     = {
          if (length(h2h_list) == 0) NA_integer_ else
            sum(vapply(h2h_list, function(m) pred_goalless_start(m, 45), logical(1)))
        },
        home_was_1up    = {
          if (length(h2h_list) == 0) NA_integer_ else
            sum(vapply(h2h_list, function(m) {
              ...
              if (isTRUE(as.logical(m$concerned_was_home[1]))) {
                ...
                own_times <- parse_goal_times(m$goal_times_opponent)
                opp_times <- parse_goal_times(m$goal_times_concerned)
                if (length(own_times) == 0) return(FALSE)
                mins  <- c(own_times, opp_times)
                sides <- c(rep("own", length(own_times)), rep("opp", length(opp_times)))
                ord <- order(mins); sides <- sides[ord]
                os <- 0; ps <- 0
                for (s in sides) {
                  if (s == "own") os <- os + 1 else ps <- ps + 1
        ...
        last_ht_over05 = if (length(h2h_list) > 0) as.integer(pred_ht_scored(h2h_list[[1]]) | pred_ht_conceded(h2h_list[[1]])) else NA_integer_,
        last_over15    = if (length(h2h_list) > 0) as.integer(pred_over_15(h2h_list[[1]])) else NA_integer_,
        last_over25    = if (length(h2h_list) > 0) as.integer(pred_over_25(h2h_list[[1]])) else NA_integer_,
        last_btts      = if (length(h2h_list) > 0) as.integer(pred_btts(h2h_list[[1]])) else NA_integer_,
        last_2h_over05 = if (length(h2h_list) > 0) as.integer(pred_2h_over_05(h2h_list[[1]])) else NA_integer_,
        last_2h_over15 = if (length(h2h_list) > 0) as.integer(pred_2h_over15(h2h_list[[1]])) else NA_integer_,
        last_goalless_draw = if (length(h2h_list) > 0) as.integer(pred_goalless_draw(h2h_list[[1]])) else NA_integer_,
        max_consec_draws   = h2h_max_consec_draws(h2h_list),
        home_clean_rate = {
          n <- length(h2h_list)
...
FEATURE_CATALOG <- local({
  ...
  for (root_desc in list(
        ...
        list("second_h_over_05","2H Over 0.5 in last N"),
        list("second_h_over25","2H Over 2.5 in last N"),
        list("ht_over15",      "HT Over 1.5 in last N"),
        list("second_h_btts",  "2H BTTS in last N"),
        list("wins",           "Wins in last N"),
        list("draws",          "Draws in last N"),
        list("losses",         "Losses in last N"))) {
    fc <- c(fc, .build_windowed(root_desc[[1]], c("home","away"),
                                c(3,5,7), "goals", root_desc[[2]]))
  }
  
  # Goal-time-based scored first / scored back (needs goal_times)
  for (root_desc in list(
    list("scored_first",  "Times scored the first goal in last N"),
    list("scored_back",   "Times scored after conceding first in last N"),
    list("goalless_15",   "Games with no goal in first 15 min"),
    list("goalless_20",   "Games with no goal in first 20 min"),
    list("goalless_45",   "Games with no goal in first 45 min (goalless 1H)"),
    list("was_1up",       "Games where team was 1+ goal up at any point"))) {
    fc <- c(fc, .build_windowed(root_desc[[1]], c("home","away"),
                                c(3,5,7), "goals", root_desc[[2]], needs_goaltime = TRUE))
  }
  # Goals mean
  for (root_desc in list(
  ...
  # Stat count features (concerned-team, per half) — dominance / pressure / discipline signals
  for (root_desc in list(
    list("possession_ge_55",     "Matches with >=55% possession"),
    list("possession_le_45",     "Matches with <=45% possession (dominated)"),
    list("shot_accuracy_ge_40",  "Matches with >=40% shot accuracy (SOT/total)"),
    list("shot_accuracy_le_25",  "Matches with <=25% shot accuracy (poor)"),
    list("corners_ge_7",         "Matches with >=7 corners won"),
    list("corners_le_5",         "Matches with <=5 corners won"),
    list("big_chances_ge_4",     "Matches with >=4 big chances created"),
    list("big_chances_le_2",     "Matches with <=2 big chances created"),
    list("blocked_shots_ge_4",   "Matches with >=4 opponent shots blocked"),
    list("xg_ge_1_5",            "Matches with team xG >=1.5"),
    list("xg_le_0_8",            "Matches with team xG <=0.8"),
    list("gk_saves_ge_4",        "Matches with GK making >=4 saves"),
    list("shots_inside_box_ge_8","Matches with >=8 shots from inside box"),
    list("shots_inside_box_le_5","Matches with <=5 shots from inside box"),
    list("shots_outside_box_ge_5","Matches with >=5 shots from outside box"),
    list("touches_opp_box_ge_24","Matches with >=24 touches in opposition box"),
    list("touches_opp_box_le_12","Matches with <=12 touches in opposition box"),
    list("yellow_cards_ge_3",    "Matches with >=3 yellow cards"),
    list("yellow_cards_le_1",    "Matches with <=1 yellow card"),
    list("red_cards_ge_1",       "Matches with a red card"),
    list("over_perform_ge_2",    "Matches where team scored xG+2 or more"),
    list("under_perform_ge_2",   "Matches where team scored xG-2 or fewer"))) {
    fc <- c(fc, .build_windowed(root_desc[[1]], c("home","away"),
                                c(5,7), "stats", root_desc[[2]], needs_stats = TRUE))
  }
  # And 1H / 2H halves for all except red_cards/over_perform/under_perform (which are full-match only)
  for (root_desc in list(
    list("possession_ge_55_1h",     "Matches with >=55% 1H possession"),
    list("possession_le_45_1h",     "Matches with <=45% 1H possession"),
    list("shot_accuracy_ge_40_1h",  "Matches with >=40% 1H shot accuracy"),
    list("shot_accuracy_le_25_1h",  "Matches with <=25% 1H shot accuracy"),
    list("corners_ge_7_1h",         "Matches with >=7 corners in 1H"),
    list("corners_le_5_1h",         "Matches with <=5 corners in 1H"),
    list("big_chances_ge_4_1h",     "Matches with >=4 big chances in 1H"),
    list("big_chances_le_2_1h",     "Matches with <=2 big chances in 1H"),
    list("blocked_shots_ge_4_1h",   "Matches with >=4 blocked shots in 1H"),
    list("xg_ge_1_5_1h",            "Matches with 1H xG >=1.5"),
    list("xg_le_0_8_1h",            "Matches with 1H xG <=0.8"),
    list("gk_saves_ge_4_1h",        "Matches with >=4 GK saves in 1H"),
    list("shots_inside_box_ge_8_1h","Matches with >=8 shots inside box in 1H"),
    list("shots_inside_box_le_5_1h","Matches with <=5 shots inside box in 1H"),
    list("shots_outside_box_ge_5_1h","Matches with >=5 shots outside box in 1H"),
    list("touches_opp_box_ge_24_1h","Matches with >=24 touches opp box in 1H"),
    list("touches_opp_box_le_12_1h","Matches with <=12 touches opp box in 1H"),
    list("possession_ge_55_2h",     "Matches with >=55% 2H possession"),
    list("possession_le_45_2h",     "Matches with <=45% 2H possession"),
    list("shot_accuracy_ge_40_2h",  "Matches with >=40% 2H shot accuracy"),
    list("shot_accuracy_le_25_2h",  "Matches with <=25% 2H shot accuracy"),
    list("corners_ge_7_2h",         "Matches with >=7 corners in 2H"),
    list("corners_le_5_2h",         "Matches with <=5 corners in 2H"),
    list("big_chances_ge_4_2h",     "Matches with >=4 big chances in 2H"),
    list("big_chances_le_2_2h",     "Matches with <=2 big chances in 2H"),
    list("blocked_shots_ge_4_2h",   "Matches with >=4 blocked shots in 2H"),
    list("xg_ge_1_5_2h",            "Matches with 2H xG >=1.5"),
    list("xg_le_0_8_2h",            "Matches with 2H xG <=0.8"),
    list("gk_saves_ge_4_2h",        "Matches with >=4 GK saves in 2H"),
    list("shots_inside_box_ge_8_2h","Matches with >=8 shots inside box in 2H"),
    list("shots_inside_box_le_5_2h","Matches with <=5 shots inside box in 2H"),
    list("shots_outside_box_ge_5_2h","Matches with >=5 shots outside box in 2H"),
    list("touches_opp_box_ge_24_2h","Matches with >=24 touches opp box in 2H"),
    list("touches_opp_box_le_12_2h","Matches with <=12 touches opp box in 2H"))) {
    fc <- c(fc, .build_windowed(root_desc[[1]], c("home","away"),
                                c(5,7), "stats", root_desc[[2]], needs_stats = TRUE))
  }
  ...
  fc[["h2h_home_wins"]] <- .catalog_entry("h2h", "Count H2H matches home won", side = "h2h")
  fc[["h2h_draws"]]     <- .catalog_entry("h2h", "Count H2H matches drew", side = "h2h")
  fc[["h2h_away_wins"]] <- .catalog_entry("h2h", "Count H2H matches away won", side = "h2h")
  fc[["h2h_avg_ht_goals"]] <- .catalog_entry("h2h", "Mean total 1H goals in H2H", side = "h2h")
  fc[["h2h_ht_over_05"]]   <- .catalog_entry("h2h", "Count H2H matches with 1H goal", side = "h2h")
  fc[["h2h_avg_second_h_goals"]] <- .catalog_entry("h2h", "Mean total 2H goals in H2H", side = "h2h")
  fc[["h2h_second_h_over_05"]]   <- .catalog_entry("h2h", "Count H2H matches with 2H goal", side = "h2h")
  fc[["h2h_second_h_over_15"]]   <- .catalog_entry("h2h", "Count H2H matches with 2H Over 1.5", side = "h2h")
  fc[["h2h_second_h_over_25"]]   <- .catalog_entry("h2h", "Count H2H matches with 2H Over 2.5", side = "h2h")
  fc[["h2h_ht_over_15"]]         <- .catalog_entry("h2h", "Count H2H matches with HT Over 1.5", side = "h2h")
  fc[["h2h_goalless_15"]] <- .catalog_entry("h2h", "H2H matches with no goal in first 15 min", side = "h2h")
  fc[["h2h_goalless_20"]] <- .catalog_entry("h2h", "H2H matches with no goal in first 20 min", side = "h2h")
  fc[["h2h_goalless_45"]] <- .catalog_entry("h2h", "H2H matches with no goal in first 45 min (goalless 1H)", side = "h2h")
  fc[["h2h_home_was_1up"]] <- .catalog_entry("h2h", "H2H matches where home was 1+ up at any point", side = "h2h")
  fc[["h2h_away_was_1up"]] <- .catalog_entry("h2h", "H2H matches where away was 1+ up at any point", side = "h2h")
  fc[["h2h_last_ht_over05"]]      <- .catalog_entry("h2h", "Most recent H2H had 1H goal", side = "h2h")
  fc[["h2h_last_over15"]]         <- .catalog_entry("h2h", "Most recent H2H had Over 1.5", side = "h2h")
  fc[["h2h_last_over25"]]         <- .catalog_entry("h2h", "Most recent H2H had Over 2.5", side = "h2h")
  fc[["h2h_last_btts"]]           <- .catalog_entry("h2h", "Most recent H2H had BTTS", side = "h2h")
  fc[["h2h_last_2h_over05"]]      <- .catalog_entry("h2h", "Most recent H2H had 2H Over 0.5", side = "h2h")
  fc[["h2h_last_2h_over15"]]      <- .catalog_entry("h2h", "Most recent H2H had 2H Over 1.5", side = "h2h")
  fc[["h2h_last_goalless_draw"]]  <- .catalog_entry("h2h", "Most recent H2H was 0-0 draw", side = "h2h")
  fc[["h2h_max_consec_draws"]]    <- .catalog_entry("h2h", "Consec draws in H2H from most recent", side = "h2h")
  
  # "Last match" features (single-entry from most recent form)
  for (root_desc in list(
    list("last_ht_over05",     "Most recent match had a 1H goal"),
    list("last_over15",        "Most recent match had Over 1.5"),
    list("last_over25",        "Most recent match had Over 2.5"),
    list("last_btts",          "Most recent match had BTTS"),
    list("last_2h_over05",     "Most recent match had 2H Over 0.5"),
    list("last_2h_over15",     "Most recent match had 2H Over 1.5"),
    list("last_goalless_draw", "Most recent match was 0-0 draw"),
    list("max_consec_draws",   "Consecutive draws from most recent match"))) {
    fc <- c(fc, .build_windowed(root_desc[[1]], c("home","away"),
                                c(5,7), "form", root_desc[[2]]))
  }
  ...
  rows <- lapply(fixtures, function(ef) {
    ...
    c(
      list(
        ...
        result_goal_times_home = I(list(fx$result_goal_times_home %||% list())),
        result_goal_times_away = I(list(fx$result_goal_times_away %||% list())),
