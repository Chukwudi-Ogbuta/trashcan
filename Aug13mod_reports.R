      ...
      htmltools::div(style = "display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-top:8px;",
        selectInput(ns("period"), "Period",
                    choices = c("Daily"="Daily","Weekly"="Weekly","Monthly"="Monthly",
                                "Yearly"="Yearly","All-time"="All-time",
                                "Consolidated"="Consolidated","Custom"="Custom"),
...
reports_server <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    period_range <- reactive({
      ...
      } else if (identical(p, "Consolidated")) {
        first <- db_get("select min(trip_date) as d from trips;")$d
        end_of_month <- seq(as.Date(format(a, "%Y-%m-01")), by = "month",
                            length.out = 2)[2] - 1
        list(start = if (length(first) == 0 || is.na(first)) a else as.Date(first),
             end = end_of_month)
    ...
    observeEvent(input$gen, {
      rng <- period_range()
      tryCatch({
        rep$report <- if (identical(input$period, "Consolidated")) {
          generate_haul_consolidated(rng$start, rng$end)
        } else {
          generate_haul_report(input$period, rng$start, rng$end)
        }
...
build_haul_xlsx <- function(start_date, end_date, period_label, file) {
  ...
  ## ---- Styles (slate/charcoal palette) -----------------------------------
  slate     <- "#4A5568"   # header fill
  slate_alt <- "#2D3748"   # deeper charcoal for eyebrow/section
  ice       <- "#F7FAFC"   # banded rows (very light cool)
  accent    <- "#2C5282"   # deep blue accent
  ink       <- "#1A202C"
  stone     <- "#718096"
  linen     <- "#E2E8F0"

  st_title  <- openxlsx::createStyle(fontSize = 14, fontColour = ink,
                                     fontName = "Inter", textDecoration = "bold")
  st_eyebrow <- openxlsx::createStyle(fontSize = 9, fontColour = accent,
                                      fontName = "Inter", textDecoration = "bold")
  st_sub    <- openxlsx::createStyle(fontSize = 10, fontColour = stone,
                                     fontName = "Inter")
  st_header <- openxlsx::createStyle(
    fontSize = 9, fontColour = "#FFFFFF", fgFill = slate, halign = "left",
    valign = "center", textDecoration = "bold", border = "TopBottom",
    borderColour = slate, wrapText = TRUE
  )
  st_header_num <- openxlsx::createStyle(
    fontSize = 9, fontColour = "#FFFFFF", fgFill = slate, halign = "right",
    valign = "center", textDecoration = "bold", border = "TopBottom",
    borderColour = slate
  )
  st_band    <- openxlsx::createStyle(fgFill = ice)
  ...
  st_kpi_lbl <- openxlsx::createStyle(fontSize = 9, fontColour = stone,
                                      ...
                                      valign = "center")
  st_kpi_val <- openxlsx::createStyle(fontSize = 13, fontColour = ink,
                                      textDecoration = "bold", halign = "left",
                                      valign = "center",
                                      fontName = "JetBrains Mono")
  st_section <- openxlsx::createStyle(fontSize = 10, fontColour = slate_alt,
                                      textDecoration = "bold")

  ## ---- Sheet 1: MASTER (joined view) -------------------------------------
  openxlsx::addWorksheet(wb, "Master", gridLines = FALSE, tabColour = slate)
  ...
  openxlsx::setRowHeights(wb, "Master", rows = kpi_row + 1, heights = 22)
  ...
  openxlsx::setColWidths(wb, "Master",
    cols = 1:12,
    widths = c(2, 14, 20, 14, 14, 16, 16, 22, 14, 18, 34, 2))

  ## ---- Sheet 2: TRIPS ---------------------------------------------------
  openxlsx::addWorksheet(wb, "Trips", gridLines = FALSE, tabColour = accent)
  ...
  openxlsx::setColWidths(wb, "Trips",
    cols = 1:14,
    widths = c(2, 14, 14, 14, 14, 16, 12, 18, 12, 14, 18, 18, 32, 2))

  ## ---- Sheet 3: EXPENSES ------------------------------------------------
  openxlsx::addWorksheet(wb, "Expenses", gridLines = FALSE, tabColour = accent)
  ...
  openxlsx::setColWidths(wb, "Expenses",
    cols = 1:9,
    widths = c(2, 14, 12, 15, 15, 30, 18, 36, 2))
  ...
  openxlsx::setColWidths(wb, "By Truck",
    cols = 1:8, widths = c(2, 15, 10, 16, 18, 18, 18, 2))
  ...
  openxlsx::setColWidths(wb, "By Driver",
    cols = 1:7, widths = c(2, 18, 10, 18, 20, 16, 2))
  ...
  openxlsx::setColWidths(wb, "By Route",
    cols = 1:8, widths = c(2, 24, 10, 16, 16, 20, 20, 2))
  ...
  openxlsx::setColWidths(wb, "By Category",
    cols = 1:5, widths = c(2, 40, 12, 20, 2))
...
## ============================================================================
## CONSOLIDATED HAULAGE REPORT (MoM + stacked monthly blocks)
## ============================================================================

## Cross-schema pulls -------------------------------------------------------
main_kifayat_revenue_for_month <- function(sd, ed) {
  ## Sum station revenue - station expenses - generator fuel value for the month
  ## Returns list(revenue, station_expenses, generator_value, net)
  q <- sprintf("
    with rev as (
      select coalesce(sum(pay.reported_revenue), 0) as revenue
      from public.payments pay
      join public.daily_reports dr on dr.id = pay.report_id
      where dr.report_date between '%s' and '%s'
    ),
    exp as (
      select coalesce(sum(e.amount_naira), 0) as expenses
      from public.expenses e
      join public.daily_reports dr on dr.id = e.report_id
      where dr.report_date between '%s' and '%s'
    ),
    gen as (
      select s.id as station_id, p.id as product_id,
             sum(d.litres) as litres
      from public.deductions d
      join public.daily_reports dr on dr.id = d.report_id
      join public.stations s on s.id = dr.station_id
      join public.tanks t on t.id = d.tank_id
      join public.products p on p.id = t.product_id
      where dr.report_date between '%s' and '%s'
        and d.recipient_type = 'generator'
        and p.code in ('PMS','AGO')
      group by s.id, p.id
    ),
    gen_prices as (
      select g.station_id, g.product_id, g.litres,
             (select pr.price_per_litre from public.prices pr
              where pr.station_id = g.station_id
                and pr.product_id = g.product_id
                and pr.effective_from <= date '%s'
                and (pr.effective_to is null or pr.effective_to >= date '%s')
              order by pr.effective_from desc limit 1) as price
      from gen g
    )
    select (select revenue from rev) as revenue,
           (select expenses from exp) as station_expenses,
           coalesce((select sum(litres * price) from gen_prices), 0) as generator_value;",
    sd, ed, sd, ed, sd, ed, ed, sd)
  r <- db_get(q)
  revenue          <- as.numeric(r$revenue[1])
  station_expenses <- as.numeric(r$station_expenses[1])
  generator_value  <- as.numeric(r$generator_value[1])
  list(
    revenue = revenue,
    station_expenses = station_expenses,
    generator_value = generator_value,
    station_net = revenue - station_expenses - generator_value
  )
}

## Average AGO price across stations for a month (for haulage diesel valuation)
avg_ago_price_for_period <- function(sd, ed) {
  q <- sprintf("
    select avg(pr.price_per_litre) as avg_price
    ...
    ed, sd)
  v <- db_get(q)
  price <- as.numeric(v$avg_price[1])
  if (is.na(price) || is.null(price)) 1700 else price
}

## Compute financials for a single month
haul_month_financials <- function(sd, ed) {
  d <- haul_summary(sd, ed)
  trips <- d$trips; expenses <- d$expenses
  ...
  ago_price   <- avg_ago_price_for_period(sd, ed)
  diesel_val  <- total_l * ago_price

  haulage_cost <- total_feed + total_exp + diesel_val

  main <- main_kifayat_revenue_for_month(sd, ed)
  net_after_haulage <- main$station_net - haulage_cost

  list(
    trips = n_trips, litres = total_l, feeding = total_feed,
    truck_expenses = total_exp, diesel_value = diesel_val,
    ago_price = ago_price, haulage_cost = haulage_cost,
    station_revenue = main$revenue,
    station_expenses = main$station_expenses,
    station_generator_value = main$generator_value,
    station_net = main$station_net,
    net_after_haulage = net_after_haulage
  )
}

## Overall trend chart: raw value + %change with red/green arrow
overall_trend_haul <- function(values, labels, title = "", y_format = "naira",
                               accent = "#B8862B", height = 260) {
  n <- length(values)
  if (n < 2) return("")
  W <- 700; H <- height; P_L <- 80; P_R <- 20; P_T <- 30; P_B <- 44
  max_y <- max(abs(values), 1)
  min_y <- min(0, min(values))
  y_range <- max_y - min_y

  xs <- seq_len(n)
  x_scale <- function(x) P_L + (W - P_L - P_R) * (x - 1) / max(1, (n - 1))
  y_scale <- function(y) H - P_B - (H - P_T - P_B) * ((y - min_y) / max(y_range, 1))

  pts <- paste(mapply(function(x, y) sprintf("%.1f,%.1f", x_scale(x), y_scale(y)),
                      xs, values), collapse = " ")

  ## Y gridlines
  y_steps <- pretty(c(min_y, max_y), n = 4)
  y_lines <- vapply(y_steps, function(v) {
    lbl <- if (y_format == "naira") {
      if (abs(v) >= 1e6) sprintf("\u20A6%.1fM", v / 1e6)
      else if (abs(v) >= 1e3) sprintf("\u20A6%.0fk", v / 1e3)
      else sprintf("\u20A6%s", format(round(v), big.mark = ","))
    ...
    sprintf(
      ...
      P_L - 10, y_scale(v) + 3, lbl)
  }, character(1))

  ## X labels
  x_labs <- vapply(xs, function(i) sprintf(
    '<text x="%.1f" y="%d" text-anchor="middle" font-size="10" fill="#8A8780" font-family="Inter">%s</text>',
    x_scale(i), H - 14, labels[i]), character(1))

  ## Value labels above/below each point + %change indicator
  val_labs <- vapply(seq_along(values), function(i) {
    v <- values[i]
    val_str <- if (y_format == "naira") {
      if (abs(v) >= 1e6) sprintf("\u20A6%.2fM", v / 1e6)
      else if (abs(v) >= 1e3) sprintf("\u20A6%.0fk", v / 1e3)
      else sprintf("\u20A6%s", format(round(v), big.mark = ","))
    } else format(round(v), big.mark = ",")
    ...
    if (i > 1 && !is.na(values[i-1]) && values[i-1] != 0) {
      pct <- 100 * (v - values[i-1]) / abs(values[i-1])
      arrow <- if (pct >= 0) "\u25B2" else "\u25BC"
      color <- if (pct >= 0) "#1F4D2C" else "#A8323A"
      pct_str <- sprintf(
        '<text x="%.1f" y="%.1f" text-anchor="middle" font-size="10" fill="%s" font-family="Inter">%s %.1f%%</text>',
        x_scale(i), y_scale(v) - 22, color, arrow, abs(pct))
    }
    sprintf(
      '<text x="%.1f" y="%.1f" text-anchor="middle" font-size="10.5" fill="#2A2A28" font-family="Inter" font-weight="500">%s</text>%s',
      x_scale(i), y_scale(v) - 8, val_str, pct_str)
  }, character(1))

  dots <- paste(mapply(function(x, y) sprintf(
    '<circle cx="%.1f" cy="%.1f" r="4" fill="%s" stroke="#FBFAF6" stroke-width="1.5"/>',
    x_scale(x), y_scale(y), accent), xs, values), collapse = "")
  ...
  sprintf('
    ...
    W, H, height,
    paste(y_lines, collapse = ""), pts, accent,
    dots, paste(x_labs, collapse = ""), paste(val_labs, collapse = ""))
}

## Build monthly range list from earliest data to end_date
build_month_ranges <- function(earliest, end_date) {
  first_month <- as.Date(format(earliest, "%Y-%m-01"))
  last_month  <- as.Date(format(end_date, "%Y-%m-01"))
  months <- seq(first_month, last_month, by = "month")
  lapply(months, function(m) {
    sd <- m
    ed <- seq(m, by = "month", length.out = 2)[2] - 1
    list(sd = sd, ed = ed, label = format(m, "%b %Y"))
  })
}

## Main consolidated builder
generate_haul_consolidated <- function(start_date, end_date, out_dir = "reports") {
  if (!dir.exists(out_dir)) dir.create(out_dir, recursive = TRUE)
  sd <- as.Date(start_date); ed <- as.Date(end_date)

  months <- build_month_ranges(sd, ed)
  n_months <- length(months)

  ## Compute financials per month
  monthly <- lapply(months, function(m) haul_month_financials(m$sd, m$ed))
  labels  <- vapply(months, `[[`, character(1), "label")

  net_series      <- vapply(monthly, `[[`, numeric(1), "net_after_haulage")
  cost_series     <- vapply(monthly, `[[`, numeric(1), "haulage_cost")
  trips_series    <- vapply(monthly, `[[`, numeric(1), "trips")
  feeding_series  <- vapply(monthly, `[[`, numeric(1), "feeding")
  expenses_series <- vapply(monthly, `[[`, numeric(1), "truck_expenses")
  diesel_series   <- vapply(monthly, `[[`, numeric(1), "diesel_value")

  ## Latest vs prior for KPI strip
  latest <- monthly[[n_months]]
  prior  <- if (n_months >= 2) monthly[[n_months - 1]] else NULL
  mom_pct <- function(new, old) if (is.null(old) || old == 0) NA else 100 * (new - old) / abs(old)

  fmt_pct_kpi <- function(pct) {
    if (is.na(pct)) return("")
    arrow <- if (pct >= 0) "\u25B2" else "\u25BC"
    color <- if (pct >= 0) "#1F4D2C" else "#A8323A"
    sprintf('<div style="font-size:10px;color:%s;margin-top:4px;">%s %.1f%% MoM</div>',
            color, arrow, abs(pct))
  }

  ## Build individual monthly report HTML (compact) — reuses generate_haul_report but returns HTML only
  monthly_htmls <- mapply(function(m, fin, i) {
    r <- generate_haul_report("Monthly", m$sd, m$ed, out_dir = tempdir())
    ## Extract just the body content (strip <html><head><body> wrapper for embedding)
    html <- r$html_content
    ## Prepend a mini finance card
    card <- sprintf('
      <div style="background:#FBF6E8;border-left:3px solid #B8862B;padding:16px 20px;margin:20px 0;border-radius:8px;">
        <div style="font-size:9px;letter-spacing:0.14em;color:#B8862B;text-transform:uppercase;font-weight:500;">Net revenue after haulage costs</div>
        <div style="font-size:22px;font-weight:500;margin-top:6px;letter-spacing:-0.015em;">%s</div>
        <div style="font-size:11px;color:#5C5A52;margin-top:6px;line-height:1.5;">
          Station revenue %s \u2212 station expenses %s \u2212 haulage costs %s = <strong>%s</strong>.
        </div>
        <div style="font-size:10px;color:#8A8780;margin-top:6px;font-style:italic;line-height:1.5;">
          Represents station-level revenue net of station operating expenses and haulage costs. Excludes cost of goods sold and other unallocated overheads (salaries, admin, etc.).
        </div>
      </div>', 
      fmt_n(fin$net_after_haulage),
      fmt_n(fin$station_revenue),
      fmt_n(fin$station_expenses + fin$station_generator_value),
      fmt_n(fin$haulage_cost),
      fmt_n(fin$net_after_haulage))
    ## Insert card after cover (before first h2)
    html <- sub('(<h2>Executive summary</h2>)', paste0(card, '\\1'), html, fixed = FALSE)
    ## Extract body content
    body_content <- sub('.*?<div class="wrap">(.*)</div>\\s*</body>.*', '\\1', html, perl = TRUE)
    sprintf('<section id="month-%d" class="month-block">%s</section>', i, body_content)
  }, months, monthly, seq_along(months), SIMPLIFY = TRUE)

  ## Reverse chronological order for stack
  order_rev <- rev(seq_len(n_months))
  monthly_htmls_rev <- monthly_htmls[order_rev]
  months_rev <- months[order_rev]

  ## TOC entries
  toc_items <- c(
    '<a href="#mom">Month-over-month comparison</a>',
    vapply(seq_along(months_rev), function(k) {
      i_orig <- order_rev[k]
      sprintf('<a href="#month-%d">%s</a>', i_orig, months_rev[[k]]$label)
    }, character(1))
  )
  toc_html <- paste0(
    '<div class="toc"><h2>Contents</h2><ol>',
    paste0("<li>", toc_items, "</li>", collapse = ""),
    '</ol></div>')

  ## MoM KPI strip
  kpi_html <- if (n_months >= 2) {
    sprintf('
      <div class="kpi-grid">
        <div class="kpi"><div class="kpi-label">Net revenue after haulage</div>
          <div class="kpi-value">%s</div>%s</div>
        <div class="kpi warn"><div class="kpi-label">Haulage cost</div>
          <div class="kpi-value">%s</div>%s</div>
        <div class="kpi"><div class="kpi-label">Trips</div>
          <div class="kpi-value">%d</div>%s</div>
        <div class="kpi"><div class="kpi-label">Driver feeding</div>
          <div class="kpi-value">%s</div>%s</div>
        <div class="kpi"><div class="kpi-label">Truck expenses</div>
          <div class="kpi-value">%s</div>%s</div>
        <div class="kpi alt"><div class="kpi-label">Diesel value</div>
          <div class="kpi-value">%s</div>%s</div>
      </div>',
      fmt_n(latest$net_after_haulage),  fmt_pct_kpi(mom_pct(latest$net_after_haulage, prior$net_after_haulage)),
      fmt_n(latest$haulage_cost),        fmt_pct_kpi(mom_pct(latest$haulage_cost,     prior$haulage_cost)),
      latest$trips,                      fmt_pct_kpi(mom_pct(latest$trips,            prior$trips)),
      fmt_n(latest$feeding),             fmt_pct_kpi(mom_pct(latest$feeding,          prior$feeding)),
      fmt_n(latest$truck_expenses),      fmt_pct_kpi(mom_pct(latest$truck_expenses,   prior$truck_expenses)),
      fmt_n(latest$diesel_value),        fmt_pct_kpi(mom_pct(latest$diesel_value,     prior$diesel_value)))
  } else "<div style='color:#8A8780;font-size:12px;'>MoM comparison requires at least 2 months of data.</div>"

  ## MoM trend charts
  net_chart <- if (n_months >= 2)
    paste0('<h3>Net revenue after haulage costs \u00B7 trend</h3>',
           '<p class="lead">Value and month-on-month percentage change shown at each point. Green up = growth, red down = decline.</p>',
           overall_trend_haul(net_series, labels, y_format = "naira", accent = "#1F4D2C"))
  else ""

  cost_chart <- if (n_months >= 2)
    paste0('<h3>Total haulage cost \u00B7 trend</h3>',
           '<p class="lead">Sum of feeding, truck expenses and diesel value per month.</p>',
           overall_trend_haul(cost_series, labels, y_format = "naira", accent = "#A8323A"))
  else ""

  trips_chart <- if (n_months >= 2)
    paste0('<h3>Trips \u00B7 trend</h3>',
           overall_trend_haul(trips_series, labels, y_format = "count", accent = "#B8862B"))
  else ""

  feeding_chart <- if (n_months >= 2)
    paste0('<h3>Driver feeding \u00B7 trend</h3>',
           overall_trend_haul(feeding_series, labels, y_format = "naira", accent = "#3D7050"))
  else ""

  expenses_chart <- if (n_months >= 2)
    paste0('<h3>Truck expenses \u00B7 trend</h3>',
           overall_trend_haul(expenses_series, labels, y_format = "naira", accent = "#7A5410"))
  else ""

  diesel_chart <- if (n_months >= 2)
    paste0('<h3>Diesel (AGO) value consumed by trucks \u00B7 trend</h3>',
           '<p class="lead">Litres consumed valued at average AGO price across stations for that month.</p>',
           overall_trend_haul(diesel_series, labels, y_format = "naira", accent = "#5C5A52"))
  else ""

  ## CSS shared with monthly reports
  css <- '<style>
    ...
    .wrap{max-width:820px;margin:0 auto;padding:42px 36px;}
    .cover{padding-bottom:22px;border-bottom:0.5px solid #E8E6E1;}
    .brand{display:flex;align-items:center;gap:12px;}
    .brand-mark{width:36px;height:36px;background:#B8862B;color:#FBF6E8;border-radius:8px;display:inline-flex;align-items:center;justify-content:center;font-weight:700;font-size:18px;}
    .brand-name{font-size:16px;font-weight:500;}
    .brand-sub{font-size:10px;color:#8A8780;letter-spacing:0.14em;text-transform:uppercase;margin-top:2px;}
    .cover-title{font-size:28px;font-weight:500;margin-top:20px;letter-spacing:-0.02em;}
    .cover-range{font-size:12px;color:#8A8780;margin-top:6px;}
    h2{font-size:12.5px;font-weight:500;color:#B8862B;letter-spacing:0.13em;text-transform:uppercase;margin:32px 0 12px;}
    h3{font-size:14px;font-weight:500;color:#2A2A28;margin:24px 0 8px;}
    .lead{font-size:11.5px;color:#8A8780;margin:-4px 0 12px;line-height:1.5;}
    .toc{background:#fff;border:0.5px solid #E8E6E1;border-radius:10px;padding:20px 24px;margin:30px 0;}
    .toc ol{padding-left:20px;margin:8px 0 0;font-size:13px;line-height:2;}
    .toc a{color:#B8862B;text-decoration:none;}
    .toc a:hover{text-decoration:underline;}
    .kpi-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-top:12px;}
    .kpi{background:#fff;border:0.5px solid #E8E6E1;border-radius:10px;padding:14px 16px;position:relative;overflow:hidden;}
    .kpi::before{content:"";position:absolute;left:0;top:0;bottom:0;width:3px;background:#B8862B;}
    .kpi.warn::before{background:#A8323A;}
    .kpi.alt::before{background:#3D7050;}
    .kpi-label{font-size:9px;color:#8A8780;letter-spacing:0.10em;text-transform:uppercase;font-weight:500;margin-bottom:6px;}
    .kpi-value{font-size:17px;font-weight:500;letter-spacing:-0.01em;line-height:1.1;font-variant-numeric:tabular-nums;}
    .chart-wrap{background:#fff;border:0.5px solid #E8E6E1;border-radius:10px;padding:16px;margin-top:10px;}
    .month-block{background:#fff;border:0.5px solid #E8E6E1;border-radius:14px;padding:32px 28px;margin:32px 0;box-shadow:0 1px 2px rgba(0,0,0,0.03);}
    .month-block .cover{border-bottom:none;padding-bottom:0;}
    table{width:100%;border-collapse:collapse;margin-top:6px;font-size:12.5px;font-variant-numeric:tabular-nums;}
    th{font-size:9.5px;color:#5C5A52;font-weight:500;letter-spacing:0.06em;text-transform:uppercase;text-align:left;padding:7px 9px;border-bottom:0.5px solid #E8E6E1;}
    td{padding:8px 9px;border-bottom:0.5px solid #F2F0EB;}
    td.num,th.num{text-align:right;}
    .footer{margin-top:42px;padding-top:18px;border-top:0.5px solid #E8E6E1;font-size:10.5px;color:#8A8780;letter-spacing:0.06em;text-transform:uppercase;display:flex;justify-content:space-between;}
    .footnote{font-size:10.5px;color:#8A8780;margin-top:14px;font-style:italic;line-height:1.5;}
  </style>'

  html <- paste0('<!DOCTYPE html><html><head><meta charset="utf-8"><title>KifayatHaul Consolidated</title>',
    css, '</head><body><div class="wrap">',
    ...
    '<div class="brand-mark">H</div>',
    '<div><div class="brand-name">KifayatHaul</div>',
    '<div class="brand-sub">Haulage Operations \u00B7 Consolidated</div></div></div>',
    '<div class="cover-title">Consolidated report</div>',
    '<div class="cover-range">', format(sd, "%d %b %Y"), ' \u2013 ', format(ed, "%d %b %Y"),
    ' \u00B7 ', n_months, ' months</div></div>',
    ...
    '<p class="lead">Latest month (', labels[n_months], ') vs prior month. Arrows show direction of change.</p>',
    kpi_html,
    net_chart,
    '<p class="footnote">Net revenue after haulage costs represents station-level revenue net of station operating expenses and haulage costs (feeding, truck expenses, diesel value). Excludes cost of goods sold and other unallocated overheads such as salaries and admin.</p>',
    cost_chart,
    trips_chart,
    feeding_chart,
    expenses_chart,
    ...
    ## Stacked monthly reports (reverse chrono)
    '<h2>Monthly reports</h2>',
    '<p class="lead">Most recent month first. Each block is the full monthly report with a net-revenue-after-haulage card at the top.</p>',
    paste(monthly_htmls_rev, collapse = ""),

    '<div class="footer"><div>Generated by KifayatHaul</div>',
    '<div>', format(Sys.time(), "%d %b %Y \u00B7 %H:%M"), '</div></div>',
    '</div></body></html>')

  fname_base <- sprintf("KifayatHaul_Consolidated_%s_to_%s",
                        format(sd, "%Y%m%d"), format(ed, "%Y%m%d"))
  html_path <- file.path(out_dir, paste0(fname_base, ".html"))
  writeLines(html, html_path, useBytes = TRUE)
  ...
  list(html_path = html_path, html_content = html,
       pdf_path = if (isTRUE(pdf_ok)) pdf_path else NULL,
       summary = list(period = "Consolidated", range_start = sd, range_end = ed,
                      trips = latest$trips,
                      total_cost = latest$haulage_cost,
                      total_litres = latest$litres,
                      total_feeding = latest$feeding,
                      total_expenses = latest$truck_expenses,
                      net_after_haulage = latest$net_after_haulage))
}
