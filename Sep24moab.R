...
# Load shared SportyBet helpers (live API matcher)
if (file.exists(SB_HELPERS_PATH)) {
  source(SB_HELPERS_PATH, local = FALSE)
} else {
  # Stub fallbacks so app doesn't crash if helper file missing
  filter_to_sportybet_df <- function(df, sb_dir = NULL) list(df = df, matched = rep(TRUE, nrow(df)), sb_dir = NULL)
...
tab_wait_for_page <- function(worker, handle, max_wait = 20) {
  for (i in seq_len(max_wait)) {
    ...
    if (length(title) != 1 || grepl("Just a moment|Checking", title, ignore.case = TRUE) ||
        nchar(title) == 0) next
    is_match <- tab_js(worker, handle,
                       "return window.location.pathname.startsWith('/match/');")
    if (isTRUE(is_match)) {
      score_txt <- tab_js(worker, handle, paste0(
...
parse_date <- function(s) {
  # Try dd.mm.yy (H2H page, e.g. "01.07.26")
  m2 <- regmatches(s, regexpr("\\d{1,2}\\.\\d{1,2}\\.\\d{2}(?![0-9])", s, perl = TRUE))
  if (length(m2) > 0 && nchar(m2) > 0) {
    d <- tryCatch(as.Date(m2, "%d.%m.%y"), error = function(e) NA)
    t <- regmatches(s, regexpr("\\d{1,2}:\\d{2}", s))
    return(list(date = d, time = if (length(t) > 0) t else NA))
  }
  # Fall back to dd.mm. (fixture page, e.g. "02.07. 18:00")
...
scrape_fixtures <- function(league_base, league_name, country = NA) {
  ...
  for (m in match_nodes) {
    # New DOM uses span.event__stageTime; keep div.event__time as legacy fallback
    time_node <- m %>% html_node("span.event__stageTime")
    if (is.null(time_node)) time_node <- m %>% html_node("div.event__time")
...
scrape_standings <- function(league_base, league_name) {
  ...
  for (r in rows) {
    ...
    form_nodes <- r %>% html_nodes("a.tableCellFormIcon div.wcl-badgeform_AKaAR, div.tableCellFormIcon div.wcl-badgeform_AKaAR")
...
parse_h2h_row <- function(row_node) {
  # Date: legacy span.h2h__date OR new span[data-testid='wcl-stageTime']
  date_node  <- row_node %>% html_node("span.h2h__date")
  if (is.null(date_node)) date_node <- row_node %>% html_node("span[data-testid='wcl-stageTime']")
  event_node <- row_node %>% html_node("span.h2h__event")
  # Team names: legacy inner span OR new wcl-name span inside participant div
  home_img <- row_node %>% html_node("div.h2h__homeParticipant img")
  away_img <- row_node %>% html_node("div.h2h__awayParticipant img")
  home_name <- if (!is.null(home_img)) html_attr(home_img, "alt") else NA
  away_name <- if (!is.null(away_img)) html_attr(away_img, "alt") else NA
  # Scores: legacy child span OR new wcl-tableScore span
  result_nodes <- row_node %>% html_nodes("span.h2h__result > span[data-testid='wcl-tableScore']")
  if (length(result_nodes) < 2) result_nodes <- row_node %>% html_nodes("span.h2h__result > span")
...
scrape_match_summary <- function(match_url) {
  ...
  #     <span data-testid="wcl-scores-overline-02">1st Half</span>
  #     <span data-testid="wcl-scores-overline-02"><div>0 - 2</div></span>
  #   </div>
...
parse_stats_page <- function(page) {
  ...
  # Row wrapper — data-testid is still stable
  rows <- page %>% html_nodes("div[data-testid='wcl-statistics']")
  if (length(rows) == 0) rows <- page %>% html_nodes("div.wcl-row_2oCpS")  # legacy
  for (r in rows) {
    # ── Stat name — try new DOM first, then legacy fallbacks ──
    # New: small rows use span.wcl-name_2lXWg; large rows use h4
    cat_node <- r %>% html_node("span[data-testid='wcl-scores-simple-text-01'].wcl-name_2lXWg")
    if (is.null(cat_node))
      cat_node <- r %>% html_node("h4[data-testid='wcl-scores-heading-04']")
    # Legacy fallbacks
    if (is.null(cat_node))
    ...
    # ── Values ──
    # New DOM: inside each row is a wcl-labelRow with three children:
    #   div > div.wcl-value_Ywp3J        (home value)
    #   div.wcl-label_sO4bA              (label)
    #   div.wcl-awayValue_smmfR > div.wcl-value_Ywp3J  (away value)
    ...
    if (!is.null(label_row)) {
      ...
      # Home value = the FIRST wcl-value_Ywp3J that is NOT inside wcl-awayValue_smmfR
    ...
    # Legacy fallback for old DOM matches
...
scrape_match_stats <- function(match_url) {
  ...
  # New-DOM fallback: sum 1st Half + 2nd Half from wcl-headerSection blocks
  if (is.na(result$ft_home) || is.na(result$ft_away)) {
    h1s <- NA_integer_; a1s <- NA_integer_; h2s <- NA_integer_; a2s <- NA_integer_
    blocks <- page %>% html_nodes("[data-testid='wcl-headerSection-text']")
    for (b in blocks) {
      spans <- b %>% html_nodes("span[data-testid='wcl-scores-overline-02']")
      if (length(spans) < 2) next
      label <- html_text(spans[[1]], trim = TRUE)
      score_txt <- html_text(spans[[2]], trim = TRUE)
      m <- regmatches(score_txt, regexec("(\\d+)\\s*-\\s*(\\d+)", score_txt))[[1]]
      if (length(m) < 3) next
      if (identical(label, "1st Half")) { h1s <- as.integer(m[2]); a1s <- as.integer(m[3]) }
      if (identical(label, "2nd Half")) { h2s <- as.integer(m[2]); a2s <- as.integer(m[3]) }
    }
    if (!is.na(h1s) && !is.na(a1s) && !is.na(h2s) && !is.na(a2s)) {
      result$ft_home <- h1s + h2s
      result$ft_away <- a1s + a2s
      result$status  <- "FINAL"
    }
  }
...
enrich_form_entry <- function(entry, concerned_team_name, cache, force_refresh = FALSE) {
  ...
  if (use_cache) cached <- cache[[cache_key]]
    ...
    # Retry if scrape returns NULL OR if FT scores came back NA (page didn't render).
    cached <- NULL
    for (attempt in seq_len(3)) {
      cached <- tryCatch(scrape_match_stats(entry$match_url), error = function(e) NULL)
      if (!is.null(cached) && !is.na(cached$ft_home) && !is.na(cached$ft_away)) break
      if (attempt < 3) Sys.sleep(attempt * 1.5)
    }
  ...
  entry$goal_times_concerned <- if (is_concerned_home) cached$goal_times_home else cached$goal_times_away
  entry$goal_times_opponent  <- if (is_concerned_home) cached$goal_times_away else cached$goal_times_home
...
run_league_worker <- function(worker_id, leagues_df, chrome_path, chromedriver_path,
  ...
  scrape_fixtures <- function(league_base, league_name, country = NA) {
    ...
    out <- data.frame()
    for (m in nodes) {
      tn <- m %>% html_node("span.event__stageTime")
      if (is.null(tn)) tn <- m %>% html_node("div.event__time")
      if (is.null(tn)) next
  ...
  scrape_standings <- function(league_base, league_name) {
    ...
    for (r in rows) {
      ...
      fn <- r %>% html_nodes("a.tableCellFormIcon div.wcl-badgeform_AKaAR, div.tableCellFormIcon div.wcl-badgeform_AKaAR")
      fc <- character()
      for (f in fn) {
...
run_parallel_worker <- function(worker_id, work_chunk, chrome_path, chromedriver_path,
  ...
  wait_for_page <- function(max_wait = 20) {
    for (i in seq_len(max_wait)) {
      ...
      if (length(title) != 1 || grepl("Just a moment|Checking", title, ignore.case = TRUE) ||
          nchar(title) == 0) next
      is_match <- js_eval("return window.location.pathname.startsWith('/match/');")
      if (isTRUE(is_match)) {
        score_txt <- js_eval(paste0(
          "var el = document.querySelector('div.detailScore__wrapper') || ",
          "         document.querySelector('span.detailScore__matchResult');",
          "return el ? el.innerText : '';"))
        score_txt <- if (is.null(score_txt)) "" else as.character(score_txt)[1]
  ...
  scrape_summary <- function(match_url) {
    ...
    if (is.na(ft_home) || is.na(ft_away)) {
      h1s <- NA_integer_; a1s <- NA_integer_; h2s <- NA_integer_; a2s <- NA_integer_
      blks <- page %>% html_nodes("[data-testid='wcl-headerSection-text']")
      for (b in blks) {
        sp <- b %>% html_nodes("span[data-testid='wcl-scores-overline-02']")
        if (length(sp) < 2) next
        lbl <- html_text(sp[[1]], trim = TRUE)
        st  <- html_text(sp[[2]], trim = TRUE)
        m <- regmatches(st, regexec("(\\d+)\\s*-\\s*(\\d+)", st))[[1]]
        if (length(m) < 3) next
        if (identical(lbl, "1st Half")) { h1s <- as.integer(m[2]); a1s <- as.integer(m[3]) }
        if (identical(lbl, "2nd Half")) { h2s <- as.integer(m[2]); a2s <- as.integer(m[3]) }
      }
      if (!is.na(h1s) && !is.na(a1s) && !is.na(h2s) && !is.na(a2s)) {
        ft_home <- h1s + h2s
        ft_away <- a1s + a2s
  ...
  scrape_stats <- function(match_url, skip_stats = FALSE) {
    ...
    if (skip_stats || is.na(result$ft_home) || is.na(result$ft_away)) {
      result$ft_home <- s$ft_home %||% result$ft_home
      result$ft_away <- s$ft_away %||% result$ft_away
    ...
    # Don't flag stats pages as missing when stats were intentionally skipped.
    mp <- character()
    if (!skip_stats && length(result$stats_home) == 0 && length(result$stats_away) == 0)
      mp <- c(mp, "stats_overall")
    if (!skip_stats && length(result$stats_home_1h) == 0 && length(result$stats_away_1h) == 0)
      mp <- c(mp, "stats_1h")
    if (!skip_stats && length(result$stats_home_2h) == 0 && length(result$stats_away_2h) == 0)
      mp <- c(mp, "stats_2h")
    # Summary is missing when we got FT (match is FINAL) but no goal times AND no HT.
    # For 0-0 matches goal_times will legitimately be empty, but HT should still be there.
    if (result$status == "FINAL" &&
        length(result$goal_times_home) == 0 && length(result$goal_times_away) == 0 &&
        is.na(result$ht_home))
      mp <- c(mp, "summary")
    result$missing_pages <- mp
    result
  }

  # ── patch_missing_pages: targeted re-scrape of only the sub-pages that failed ──
  # Takes an existing result that has non-empty missing_pages, re-hits ONLY those

  # sub-pages, patches the retrieved fields back in, and updates missing_pages.
  patch_missing_pages <- function(match_url, existing_result) {
    mp <- existing_result$missing_pages
    if (length(mp) == 0) return(existing_result)

    base_match_url <- sub("#/match-summary(/summary)?$", "", match_url)
    base_match_url <- sub("/$", "", base_match_url)

    patched <- existing_result
    still_missing <- character()

    # --- stats-overall ---
    if ("stats_overall" %in% mp) {
      stats_url <- paste0(base_match_url, "#/match-summary/match-statistics/0")
      navigate(stats_url)
      Sys.sleep(3)
      wait_for_page(15)
      page <- tryCatch(GET(paste0("http://localhost:", port, "/session/", session_id, "/source"),
                           timeout(15)), error = function(e) NULL)
      if (!is.null(page)) {
        html <- read_html(content(page, as = "text")$value %||% "")
        rows <- html %>% html_nodes("div._category_1rfpx_4")
        if (length(rows) > 0) {
          sh <- list(); sa <- list()
          for (r in rows) {
            lbl  <- r %>% html_node("div._category_1rfpx_4 > div:nth-child(1)") %>% html_text(trim = TRUE)
            vals <- r %>% html_nodes("div._value_s3nfq_4") %>% html_text(trim = TRUE)
            if (!is.null(lbl) && length(vals) >= 2) {
              key <- tolower(gsub("\\s+", "_", lbl))
              sh[[key]] <- vals[1]; sa[[key]] <- vals[2]
            }
          }
          if (length(sh) > 0) { patched$stats_home <- sh; patched$stats_away <- sa }
          else still_missing <- c(still_missing, "stats_overall")
        } else still_missing <- c(still_missing, "stats_overall")
      } else still_missing <- c(still_missing, "stats_overall")
    }
    ...
    # --- stats-1h ---
    if ("stats_1h" %in% mp) {
      stats_url <- paste0(base_match_url, "#/match-summary/match-statistics/1")
      navigate(stats_url)
      Sys.sleep(3)
      wait_for_page(15)
      page <- tryCatch(GET(paste0("http://localhost:", port, "/session/", session_id, "/source"),
      ...
      if (!is.null(page)) {
        ...
        if (length(rows) > 0) {
          ...
          if (length(sh) > 0) { patched$stats_home_1h <- sh; patched$stats_away_1h <- sa }
          else still_missing <- c(still_missing, "stats_1h")
        } else still_missing <- c(still_missing, "stats_1h")
      } else still_missing <- c(still_missing, "stats_1h")
    }
    ...
    # --- stats-2h ---
    if ("stats_2h" %in% mp) {
      stats_url <- paste0(base_match_url, "#/match-summary/match-statistics/2")
      navigate(stats_url)
      Sys.sleep(3)
      wait_for_page(15)
      page <- tryCatch(GET(paste0("http://localhost:", port, "/session/", session_id, "/source"),
      ...
      if (!is.null(page)) {
        ...
        if (length(rows) > 0) {
          ...
          if (length(sh) > 0) { patched$stats_home_2h <- sh; patched$stats_away_2h <- sa }
          else still_missing <- c(still_missing, "stats_2h")
        } else still_missing <- c(still_missing, "stats_2h")
      } else still_missing <- c(still_missing, "stats_2h")
    }

    # --- summary (goal times + HT) ---
    if ("summary" %in% mp) {
      summary_url <- paste0(base_match_url, "#/match-summary/match-summary")
      ...
      if (!is.null(page)) {
        html <- read_html(content(page, as = "text")$value %||% "")
        # Goal times
        gt_home <- list(); gt_away <- list()
        incidents <- html %>% html_nodes("div.smv__incident")
        for (inc in incidents) {
          minute <- inc %>% html_node("div.smv__timeBox") %>% html_text(trim = TRUE)
          is_home <- length(inc %>% html_nodes("div.smv__incidentHomeParticipant div.smv__playerName")) > 0
          pname <- inc %>% html_node("div.smv__playerName") %>% html_text(trim = TRUE)
          if (!is.null(minute) && nzchar(minute %||% "")) {
            entry <- list(minute = minute, player = pname %||% "")
            if (is_home) gt_home <- c(gt_home, list(entry))
            else gt_away <- c(gt_away, list(entry))
          }
        }
        # HT score
        ht_parts <- html %>% html_nodes("div.smv__partResult")
        ht_h <- NA; ht_a <- NA
        if (length(ht_parts) >= 1) {
          ht_text <- ht_parts[1] %>% html_text(trim = TRUE)
          ht_split <- strsplit(ht_text, "\\s*-\\s*")[[1]]
          if (length(ht_split) == 2) {
            ht_h <- suppressWarnings(as.integer(ht_split[1]))
            ht_a <- suppressWarnings(as.integer(ht_split[2]))
          }
        }
        ...
        } else still_missing <- c(still_missing, "summary")
      } else still_missing <- c(still_missing, "summary")
    }
...
ui <- fluidPage(
  ...
  div(class = "pg",
      tabsetPanel(id = "main_tabs",
                           ...
                           div(style = "padding-top:8px;",
                               ...
                               div(class = "card-elite",
                                   ...
                                   div(style = "display:flex; gap:14px; align-items:center; margin-top:14px;",
                                       ...
                                       div(style = "flex:1;"),
                                       checkboxInput("sb_scrape_filter", "SportyBet leagues only", value = TRUE),
                  ...
                  tabPanel("Flat data",
                           div(style = "padding-top:8px;",
                               div(class = "section-lab", "Flat data audit"),
                               div(class = "section-sub", "One row per fixture. All form/H2H entries exposed inline. Filter to spot what's complete vs missing."),
                               div(class = "card-elite",
                                   div(class = "card-eyebrow", "Filters"),
                                   div(style = "display:flex; gap:10px; flex-wrap:wrap; align-items:center; margin-top:12px;",
                                       textInput("flat_country_filter", "Country", placeholder = "filter..."),
                                       textInput("flat_league_filter", "League", placeholder = "filter..."),
                                       selectInput("flat_missing_filter", "Missing data?",
                                                   choices = c("All", "Any missing", "Fully complete"),
                                                   selected = "All", width = "160px"),
                                       div(style = "margin-left:auto; display:flex; gap:10px;",
                                           actionButton("flat_refresh", "Refresh from enriched", class = "btn-outline-moab"),
                                           downloadButton("dl_flat_csv", "Download CSV", class = "btn-outline-moab"),
                                           downloadButton("dl_flat_xlsx", "Download Excel", class = "btn-primary-moab")))),
                               div(class = "card-elite",
                                   div(class = "card-eyebrow", "Fixtures"),
                                   div(style = "font-family:var(--fb); font-size:12px; margin-bottom:8px;",
                                       textOutput("flat_count_status")),
                                   DT::dataTableOutput("flat_data_table")))),
                  # ─────────────── STEP 2 LOG ───────────────
                  tabPanel("Step 2 log",
                           div(style = "padding-top:8px;",
                               div(class = "section-lab", "Step 2 (deep scrape) failures"),
                               div(class = "section-sub", "Matches where the inner page failed to scrape."),
                               div(class = "card-elite",
                                   div(class = "card-eyebrow", "Summary"),
                                   uiOutput("step2_summary")),
                               div(class = "card-elite",
                                   div(class = "card-eyebrow", "Failed match URLs"),
                                   DT::dataTableOutput("step2_failures_table"),
                                   downloadButton("dl_step2_csv", "Download CSV", class = "btn-outline-moab")))),
                  # ─────────────── SPORTYBET COVERAGE ───────────────
                  tabPanel("SportyBet coverage",
                           div(style = "padding-top:8px;",
                               div(class = "section-lab", "SportyBet coverage"),
                               div(class = "section-sub", "Live match between your verified leagues and what SportyBet offers."),
                               div(class = "card-elite",
                                   div(class = "card-eyebrow", "Summary"),
                                   uiOutput("sb_coverage_summary")),
                               div(class = "card-elite",
                                   div(class = "card-eyebrow", "Per-league match status"),
                                   div(style = "display:flex; gap:10px; margin-bottom:12px;",
                                       selectInput("sb_diag_filter", "Show",
                                                   choices = c("All", "Matched (exact)", "Matched (fuzzy)", "Not matched"),
                                                   selected = "All", width = "180px"),
                                       textInput("sb_diag_country", "Country", placeholder = "filter...")),
                                   DT::dataTableOutput("sb_diagnostic_table"),
                                   downloadButton("dl_sb_diag_csv", "Download CSV", class = "btn-outline-moab")))),
                  # ─────────────── SETTINGS ───────────────
                  tabPanel("Settings",
                           div(style = "padding-top:8px;",
...
server <- function(input, output, session) {
  rv <- reactiveValues(
    ...
    # SportyBet match diagnostic (from last fetch run)
    sb_diagnostic = NULL,
    sb_directory  = NULL,
    # Step 2 failure log (match_id -> list(url, competition, recovered, ts))
    step2_failures = list(),
    # Flat data for moab data audit
    flat_data = NULL
  )
  ...
  # Flat data audit
  # ─────────────────────────────────────────────────────────────
  build_flat_data_audit <- function(enriched) {
    if (length(enriched) == 0) return(NULL)
    rws <- list()
    for (idx in seq_along(enriched)) {
      ef <- enriched[[idx]]
      fx <- ef$fixture
      if (is.null(fx) || nrow(fx) == 0) next
      
      h_form <- ef$home_form %||% list()
      a_form <- ef$away_form %||% list()
      h2h    <- ef$h2h %||% list()
      
      missing_bits <- character()
      h_ft_missing <- 0L; h_ht_missing <- 0L
      a_ft_missing <- 0L; a_ht_missing <- 0L
      h2h_ft_missing <- 0L; h2h_ht_missing <- 0L
      
      for (k in 1:7) {
        m <- if (length(h_form) >= k) h_form[[k]] else NULL
        if (is.null(m) || is.na(m$ft_for %||% NA) || is.na(m$ft_against %||% NA)) h_ft_missing <- h_ft_missing + 1L
        if (is.null(m) || is.na(m$ht_for %||% NA) || is.na(m$ht_against %||% NA)) h_ht_missing <- h_ht_missing + 1L
        m <- if (length(a_form) >= k) a_form[[k]] else NULL
        if (is.null(m) || is.na(m$ft_for %||% NA) || is.na(m$ft_against %||% NA)) a_ft_missing <- a_ft_missing + 1L
        if (is.null(m) || is.na(m$ht_for %||% NA) || is.na(m$ht_against %||% NA)) a_ht_missing <- a_ht_missing + 1L
      }
      for (k in 1:5) {
        m <- if (length(h2h) >= k) h2h[[k]] else NULL
        if (is.null(m) || is.na(m$ft_for %||% NA) || is.na(m$ft_against %||% NA)) h2h_ft_missing <- h2h_ft_missing + 1L
        if (is.null(m) || is.na(m$ht_for %||% NA) || is.na(m$ht_against %||% NA)) h2h_ht_missing <- h2h_ht_missing + 1L
      }
      
      if (h_ft_missing > 0) missing_bits <- c(missing_bits, paste0("H_ft:", h_ft_missing))
      if (h_ht_missing > 0) missing_bits <- c(missing_bits, paste0("H_ht:", h_ht_missing))
      if (a_ft_missing > 0) missing_bits <- c(missing_bits, paste0("A_ft:", a_ft_missing))
      if (a_ht_missing > 0) missing_bits <- c(missing_bits, paste0("A_ht:", a_ht_missing))
      if (h2h_ft_missing > 0) missing_bits <- c(missing_bits, paste0("h2h_ft:", h2h_ft_missing))
      if (h2h_ht_missing > 0) missing_bits <- c(missing_bits, paste0("h2h_ht:", h2h_ht_missing))
      
      row <- list(
        country = as.character(fx$country[1] %||% NA),
        league  = as.character(fx$league[1] %||% NA),
        date    = as.character(fx$fixture_date[1] %||% NA),
        home    = as.character(fx$home[1] %||% NA),
        away    = as.character(fx$away[1] %||% NA),
        missing = if (length(missing_bits) == 0) "complete" else paste(missing_bits, collapse = ", "),
        h_ft_missing = h_ft_missing,
        h_ht_missing = h_ht_missing,
        a_ft_missing = a_ft_missing,
        a_ht_missing = a_ht_missing,
        h2h_ft_missing = h2h_ft_missing,
        h2h_ht_missing = h2h_ht_missing,
        match_id = as.character(fx$match_id[1] %||% NA)
      )
      for (k in 1:7) {
        if (length(h_form) >= k) {
          m <- h_form[[k]]
          fs <- m$ft_for %||% NA; fa <- m$ft_against %||% NA
          hs <- m$ht_for %||% NA; ha <- m$ht_against %||% NA
          score <- if (!is.na(fs) && !is.na(fa)) paste0(fs, "-", fa) else ""
          ht    <- if (!is.na(hs) && !is.na(ha)) paste0(hs, "-", ha) else ""
          res   <- if (!is.na(fs) && !is.na(fa)) (if (fs > fa) "W" else if (fs < fa) "L" else "D") else ""
          venue <- if (isTRUE(m$concerned_was_home)) "h" else if (identical(m$concerned_was_home, FALSE)) "a" else ""
        } else { score <- ""; ht <- ""; res <- ""; venue <- "" }
        row[[paste0("H", k, "_score")]] <- score
        row[[paste0("H", k, "_ht")]]    <- ht
        row[[paste0("H", k, "_res")]]   <- res
        row[[paste0("H", k, "_v")]]     <- venue
      }
      for (k in 1:7) {
        if (length(a_form) >= k) {
          m <- a_form[[k]]
        ...
        row[[paste0("A", k, "_score")]] <- score
        row[[paste0("A", k, "_ht")]]    <- ht
        row[[paste0("A", k, "_res")]]   <- res
        row[[paste0("A", k, "_v")]]     <- venue
      }
      for (k in 1:5) {
        if (length(h2h) >= k) {
          m <- h2h[[k]]
        ...
        row[[paste0("H2H", k, "_score")]] <- score
        row[[paste0("H2H", k, "_ht")]]    <- ht
        row[[paste0("H2H", k, "_res")]]   <- res
        row[[paste0("H2H", k, "_v")]]     <- venue
      }
      rws[[length(rws) + 1]] <- as.data.frame(row, stringsAsFactors = FALSE)
    }
    if (length(rws) == 0) return(NULL)
    do.call(rbind, rws)
  }
  
  observe({
    e <- rv$enriched
    if (is.null(e) || length(e) == 0) { rv$flat_data <- NULL; return() }
    rv$flat_data <- build_flat_data_audit(e)
  })
  observeEvent(input$flat_refresh, {
    rv$flat_data <- build_flat_data_audit(rv$enriched)
    showNotification("Flat data refreshed", type = "message", duration = 2)
  })
  
  flat_filtered_moab <- reactive({
    fd <- rv$flat_data
    if (is.null(fd) || nrow(fd) == 0) return(NULL)
    d <- fd
    cf <- trimws(input$flat_country_filter %||% "")
    if (nzchar(cf)) d <- d[grepl(cf, d$country, ignore.case = TRUE), , drop = FALSE]
    lf <- trimws(input$flat_league_filter %||% "")
    if (nzchar(lf)) d <- d[grepl(lf, d$league, ignore.case = TRUE), , drop = FALSE]
    mf <- input$flat_missing_filter %||% "All"
    if (mf == "Any missing") d <- d[d$missing != "complete", , drop = FALSE]
    else if (mf == "Fully complete") d <- d[d$missing == "complete", , drop = FALSE]
    d
  })
  
  output$flat_count_status <- renderText({
    fd <- flat_filtered_moab()
    if (is.null(fd) || nrow(fd) == 0) return("No enriched data loaded yet.")
    n_complete <- sum(fd$missing == "complete")
    paste0(nrow(fd), " fixtures  \u00b7  ", n_complete, " complete  \u00b7  ",
           nrow(fd) - n_complete, " with missing data")
  })
  
  output$flat_data_table <- DT::renderDataTable({
    fd <- flat_filtered_moab()
    if (is.null(fd) || nrow(fd) == 0) {
      return(DT::datatable(data.frame(message = "Load enriched data to populate."),
                            options = list(dom = "t"), rownames = FALSE))
    }
    DT::datatable(fd, filter = "top", rownames = FALSE, class = "compact stripe hover",
      ...
      DT::formatStyle("missing",
        backgroundColor = DT::styleEqual("complete", "#e8f7ee", default = "#fde4e1"))
  }, server = TRUE)
  
  output$dl_flat_csv <- downloadHandler(
    filename = function() paste0("moab_flat_", format(Sys.time(), "%Y%m%d_%H%M"), ".csv"),
    content = function(file) {
      fd <- flat_filtered_moab(); if (is.null(fd)) fd <- data.frame()
      write.csv(fd, file, row.names = FALSE, na = "")
    })
  output$dl_flat_xlsx <- downloadHandler(
    filename = function() paste0("moab_flat_", format(Sys.time(), "%Y%m%d_%H%M"), ".xlsx"),
    content = function(file) {
      fd <- flat_filtered_moab(); if (is.null(fd)) fd <- data.frame(message = "No data")
      if (!requireNamespace("openxlsx", quietly = TRUE)) {
        write.csv(fd, file, row.names = FALSE, na = "")
        return()
      }
      wb <- openxlsx::createWorkbook()
      openxlsx::addWorksheet(wb, "fixtures")
      openxlsx::writeData(wb, "fixtures", fd)
      openxlsx::freezePane(wb, "fixtures", firstActiveRow = 2, firstActiveCol = 7)
      if (nrow(fd) > 0 && "missing" %in% names(fd)) {
        ok_style <- openxlsx::createStyle(fgFill = "#e8f7ee")
        bad_style <- openxlsx::createStyle(fgFill = "#fde4e1")
        ok_rows <- which(fd$missing == "complete") + 1L
        bad_rows <- which(fd$missing != "complete") + 1L
        if (length(ok_rows) > 0) openxlsx::addStyle(wb, "fixtures", ok_style, rows = ok_rows, cols = seq_len(ncol(fd)), gridExpand = TRUE)
        if (length(bad_rows) > 0) openxlsx::addStyle(wb, "fixtures", bad_style, rows = bad_rows, cols = seq_len(ncol(fd)), gridExpand = TRUE)
      }
  ...
  # Step 2 failure log
  # ─────────────────────────────────────────────────────────────
  step2_failures_df <- reactive({
    fl <- rv$step2_failures
    if (length(fl) == 0) return(NULL)
    rws <- lapply(names(fl), function(mid) {
      f <- fl[[mid]]
      data.frame(
        match_id = mid,
        url = f$url %||% NA,
        competition = f$competition %||% NA,
        recovered_from_listing = isTRUE(f$recovered_from_listing),
        timestamp = format(f$ts, "%Y-%m-%d %H:%M:%S"),
        stringsAsFactors = FALSE)
    })
    do.call(rbind, rws)
  })
  
  output$step2_summary <- renderUI({
    df <- step2_failures_df()
    if (is.null(df) || nrow(df) == 0)
      return(div(style = "padding:20px; color:var(--text-3); font-family:var(--fb);",
                  "No Step 2 failures recorded. Either the last scrape was clean, or you haven't fetched yet."))
    n <- nrow(df)
    n_rec <- sum(df$recovered_from_listing)
    div(style = "display:flex; gap:30px; padding:14px; font-family:var(--fm);",
        div(div(style = "font-size:11px; color:var(--text-3);", "TOTAL FAILURES"),
            div(style = "font-size:24px; font-weight:600;", n)),
        div(div(style = "font-size:11px; color:var(--text-3);", "FT RECOVERED FROM LISTING"),
            div(style = "font-size:24px; font-weight:600; color:#0a8a3e;", n_rec)),
        div(div(style = "font-size:11px; color:var(--text-3);", "TRUE DATA LOSS"),
            div(style = "font-size:24px; font-weight:600; color:#c0392b;", n - n_rec)))
  })
  
  output$step2_failures_table <- DT::renderDataTable({
    df <- step2_failures_df()
    if (is.null(df) || nrow(df) == 0) {
      return(DT::datatable(data.frame(message = "No failures."),
                            options = list(dom = "t"), rownames = FALSE))
    }
    DT::datatable(df, filter = "top", rownames = FALSE, class = "compact stripe hover",
                  options = list(pageLength = 25, scrollX = TRUE, dom = "lftip"))
  }, server = TRUE)
  
  output$dl_step2_csv <- downloadHandler(
    filename = function() paste0("step2_failures_", format(Sys.time(), "%Y%m%d_%H%M"), ".csv"),
    content = function(file) {
      df <- step2_failures_df(); if (is.null(df)) df <- data.frame()
  ...
  # SportyBet coverage
  # ─────────────────────────────────────────────────────────────
  output$sb_coverage_summary <- renderUI({
    diag <- rv$sb_diagnostic
    sb_dir <- rv$sb_directory
    if (is.null(diag) || nrow(diag) == 0)
      return(div(style = "padding:20px; color:var(--text-3); font-family:var(--fb);",
                  "Tick 'SportyBet leagues only' and click Fetch data to populate coverage."))
    n_sb <- if (!is.null(sb_dir)) nrow(sb_dir) else NA
    n_sb_countries <- if (!is.null(sb_dir)) length(unique(sb_dir$country)) else NA
    n_total <- nrow(diag)
    n_matched <- sum(diag$matched)
    n_exact <- sum(diag$reason == "exact")
    n_fuzzy <- sum(grepl("^fuzzy", diag$reason))
    n_dropped <- n_total - n_matched
    div(style = "display:flex; gap:30px; padding:14px; font-family:var(--fm); flex-wrap:wrap;",
        div(div(style = "font-size:11px; color:var(--text-3);", "SPORTYBET LEAGUES (API)"),
            div(style = "font-size:22px; font-weight:600;", paste0(n_sb, " across ", n_sb_countries, " countries"))),
        div(div(style = "font-size:11px; color:var(--text-3);", "YOUR VERIFIED LEAGUES"),
            div(style = "font-size:22px; font-weight:600;", n_total)),
        div(div(style = "font-size:11px; color:var(--text-3);", "MATCHED EXACT"),
            div(style = "font-size:22px; font-weight:600; color:#0a8a3e;", n_exact)),
        div(div(style = "font-size:11px; color:var(--text-3);", "MATCHED FUZZY"),
            div(style = "font-size:22px; font-weight:600; color:#dba000;", n_fuzzy)),
        div(div(style = "font-size:11px; color:var(--text-3);", "DROPPED (NO MATCH)"),
            div(style = "font-size:22px; font-weight:600; color:#c0392b;", n_dropped)))
  })
  
  sb_diag_filtered <- reactive({
    d <- rv$sb_diagnostic
    if (is.null(d) || nrow(d) == 0) return(NULL)
    f <- input$sb_diag_filter %||% "All"
    if (f == "Matched (exact)") d <- d[d$reason == "exact", , drop = FALSE]
    else if (f == "Matched (fuzzy)") d <- d[grepl("^fuzzy", d$reason), , drop = FALSE]
    else if (f == "Not matched") d <- d[!d$matched, , drop = FALSE]
    cf <- trimws(input$sb_diag_country %||% "")
    if (nzchar(cf)) d <- d[grepl(cf, d$fs_country, ignore.case = TRUE), , drop = FALSE]
    d
  })
  
  output$sb_diagnostic_table <- DT::renderDataTable({
    d <- sb_diag_filtered()
    if (is.null(d) || nrow(d) == 0) {
      return(DT::datatable(data.frame(message = "No diagnostic data."),
                            options = list(dom = "t"), rownames = FALSE))
    }
    DT::datatable(d, filter = "top", rownames = FALSE, class = "compact stripe hover",
                  options = list(pageLength = 25, scrollX = TRUE, dom = "lftip")) %>%
      DT::formatStyle("matched",
        target = "row",
        backgroundColor = DT::styleEqual(c(TRUE, FALSE), c("#e8f7ee", "#fde4e1")))
  }, server = TRUE)
  
  output$dl_sb_diag_csv <- downloadHandler(
    filename = function() paste0("sb_coverage_", format(Sys.time(), "%Y%m%d_%H%M"), ".csv"),
    content = function(file) {
      d <- sb_diag_filtered(); if (is.null(d)) d <- data.frame()
      write.csv(d, file, row.names = FALSE, na = "")
    })

  output$live_progress_card <- renderUI({
    # Self-invalidate every 2s so the card refreshes without depending on the outer timer
    if (isTRUE(rv$scrape_active)) invalidateLater(2000, session)
    live_tick()  # also refresh on outer timer tick
  ...
  outputOptions(output, "live_progress_card", suspendWhenHidden = FALSE, priority = 1000)
  ...
  observeEvent(input$fetch_btn, {
    ...
    # SportyBet filter — read from reviewed sporty_mapper.rds (source of truth)
    if (isTRUE(input$sb_scrape_filter)) {
      MAP_PATH <- file.path(BASE_PATH, "sporty_mapper.rds")
      if (!file.exists(MAP_PATH)) {
        log_msg("ERROR: sporty_mapper.rds not found. Run SB Mapper first.")
        showNotification("sporty_mapper.rds missing — open SB Mapper to build it",
                         type = "error", duration = 6)
        return()
      }
      mapper <- tryCatch(readRDS(MAP_PATH), error = function(e) NULL)
      if (is.null(mapper) || nrow(mapper) == 0) {
        log_msg("ERROR: sporty_mapper.rds is empty or unreadable.")
        showNotification("sporty_mapper.rds is empty",
                         type = "error", duration = 6)
        return()
      }
      verified_pairs <- mapper[!is.na(mapper$status) & mapper$status == "Verified" &
                                 !is.na(mapper$fs_country) & !is.na(mapper$fs_league),
                               c("fs_country", "fs_league"), drop = FALSE]
      log_msg(paste0("SB Mapper: ", nrow(verified_pairs),
                     " verified pairings loaded from sporty_mapper.rds"))
      
      before_n <- nrow(verified)
      keys_v <- paste0(tolower(verified_pairs$fs_country), "||",
                       tolower(verified_pairs$fs_league))
      keys_dir <- paste0(tolower(as.character(verified$country)), "||",
                         tolower(as.character(verified$league_name)))
      keep_mask <- keys_dir %in% keys_v
      verified <- verified[keep_mask, , drop = FALSE]
      
      # Fake diagnostic so the SportyBet coverage tab still works
      rv$sb_diagnostic <- data.frame(
        fs_country = as.character(verified$country),
        fs_league  = as.character(verified$league_name),
        matched    = TRUE,
        score      = 1.0,
        sb_country = NA_character_,
        sb_league  = NA_character_,
        reason     = "mapper-verified",
        stringsAsFactors = FALSE)
      
      log_msg(paste0("SB filter: kept ", nrow(verified), " of ", before_n,
                     " leagues (source: sporty_mapper.rds)"))
    }
    
    if (nrow(verified) == 0) {
      log_msg("ERROR: No leagues left after SportyBet filter.")
      showNotification("No leagues survived SportyBet filter", type = "error", duration = 5)
      return()
    }
    # Clear previous step-2 failures
    rv$step2_failures <- list()
    today <- Sys.Date(); horizon <- today + 6
    ...
    tryCatch({
      ...
      # Diagnostic: date parse rate
      n_valid_dates <- sum(!is.na(all_fixtures$fixture_date))
      log_msg(paste0("Fixtures with parsed dates: ", n_valid_dates,
                     " / ", nrow(all_fixtures)))
      if (n_valid_dates > 0) {
        dr <- range(all_fixtures$fixture_date, na.rm = TRUE)
        log_msg(paste0("Date range in scraped fixtures: ", dr[1], " to ", dr[2]))
      }
      log_msg(paste0("Filter window: ", today, " to ", horizon))
      ...
      # Save even if enrichment is skipped, so user can inspect
      if (nrow(in_window) == 0) {
        log_msg("WARN: No fixtures in window. Nothing to enrich.")
        log_msg("Check the Data tab for scraped fixtures and their dates.")
      ...
      if (nrow(in_window) > 0) {
        ...
        if (length(work_queue) > 0) {
          ...
          # ── Retry waves: re-fetch matches that didn't come back complete ──
          # (status != FINAL or missing FT score — same test is_valid_cache_entry
          # uses to gate the real cache). Nothing here touches `cache` yet —
          # all_results is a holding area; only what survives (original pass
          # OR a successful retry) gets written to cache after this block.
          MAX_RETRY_WAVES <- 5L
          # match_id -> original work_queue item, so a retry can re-dispatch
          # the exact same match_url/side/fixture_idx without re-deriving it.
          wq_by_id <- list()
          for (w in work_queue) wq_by_id[[w$match_id]] <- w

          for (retry_wave in seq_len(MAX_RETRY_WAVES)) {
            # --- Bucket 1: FT-missing matches (full re-scrape via worker) ---
            bad_ids <- names(all_results)[!vapply(all_results, is_valid_cache_entry, logical(1))]

            # --- Bucket 2: FT-valid but with missing sub-pages (targeted patch) ---
            partial_ids <- names(all_results)[vapply(all_results, function(d) {
              is_valid_cache_entry(d) && length(d$missing_pages %||% character()) > 0
            }, logical(1))]

            if (length(bad_ids) == 0 && length(partial_ids) == 0) break  # everything complete

            # ── Full re-scrape for FT-missing matches ──
            if (length(bad_ids) > 0) {
              retry_chunk <- Filter(Negate(is.null), lapply(bad_ids, function(mid) wq_by_id[[mid]]))
              if (length(retry_chunk) > 0) {
                log_msg(paste0("Retry wave ", retry_wave, "/", MAX_RETRY_WAVES, ": ",
                               length(retry_chunk), " match(es) missing FT, full re-scrape..."))

                n_retry_workers <- min(N_PARALLEL_WORKERS, length(retry_chunk))
                retry_worker_chunks <- if (n_retry_workers <= 1 || length(retry_chunk) <= 1) {
                  list(retry_chunk)
                } else {
                  split(retry_chunk, cut(seq_along(retry_chunk), n_retry_workers, labels = FALSE))
                }

                plan(multisession, workers = n_retry_workers)
                retry_futures <- list()
                for (i in seq_len(n_retry_workers)) {
                  local_retry_chunk <- retry_worker_chunks[[i]]
                  retry_futures[[i]] <- future({
                    run_parallel_worker(1000 + i, local_retry_chunk, chrome_p, cd_p,
                                        base_u, prog_dir, cv, ws,
                                        no_stats_lg, no_stats_thr)
                  }, seed = TRUE,
                  globals = list(i = i, local_retry_chunk = local_retry_chunk,
                                 chrome_p = chrome_p, cd_p = cd_p, base_u = base_u,
                                 prog_dir = prog_dir, cv = cv, ws = ws,
                                 no_stats_lg = no_stats_lg, no_stats_thr = no_stats_thr,
                                 run_parallel_worker = run_parallel_worker))
                ...
                for (i in seq_len(n_retry_workers)) {
                  rres <- tryCatch(value(retry_futures[[i]]), error = function(e) {
                    log_msg(paste0("Retry worker ", i, " error: ", e$message))
                    list(success = FALSE, results = list())
                  })
                  if (!is.null(rres$success) && rres$success) {
                    retry_results <- c(retry_results, rres$results)
                  }
                }
                plan(sequential)

                # Clean up this wave's worker files
                for (i in seq_len(n_retry_workers)) {
                  wid <- 1000 + i
                  for (fn in c(paste0("worker_", wid, "_progress.txt"),
                              paste0("worker_", wid, ".rds"),
                              paste0("nostats_w_", wid, ".txt"))) {
                    fp <- file.path(WORKER_PROGRESS_DIR, fn)
                    if (file.exists(fp)) {
                      if (fn == paste0("nostats_w_", wid, ".txt")) {
                        lines <- tryCatch(readLines(fp, warn = FALSE), error = function(e) character())
                        newly_flagged_retry <- trimws(lines[nchar(trimws(lines)) > 0])
                        if (length(newly_flagged_retry) > 0) {
                          existing_no_stats <- load_no_stats_leagues()
                          combined <- unique(c(existing_no_stats, newly_flagged_retry))
                          if (length(setdiff(combined, existing_no_stats)) > 0) save_no_stats_leagues(combined)
                        }
                      }
                      file.remove(fp)
                    }
                ...
                for (mid in names(retry_results)) {
                  all_results[[mid]] <- retry_results[[mid]]
                }

                n_recovered <- sum(vapply(bad_ids, function(mid) {
                  !is.null(all_results[[mid]]) && is_valid_cache_entry(all_results[[mid]])
                }, logical(1)))
                log_msg(paste0("Retry wave ", retry_wave, " (FT-missing): recovered ", n_recovered,
                               " of ", length(bad_ids), " match(es)."))
              }
            }

            # ── Targeted patch for matches with missing sub-pages ──
            # Re-check partial_ids because some may have been in bad_ids too
            # and just got fully re-scraped above.
            partial_ids <- names(all_results)[vapply(all_results, function(d) {
              is_valid_cache_entry(d) && length(d$missing_pages %||% character()) > 0
            }, logical(1))]
            ...
            if (length(partial_ids) > 0) {
              ...
              # Build patch work items: each carries the match_url + existing result
              patch_items <- lapply(partial_ids, function(mid) {
                w <- wq_by_id[[mid]]
                list(match_id = mid, match_url = w$match_url, existing = all_results[[mid]])
              })

              n_patch_workers <- min(N_PARALLEL_WORKERS, length(patch_items))
              patch_chunks <- if (n_patch_workers <= 1 || length(patch_items) <= 1) {
                list(patch_items)
              } else {
                split(patch_items, cut(seq_along(patch_items), n_patch_workers, labels = FALSE))
              }
              ...
              for (i in seq_len(n_patch_workers)) {
                ...
                patch_futures[[i]] <- future({
                  # Lightweight patch worker: starts a chromedriver, patches each item, returns results
                  ...
                  wid <- 2000 + i
                  prt <- 20000 + wid * 137 + sample(1:999, 1)
                  sid <- NULL
                  ...
                  wait_for_page_p <- function(timeout_s = 15) {
                    end_time <- Sys.time() + timeout_s
                    while (Sys.time() < end_time) {
                      state <- tryCatch({
                        r <- GET(paste0("http://localhost:", prt, "/session/", sid, "/execute/sync"),
                                 body = list(script = "return document.readyState", args = list()),
                                 encode = "json", timeout(5))
                        fromJSON(content(r, as = "text"))$value
                      }, error = function(e) "loading")
                      if (identical(state, "complete")) return(TRUE)
                      Sys.sleep(0.5)
                    }
                    FALSE
                  }
                  ...
                  for (attempt in 1:3) {
                    system2(cd_p, args = paste0("--port=", prt), wait = FALSE)
                    Sys.sleep(3)
                    resp <- tryCatch(POST(
                  ...
                  # Warm up
                  navigate_p(paste0(base_u, "/"))
                  Sys.sleep(4)
                  wait_for_page_p(30)
                  # Dismiss cookie
                  tryCatch({
                    POST(paste0("http://localhost:", prt, "/session/", sid, "/execute/sync"),
                         body = list(script = "var b=document.getElementById('onetrust-accept-btn-handler');if(b)b.click();", args = list()),
                         encode = "json", timeout(5))
                  }, error = function(e) invisible())
                  Sys.sleep(2)
                  ...
                  for (item in local_chunk) {
                    ...
                    base_match_url <- sub("#/match-summary(/summary)?$", "", item$match_url)
                    base_match_url <- sub("/$", "", base_match_url)
                    ...
                    for (pg in mp) {
                      sub_url <- switch(pg,
                        stats_overall = paste0(base_match_url, "#/match-summary/match-statistics/0"),
                        stats_1h      = paste0(base_match_url, "#/match-summary/match-statistics/1"),
                        stats_2h      = paste0(base_match_url, "#/match-summary/match-statistics/2"),
                        summary       = paste0(base_match_url, "#/match-summary/match-summary"),
                        NA)
                      if (is.na(sub_url)) next

                      navigate_p(sub_url)
                      Sys.sleep(3)
                      wait_for_page_p(15)
                      page_resp <- tryCatch(GET(paste0("http://localhost:", prt, "/session/", sid, "/source"),
                                                timeout(15)), error = function(e) NULL)
                      if (is.null(page_resp)) { still_missing <- c(still_missing, pg); next }

                      html <- read_html(fromJSON(content(page_resp, as = "text"))$value %||% "")
                      ...
                      if (pg %in% c("stats_overall", "stats_1h", "stats_2h")) {
                        ...
                        if (length(rows) > 0) {
                          ...
                          if (length(sh) > 0) {
                            if (pg == "stats_overall") { patched$stats_home <- sh; patched$stats_away <- sa }
                            else if (pg == "stats_1h") { patched$stats_home_1h <- sh; patched$stats_away_1h <- sa }
                            else if (pg == "stats_2h") { patched$stats_home_2h <- sh; patched$stats_away_2h <- sa }
                  ...
                  tryCatch(DELETE(paste0("http://localhost:", prt, "/session/", sid), timeout(5)),
                           error = function(e) invisible())
                  tryCatch(system2("pkill", args = paste0("-f 'chromedriver.*--port=", prt, "'"),
                                   stdout = FALSE, stderr = FALSE), error = function(e) invisible())
              ...
              for (i in seq_len(n_patch_workers)) {
                pres <- tryCatch(value(patch_futures[[i]]), error = function(e) {
                  log_msg(paste0("Patch worker ", i, " error: ", e$message))
                  list(success = FALSE, results = list())
                })
              ...
              log_msg(paste0("Retry wave ", retry_wave, " (targeted): patched ", n_patched,
                             " of ", length(partial_ids), " partial match(es)."))
            }
          }

          still_bad <- names(all_results)[!vapply(all_results, is_valid_cache_entry, logical(1))]
          if (length(still_bad) > 0) {
            log_msg(paste0(length(still_bad),
                           " match(es) still incomplete after all retry waves ",
                           "(left out of cache, will retry on next full run): ",
                           paste(head(still_bad, 10), collapse = ", "),
                           if (length(still_bad) > 10) "..." else ""))
          }
          still_partial <- names(all_results)[vapply(all_results, function(d) {
            is_valid_cache_entry(d) && length(d$missing_pages %||% character()) > 0
          }, logical(1))]
          if (length(still_partial) > 0) {
            log_msg(paste0(length(still_partial),
                           " match(es) cached with partial data (some sub-pages still missing): ",
                           paste(head(still_partial, 10), collapse = ", "),
                           if (length(still_partial) > 10) "..." else ""))
          }

          # Merge into main cache — matches with valid FT go in even if
          # some sub-pages are still missing (partial data > no data).
        ...
        for (k in seq_len(nrow(in_window))) {
          ...
          enrich_side <- function(form_list, team_name) {
            ...
            for (e in form_list) {
              ...
              # Track Step 2 failures (fresh scrape returned NA FT)
              if (!is.null(e$match_id) && !is.na(e$match_id) &&
                  is.null(cache[[e$match_id]]) &&
                  (is.na(r$entry$ft_for %||% NA) || is.na(r$entry$ft_against %||% NA))) {
                rv$step2_failures[[e$match_id]] <- list(
                  url = e$match_url %||% NA,
                  competition = e$competition_full %||% NA,
      ...
      rv$scrape_active <- FALSE  # ensure countdown card hides on completion
    }, error = function(e) {
      ...
      rv$scrape_active <- FALSE  # hide countdown on error too
    })
  })
  ...
  output$predictions_summary <- renderUI({
    req(rv$predictions)
    p <- rv$predictions
    counts <- as.list(sapply(p, function(d) if (is.null(d)) 0 else nrow(d)))
    total_picks <- sum(unlist(counts))
    div(class = "kpi-grid",
        div(class = "kpi-card", div(class = "kpi-num", total_picks),
