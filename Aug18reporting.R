...
report_data <- function(start_date, end_date) {
  ...
  debtor_summary <- db_get(sprintf(
    "select coalesce(deb.name, d.label, '(unnamed)') as debtor,
      ...
      group by coalesce(deb.name, d.label, '(unnamed)'), p.code
  ...
  issues <- db_get(sprintf(
    "select i.issue_type, i.severity, i.note, i.variance, s.name as station, dr.report_date
       from issues i
      ...
      order by case i.severity when 'high' then 1 when 'medium' then 2 else 3 end,
               case i.issue_type
                 when 'cash_math'                then 1
                 when 'revenue_math'             then 2
                 when 'dip_variance_cross_day'   then 3
                 when 'dip_variance_cross_block' then 4
                 when 'dip_variance_same_day'    then 5
                 when 'delivery_shortage'        then 6
                 when 'pump_meter_reverse'       then 7
                 when 'pump_continuity_gap'      then 8
                 when 'tank_dip_reset'           then 9
                 else 10 end,
               abs(coalesce(i.variance, 0)) desc,
               dr.report_date desc;", sd, ed))

  ## Generator fuel usage per station × product (PMS + AGO)
  generator <- db_get(sprintf(
    "select s.name as station, p.code as product, sum(d.litres) as litres
       from deductions d
       join daily_reports dr on dr.id = d.report_id
       join stations s on s.id = dr.station_id
       join tanks t on t.id = d.tank_id
  ...
  ## Prevailing per-product price per station (weighted by days each price active)
  gen_prices <- db_get(sprintf(
    "with bounds as (
       select s.id as station_id, s.name as station, p.code as product,
              pr.price_per_litre,
...
generate_report <- function(period_label, start_date, end_date,
  ...
  trend_svg <- ""
  if (nrow(trend) > 1) {
    trend_svg <- overall_trend_with_pct(
      values = trend$revenue,
      labels = format(trend$report_date, "%d %b"),
      title  = "",
      y_format = "naira",
      accent = "#1F4D2C",
      height = 230
    ...
    if (nzchar(trend_svg)) {
      trend_svg <- paste0('<div class="chart-wrap">', trend_svg, '</div>')
    }
  }
  ...
  ## ---- debtor list (pivoted to PMS + AGO columns) ----------------------
  debtor_rows <- if (nrow(d$debtor_summary) > 0) {
    pivot <- d$debtor_summary %>%
      dplyr::group_by(debtor) %>%
      dplyr::summarise(
        pms   = sum(litres[product == "PMS"], na.rm = TRUE),
        ago   = sum(litres[product == "AGO"], na.rm = TRUE),
        total = sum(litres, na.rm = TRUE),
        .groups = "drop") %>%
      dplyr::arrange(dplyr::desc(total))
    top <- pivot %>% dplyr::slice(seq_len(min(5, dplyr::n())))
    paste0(vapply(seq_len(nrow(top)), function(i) {
      sprintf('<tr><td><strong>%s</strong></td><td class="num">%s</td><td class="num">%s</td><td class="num">%s</td></tr>',
              top$debtor[i],
              litres(top$pms[i]),
              litres(top$ago[i]),
              litres(top$total[i]))
    }, character(1)), collapse = "")
  } else "<tr><td colspan='4' style='color:#8A8780;'>No debtor fuel issued in this period.</td></tr>"
  ...
  generator_html <- if (!is.null(d$generator) && nrow(d$generator) > 0) {
    rows <- paste(vapply(seq_len(nrow(d$generator)), function(i) sprintf(
      '<tr><td><strong>%s</strong></td><td>%s</td><td class="num">%s</td><td class="num">%s</td><td class="num">%s</td></tr>',
      d$generator$station[i],
      d$generator$product[i],
      litres(d$generator$litres[i]),
      naira(d$generator$price[i] %||% 0),
      naira(d$generator$value_naira[i])), character(1)), collapse = "")
    paste0(
      '<table><thead><tr><th>Station</th><th>Product</th><th class="num">Litres</th><th class="num">Avg price</th><th class="num">Est. value</th></tr></thead>',
      '<tbody>', rows, '</tbody></table>',
      sprintf('<p style="font-size:10.5px;color:#8A8780;margin-top:8px;font-style:italic;line-height:1.5;">Generator fuel is dispatched from station tanks (PMS and/or AGO) for in-house use, not cash sale. Litres valued at the prevailing per-product price. Total in-kind expense: <strong>%s</strong> (%s).</p>',
  ...
  html <- paste0('<!DOCTYPE html><html><head><meta charset="utf-8">',
    ...
    '<thead><tr><th>Debtor</th><th class="num">PMS</th><th class="num">AGO</th><th class="num">Total</th></tr></thead>',
    ...
    monthly_html <- paste0(
      ...
      if (nzchar(m_gen_litres_svg))
        paste0('<h3>Generator fuel litres \u00B7 trend across months</h3>',
               '<p class="lead">Total litres of fuel (PMS + AGO) consumed by station generators per month. Value at prevailing per-product price shown alongside each point.</p>',
    ...
    weekly_html <- paste0(
      ...
      if (nzchar(gen_litres_svg))
        paste0('<h3>Generator fuel litres \u00B7 trend across weeks</h3>',
               '<p class="lead">Total litres of fuel (PMS + AGO) consumed by station generators per week. Value at prevailing per-product price shown alongside each point.</p>',
               gen_litres_svg) else "",
