...
BK_HLP     <- file.path(BASE_PATH, "betking_helpers.R")

source(HELPERS)
if (file.exists(BK_HLP)) source(BK_HLP)
  ...
  div(class = "pg",
      ...
      div(class = "card",
          tabsetPanel(id = "tabs",
                      ...
                      tabPanel("BetKing mapping",
                               br(),
                               div(style = "margin-bottom:12px; display:flex; gap:12px; align-items:center;",
                                   actionButton("bk_refresh_btn", "Refresh from BetKing API",
                                                class = "btn-primary"),
                                   actionButton("bk_submit_btn", "Submit BetKing reviews",
                                                class = "btn-primary"),
                                   div(style = "flex:1;"),
                                   textOutput("bk_last_action", inline = TRUE)),
                               div(style = "margin-bottom:12px;",
                                   radioButtons("bk_filter", NULL,
                                                choices = c("Needs review" = "needs",
                                                            "Reviewed" = "reviewed",
                                                            "All" = "all"),
                                                selected = "needs", inline = TRUE)),
                               div(style = "margin-bottom:12px; font-size:12px; color:#666;",
                                   "Only Verified SB rows shown. Approve auto-match, mark as Not Available on BetKing, or Specify manually."),
                               uiOutput("bk_review_ui")),
                      tabPanel("msport mapping",
                               br(),
                               div(style = "margin-bottom:12px; display:flex; gap:12px; align-items:center;",
                                   actionButton("msp_refresh_btn", "Refresh from msport API",
                                                class = "btn-primary"),
                                   actionButton("msp_submit_btn", "Submit msport reviews",
                                                class = "btn-primary"),
                                   div(style = "flex:1;"),
                                   textOutput("msp_last_action", inline = TRUE)),
                               div(style = "margin-bottom:12px;",
                                   radioButtons("msp_filter", NULL,
                                                choices = c("Needs review" = "needs",
                                                            "Reviewed" = "reviewed",
                                                            "All" = "all"),
                                                selected = "needs", inline = TRUE)),
                               div(style = "margin-bottom:12px; font-size:12px; color:#666;",
                                   "Only Verified SB rows shown. Approve auto-match, mark as Not Available on msport, or Specify manually."),
                               uiOutput("msp_review_ui")),
...
server <- function(input, output, session) {
  ...
  MSP_HLP <- file.path(BASE_PATH, "msport_helpers.R")
  if (file.exists(MSP_HLP)) source(MSP_HLP)
  ...
  observeEvent(input$refresh_btn, {
    ...
    withProgress(message = "Fetching SB API + matching...", value = 0.2, {
      ...
      for (i in seq_along(key_new)) {
        ...
        # Preserve betking_* fields in all cases where the row already existed.
        bk_cols <- c("betking_country_id", "betking_league_id",
                     "betking_country_name", "betking_league_name",
                     "betking_match_reason", "betking_reviewed_on")
        for (col in bk_cols) {
          if (!col %in% names(new_mapper)) {
            new_mapper[[col]] <- if (col %in% c("betking_country_id",
                                                "betking_league_id"))
        ...
        msp_cols <- c("msport_country_id", "msport_league_id",
                      "msport_country_name", "msport_league_name",
                      "msport_match_reason", "msport_reviewed_on")
        for (col in msp_cols) {
          if (!col %in% names(new_mapper)) {
            new_mapper[[col]] <- NA_character_
          }
          if (col %in% names(old)) new_mapper[[col]][i] <- old[[col]][j]
        }
  ...
  # ═══ BETKING MAPPING TAB ═══════════════════════════════════
  BK_CACHE_PATH <- file.path(BASE_PATH, "betking_directory_cache.rds")
  isolate({
    rv$bk_dir <- if (file.exists(BK_CACHE_PATH))
      tryCatch(readRDS(BK_CACHE_PATH),
               error = function(e) NULL) else NULL
    rv$bk_last_action <- if (!is.null(rv$bk_dir))
      paste("Loaded cached directory (",
            nrow(rv$bk_dir), "leagues)")
    else "Not refreshed yet"
  })
  
  output$bk_last_action <- renderText(rv$bk_last_action)
  
  observeEvent(input$bk_refresh_btn, {
    req(rv$mapper)
    if (!exists("fetch_betking_directory")) {
      showNotification("betking_helpers.R not loaded.", type = "error"); return()
    }
    withProgress(message = "Fetching BetKing directory...", value = 0.3, {
      bkdir <- fetch_betking_directory()
      if (is.null(bkdir) || nrow(bkdir) == 0) {
        showNotification("BetKing API returned nothing.", type = "error"); return()
      }
      rv$bk_dir <- bkdir
      saveRDS(bkdir, BK_CACHE_PATH)
      incProgress(0.5, detail = "Matching Verified SB rows")
      diag <- match_verified_to_betking(rv$mapper, bkdir, fuzzy_threshold = 0.75)
      rv$bk_diag <- diag
      
      m <- rv$mapper
      for (col in c("betking_country_id", "betking_league_id",
                    "betking_country_name", "betking_league_name",
                    "betking_match_reason", "betking_reviewed_on")) {
        if (!col %in% names(m)) {
          m[[col]] <- if (col %in% c("betking_country_id", "betking_league_id"))
            NA_integer_ else NA_character_
        }
      }
      for (i in seq_len(nrow(diag))) {
        ...
        if (length(idx) == 0) next
        if (!is.na(m$betking_reviewed_on[idx])) next
        m$betking_country_id[idx]   <- d$betking_country_id
        m$betking_league_id[idx]    <- d$betking_league_id
        m$betking_country_name[idx] <- d$betking_country_name
        m$betking_league_name[idx]  <- d$betking_league_name
        m$betking_match_reason[idx] <- d$reason
      }
      rv$mapper <- m
      saveRDS(m, MAP_PATH)
      rv$bk_last_action <- paste("Refreshed at",
                                 format(Sys.time(), "%H:%M:%S"))
      showNotification(paste0(
        "BetKing directory: ", nrow(bkdir), " leagues \u00b7 ",
        "SB \u2192 BetKing matches: ", sum(!is.na(diag$betking_league_id)),
        " / ", nrow(diag)),
        type = "message", duration = 5)
    })
  })
  
  output$bk_review_ui <- renderUI({
    req(rv$mapper)
    m <- rv$mapper
    verified <- m[!is.na(m$status) & m$status == "Verified" &
                    !is.na(m$fs_country) & !is.na(m$fs_league), , drop = FALSE]
    if (!"betking_reviewed_on" %in% names(verified))
      verified$betking_reviewed_on <- NA_character_
    filt <- input$bk_filter %||% "needs"
    verified <- switch(filt,
                       "needs"    = verified[is.na(verified$betking_reviewed_on), , drop = FALSE],
                       "reviewed" = verified[!is.na(verified$betking_reviewed_on), , drop = FALSE],
                       verified)
    if (nrow(verified) == 0) {
    ...
    bkdir <- rv$bk_dir
    bk_countries <- if (!is.null(bkdir)) sort(unique(bkdir$country_name))
    else character()
    ...
    action_html <- vapply(seq_len(nrow(verified)), function(i) {
      rid <- row_ids[i]
      default <- if (!is.na(verified$betking_league_id[i])) "Approve"
      else "Not Available"
      paste0(
        '<select data-row="', rid, '" ',
        'onchange="Shiny.setInputValue(\'bk_action_', rid, '\', this.value, {priority:\'event\'});',
        'var ct=document.getElementById(\'bk_ct_', rid, '\');',
        'var lg=document.getElementById(\'bk_lg_', rid, '\');',
        'if(ct){ct.disabled=(this.value!==\'Specify BetKing\');}',
        'if(lg){lg.disabled=(this.value!==\'Specify BetKing\');}">',
        '<option value="Approve"', if (default == "Approve") ' selected' else '', '>Approve</option>',
        '<option value="Not Available"', if (default == "Not Available") ' selected' else '', '>Not Available on BetKing</option>',
        '<option value="Specify BetKing">Specify BetKing</option>',
        '</select>')
    }, character(1))
    
    country_html <- vapply(seq_len(nrow(verified)), function(i) {
      rid <- row_ids[i]
      default_ct <- verified$betking_country_name[i] %||%
        (if (verified$fs_country[i] %in% bk_countries)
          verified$fs_country[i] else "")
      opts <- paste0('<option value="">-- country --</option>',
                     paste0('<option value="', htmltools::htmlEscape(bk_countries),
                            '"', ifelse(bk_countries == default_ct,
                                        ' selected', ''),
                            '>', htmltools::htmlEscape(bk_countries),
                            '</option>', collapse = ""))
      paste0('<select id="bk_ct_', rid, '" disabled ',
             'onchange="Shiny.setInputValue(\'bk_specify_ct_', rid,
    ...
    league_html <- vapply(seq_len(nrow(verified)), function(i) {
      rid <- row_ids[i]
      init_ct <- verified$betking_country_name[i] %||%
        (if (verified$fs_country[i] %in% bk_countries)
          verified$fs_country[i] else NA_character_)
      choices <- if (!is.na(init_ct) && !is.null(bkdir))
        sort(unique(bkdir$league_name[
          bkdir$country_name == init_ct]))
      else character()
      opts <- paste0('<option value="">-- league --</option>',
                     paste0('<option value="', htmltools::htmlEscape(choices),
                            '">', htmltools::htmlEscape(choices),
                            '</option>', collapse = ""))
      paste0('<select id="bk_lg_', rid, '" disabled ',
             'onchange="Shiny.setInputValue(\'bk_specify_lg_', rid,
             '\', this.value, {priority:\'event\'});">',
             opts, '</select>')
    }, character(1))
    ...
    display <- data.frame(
      ...
      Proposed_BetKing = ifelse(is.na(verified$betking_league_id), "\u2014",
                                paste0(verified$betking_country_name, " \u00b7 ",
                                       verified$betking_league_name)),
      Reason          = ifelse(is.na(verified$betking_match_reason),
                               "not attempted", verified$betking_match_reason),
      Action          = action_html,
      Specify_Country = country_html,
      Specify_League  = league_html,
      Reviewed_on     = verified$betking_reviewed_on %||% NA,
      stringsAsFactors = FALSE
    )
  ...
  observe({
    ...
    for (i in seq_len(nrow(verified))) {
      local({
        rid <- row_ids[i]
        ct_input <- paste0("bk_specify_ct_", rid)
        observeEvent(input[[ct_input]], {
          ct <- input[[ct_input]]
          if (is.null(ct) || !nzchar(ct)) return()
          choices <- sort(unique(rv$bk_dir$league_name[
            rv$bk_dir$country_name == ct]))
          opts <- paste0('<option value="">-- league --</option>',
                         paste0('<option value="',
                                htmltools::htmlEscape(choices), '">',
                                htmltools::htmlEscape(choices), '</option>',
                                collapse = ""))
          session$sendCustomMessage("updateLeagueOpts",
                                    list(id = paste0("bk_lg_", rid), html = opts))
        }, ignoreInit = TRUE)
      })
    }
  })
  
  observeEvent(input$bk_submit_btn, {
    req(rv$mapper, rv$bk_dir)
    m <- rv$mapper
    ...
    for (i in seq_len(nrow(verified))) {
      row <- verified[i, ]
      rid <- row_ids[i]
      action <- input[[paste0("bk_action_", rid)]]
      if (is.null(action) || !nzchar(action)) {
        action <- if (!is.na(row$betking_league_id)) "Approve" else "Not Available"
      }
      full_idx <- which(m$sb_country == row$sb_country &
                          m$sb_league  == row$sb_league)
      if (length(full_idx) == 0) { n_skip <- n_skip + 1; next }
      ...
      if (action == "Approve") {
        if (is.na(row$betking_league_id)) { n_skip <- n_skip + 1; next }
        m$betking_reviewed_on[full_idx] <- today
        n_ok <- n_ok + 1
      } else if (action == "Not Available") {
        m$betking_country_id[full_idx]   <- NA_integer_
        m$betking_league_id[full_idx]    <- NA_integer_
        m$betking_country_name[full_idx] <- NA_character_
        m$betking_league_name[full_idx]  <- NA_character_
        m$betking_match_reason[full_idx] <- "not_available"
        m$betking_reviewed_on[full_idx]  <- today
        n_ok <- n_ok + 1
      } else if (action == "Specify BetKing") {
        ct <- input[[paste0("bk_specify_ct_", rid)]]
        lg <- input[[paste0("bk_specify_lg_", rid)]]
        if (is.null(ct) || is.na(ct) || !nzchar(ct)) ct <- row$betking_country_name
        if (is.null(ct) || is.na(ct) || !nzchar(ct)) {
          if (!is.null(rv$bk_dir) &&
              row$fs_country %in% rv$bk_dir$country_name) {
            ct <- row$fs_country
          }
        }
        if (is.null(lg) || is.na(lg) || !nzchar(lg)) lg <- row$betking_league_name
        if (is.null(ct) || is.na(ct) || !nzchar(ct) ||
            is.null(lg) || is.na(lg) || !nzchar(lg)) {
          n_skip <- n_skip + 1; next
        }
        pick <- rv$bk_dir[rv$bk_dir$country_name == ct &
                            rv$bk_dir$league_name == lg, , drop = FALSE]
        if (nrow(pick) == 0) { n_skip <- n_skip + 1; next }
        m$betking_country_id[full_idx]   <- pick$country_id[1]
        m$betking_league_id[full_idx]    <- pick$league_id[1]
        m$betking_country_name[full_idx] <- pick$country_name[1]
        m$betking_league_name[full_idx]  <- pick$league_name[1]
        m$betking_match_reason[full_idx] <- "manual"
        m$betking_reviewed_on[full_idx]  <- today
        n_ok <- n_ok + 1
      }
    }
    ...
    rv$mapper <- m
    saveRDS(m, MAP_PATH)
    rv$bk_last_action <- paste("Saved at", format(Sys.time(), "%H:%M:%S"))
    showNotification(paste0("Submitted ", n_ok, " BetKing decisions",
                            if (n_skip > 0) paste0(" (", n_skip, " skipped)") else ""),
                     type = "message", duration = 5)
  })
  
  # ═══ MSPORT MAPPING TAB ════════════════════════════════════
  MSP_CACHE_PATH <- file.path(BASE_PATH, "msport_directory_cache.rds")
  isolate({
    rv$msp_dir <- if (file.exists(MSP_CACHE_PATH))
      tryCatch(readRDS(MSP_CACHE_PATH),
               error = function(e) NULL) else NULL
    rv$msp_last_action <- if (!is.null(rv$msp_dir))
      paste("Loaded cached directory (",
            nrow(rv$msp_dir), "leagues)")
    else "Not refreshed yet"
  })
  
  output$msp_last_action <- renderText(rv$msp_last_action)
  
  observeEvent(input$msp_refresh_btn, {
    req(rv$mapper)
    if (!exists("fetch_msport_directory")) {
      showNotification("msport_helpers.R not loaded.", type = "error"); return()
    }
    withProgress(message = "Fetching msport directory...", value = 0.3, {
      mdir <- fetch_msport_directory()
      if (is.null(mdir) || nrow(mdir) == 0) {
        showNotification("msport API returned nothing.", type = "error"); return()
      }
      rv$msp_dir <- mdir
      saveRDS(mdir, MSP_CACHE_PATH)
      incProgress(0.5, detail = "Matching Verified SB rows")
      diag <- match_verified_to_msport(rv$mapper, mdir, fuzzy_threshold = 0.75)
      rv$msp_diag <- diag
      
      m <- rv$mapper
      for (col in c("msport_country_id", "msport_league_id",
                    "msport_country_name", "msport_league_name",
                    "msport_match_reason", "msport_reviewed_on")) {
        if (!col %in% names(m)) m[[col]] <- NA_character_
      }
      for (i in seq_len(nrow(diag))) {
        d <- diag[i, ]
        idx <- which(m$sb_country == d$sb_country & m$sb_league == d$sb_league)
        if (length(idx) == 0) next
        if (!is.na(m$msport_reviewed_on[idx])) next
        m$msport_country_id[idx]   <- d$msport_country_id
        m$msport_league_id[idx]    <- d$msport_league_id
        m$msport_country_name[idx] <- d$msport_country_name
        m$msport_league_name[idx]  <- d$msport_league_name
        m$msport_match_reason[idx] <- d$reason
      }
      rv$mapper <- m
      saveRDS(m, MAP_PATH)
      rv$msp_last_action <- paste("Refreshed at",
                                  format(Sys.time(), "%H:%M:%S"))
      showNotification(paste0(
        "msport directory: ", nrow(mdir), " leagues \u00b7 ",
        "SB \u2192 msport matches: ", sum(!is.na(diag$msport_league_id)),
        " / ", nrow(diag)),
        type = "message", duration = 5)
    })
  })
  
  output$msp_review_ui <- renderUI({
    req(rv$mapper)
    m <- rv$mapper
    verified <- m[!is.na(m$status) & m$status == "Verified" &
                    !is.na(m$fs_country) & !is.na(m$fs_league), , drop = FALSE]
    if (!"msport_reviewed_on" %in% names(verified))
      verified$msport_reviewed_on <- NA_character_
    filt <- input$msp_filter %||% "needs"
    verified <- switch(filt,
                       "needs"    = verified[is.na(verified$msport_reviewed_on), , drop = FALSE],
                       "reviewed" = verified[!is.na(verified$msport_reviewed_on), , drop = FALSE],
                       verified)
    if (nrow(verified) == 0) {
      return(div(style = "text-align:center; padding:32px; color:#888;",
                 if (filt == "needs") "All Verified SB rows reviewed."
    ...
    mdir <- rv$msp_dir
    msp_countries <- if (!is.null(mdir)) sort(unique(mdir$country_name))
    else character()
    ...
    action_html <- vapply(seq_len(nrow(verified)), function(i) {
      rid <- row_ids[i]
      default <- if (!is.na(verified$msport_league_id[i])) "Approve"
      else "Not Available"
      paste0(
        '<select data-row="', rid, '" ',
        'onchange="Shiny.setInputValue(\'msp_action_', rid, '\', this.value, {priority:\'event\'});',
        'var ct=document.getElementById(\'msp_ct_', rid, '\');',
        'var lg=document.getElementById(\'msp_lg_', rid, '\');',
        'if(ct){ct.disabled=(this.value!==\'Specify msport\');}',
        'if(lg){lg.disabled=(this.value!==\'Specify msport\');}">',
        '<option value="Approve"', if (default == "Approve") ' selected' else '', '>Approve</option>',
        '<option value="Not Available"', if (default == "Not Available") ' selected' else '', '>Not Available on msport</option>',
        '<option value="Specify msport">Specify msport</option>',
        '</select>')
    }, character(1))
    
    country_html <- vapply(seq_len(nrow(verified)), function(i) {
      rid <- row_ids[i]
      default_ct <- verified$msport_country_name[i] %||%
        (if (verified$fs_country[i] %in% msp_countries)
          verified$fs_country[i] else "")
      opts <- paste0('<option value="">-- country --</option>',
                     paste0('<option value="', htmltools::htmlEscape(msp_countries),
                            '"', ifelse(msp_countries == default_ct,
                                        ' selected', ''),
                            '>', htmltools::htmlEscape(msp_countries),
                            '</option>', collapse = ""))
      paste0('<select id="msp_ct_', rid, '" disabled ',
             'onchange="Shiny.setInputValue(\'msp_specify_ct_', rid,
             '\', this.value, {priority:\'event\'});">',
             opts, '</select>')
    }, character(1))
    ...
    league_html <- vapply(seq_len(nrow(verified)), function(i) {
      rid <- row_ids[i]
      init_ct <- verified$msport_country_name[i] %||%
        (if (verified$fs_country[i] %in% msp_countries)
          verified$fs_country[i] else NA_character_)
      choices <- if (!is.na(init_ct) && !is.null(mdir))
        sort(unique(mdir$league_name[
          mdir$country_name == init_ct]))
      else character()
      opts <- paste0('<option value="">-- league --</option>',
                     paste0('<option value="', htmltools::htmlEscape(choices),
                            '">', htmltools::htmlEscape(choices),
                            '</option>', collapse = ""))
      paste0('<select id="msp_lg_', rid, '" disabled ',
             'onchange="Shiny.setInputValue(\'msp_specify_lg_', rid,
             '\', this.value, {priority:\'event\'});">',
             opts, '</select>')
    }, character(1))
    ...
    display <- data.frame(
      ...
      SB_League        = verified$sb_league,
      FS_League        = verified$fs_league,
      Proposed_msport  = ifelse(is.na(verified$msport_league_id), "\u2014",
                                paste0(verified$msport_country_name, " \u00b7 ",
                                       verified$msport_league_name)),
      Reason           = ifelse(is.na(verified$msport_match_reason),
                                "not attempted", verified$msport_match_reason),
      Action           = action_html,
      Specify_Country  = country_html,
      Specify_League   = league_html,
      Reviewed_on      = verified$msport_reviewed_on %||% NA,
      stringsAsFactors = FALSE
    )
  ...
  observe({
    ...
    for (i in seq_len(nrow(verified))) {
      local({
        rid <- row_ids[i]
        ct_input <- paste0("msp_specify_ct_", rid)
        observeEvent(input[[ct_input]], {
          ct <- input[[ct_input]]
          if (is.null(ct) || !nzchar(ct)) return()
          choices <- sort(unique(rv$msp_dir$league_name[
            rv$msp_dir$country_name == ct]))
          opts <- paste0('<option value="">-- league --</option>',
                         paste0('<option value="',
                                htmltools::htmlEscape(choices), '">',
                                htmltools::htmlEscape(choices), '</option>',
                                collapse = ""))
          session$sendCustomMessage("updateLeagueOpts",
                                    list(id = paste0("msp_lg_", rid), html = opts))
        }, ignoreInit = TRUE)
      })
    }
  })
  
  observeEvent(input$msp_submit_btn, {
    req(rv$mapper, rv$msp_dir)
    m <- rv$mapper
    ...
    for (i in seq_len(nrow(verified))) {
      row <- verified[i, ]
      rid <- row_ids[i]
      action <- input[[paste0("msp_action_", rid)]]
      if (is.null(action) || !nzchar(action)) {
        action <- if (!is.na(row$msport_league_id)) "Approve" else "Not Available"
      }
      full_idx <- which(m$sb_country == row$sb_country &
                          m$sb_league  == row$sb_league)
      if (length(full_idx) == 0) { n_skip <- n_skip + 1; next }
      ...
      if (action == "Approve") {
        if (is.na(row$msport_league_id)) { n_skip <- n_skip + 1; next }
        m$msport_reviewed_on[full_idx] <- today
        n_ok <- n_ok + 1
      } else if (action == "Not Available") {
        m$msport_country_id[full_idx]   <- NA_character_
        m$msport_league_id[full_idx]    <- NA_character_
        m$msport_country_name[full_idx] <- NA_character_
        m$msport_league_name[full_idx]  <- NA_character_
        m$msport_match_reason[full_idx] <- "not_available"
        m$msport_reviewed_on[full_idx]  <- today
        n_ok <- n_ok + 1
      } else if (action == "Specify msport") {
        ct <- input[[paste0("msp_specify_ct_", rid)]]
        lg <- input[[paste0("msp_specify_lg_", rid)]]
        if (is.null(ct) || is.na(ct) || !nzchar(ct)) ct <- row$msport_country_name
        if (is.null(ct) || is.na(ct) || !nzchar(ct)) {
          if (!is.null(rv$msp_dir) &&
              row$fs_country %in% rv$msp_dir$country_name) {
            ct <- row$fs_country
          }
        }
        if (is.null(lg) || is.na(lg) || !nzchar(lg)) lg <- row$msport_league_name
        if (is.null(ct) || is.na(ct) || !nzchar(ct) ||
            is.null(lg) || is.na(lg) || !nzchar(lg)) {
          n_skip <- n_skip + 1; next
        }
        pick <- rv$msp_dir[rv$msp_dir$country_name == ct &
                             rv$msp_dir$league_name == lg, , drop = FALSE]
        if (nrow(pick) == 0) { n_skip <- n_skip + 1; next }
        m$msport_country_id[full_idx]   <- pick$country_id[1]
        m$msport_league_id[full_idx]    <- pick$league_id[1]
        m$msport_country_name[full_idx] <- pick$country_name[1]
        m$msport_league_name[full_idx]  <- pick$league_name[1]
        m$msport_match_reason[full_idx] <- "manual"
        m$msport_reviewed_on[full_idx]  <- today
        n_ok <- n_ok + 1
      }
    }
    ...
    rv$mapper <- m
    saveRDS(m, MAP_PATH)
    rv$msp_last_action <- paste("Saved at", format(Sys.time(), "%H:%M:%S"))
    showNotification(paste0("Submitted ", n_ok, " msport decisions",
                            if (n_skip > 0) paste0(" (", n_skip, " skipped)") else ""),
                     type = "message", duration = 5)
  })
